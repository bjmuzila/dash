...
function noteRestApiPull(apiPath) {
  ...
  entry.stalled = false;
  warn(`REST API ${formatEtTimestamp(now)} ET | ${apiPath} | hit #${entry.count} | gap ${gapSeconds === 'first' ? gapSeconds : `${gapSeconds}s`}`);
}
...
async function ensureTodaySpxOptionSubscriptions() {
  ...
  spx0dteEnsurePromise = (async () => {
    ...
    const dxSpot = firstFiniteNumber(dxQuoteCache['SPX']?.last, dxQuoteCache['SPX']?.bidPrice, dxQuoteCache['SPX']?.askPrice, 0);
      ...
      if (fieldCount > 0) {
            for (let i = 0; i < rows.length; i += fieldCount) {
              ...
              if (eventType === 'Greeks') {
                ...
                dxTradeCache[sym] = dxTradeCache[sym] || {};
                timeAndSaleFields.forEach((f, j) => dxTradeCache[sym][f] = rows[base + j]);
                cacheTradeAsFiveMinuteCandle(sym, dxTradeCache[sym].price, dxTradeCache[sym].size);
              }
            }
          }
...
const server = http.createServer(async (req, res) => {
  ...
  if (req.method === 'GET' && p === '/proxy/api/tt/gex-top-3') {
    ...
    for (const [sym, greeks] of Object.entries(dxGreeksCache)) {
      ...
      const oi = maxWholeNumber(summary.openInterest);
      const isCall = /C\d{4,8}$/.test(sym);
      if (!strikeMap[strike]) strikeMap[strike] = { strike, callGEX: 0, putGEX: 0, netGEX: 0 };
  ...
  if (req.method === 'GET' && p === '/proxy/api/tt/quotes-batch') {
    ...
    // Fetch equity prev-closes from TT market-data endpoint
    const equitySyms = allSyms.filter(s => !s.startsWith('/') && !['SPX','VIX','NDX'].includes(s));
    let marketDataMap = {};
    if (equitySyms.length > 0) {
      try {
        const equityParam = equitySyms.map(s => `equity[]=${encodeURIComponent(s)}`).join('&');
        const resp = await ttGet(`/market-data/by-type?${equityParam}`);
        (resp.data?.data?.items || resp.data?.items || []).forEach(item => {
          if (item.symbol && item['previous-close']) {
            marketDataMap[item.symbol] = parseFloat(item['previous-close']);
          }
        });
      } catch(e) {
        console.warn('[Quotes-batch] Market-data fetch failed:', e.message);
      }
    }
  ...
  if (req.method === 'GET' && p.startsWith('/proxy/api/tt/chains/')) {
    ...
    // Keep the lightweight SPY block for streaming callers, but allow
    // read-only requests like estimated moves to fetch the chain.
    if (/^SPY$/i.test(sym) && !noSubscribe) {
    ...
    const dxCacheEntry = dxQuoteCache[dxSpotKey] || dxQuoteCache[sym] || {};
    const dxSpotPrice = firstFiniteNumber(dxCacheEntry.last, dxCacheEntry.bidPrice, dxCacheEntry.askPrice, 0);
    // Use quote first, fallback to dxLink spot, then hardcoded
    const underlyingPrice = firstFiniteNumber(quotePrice, dxSpotPrice, isSpxFamily ? 5800 : 0);
    ...
    const streamerSyms = noSubscribe ? [] : pickNearestOptionStreamerSymbols(filteredOptions, underlyingPrice);
    if (streamerSyms.length && dxSocket && dxSocket.readyState === WebSocket.OPEN && dxChannelOpen) {
      ...
      sendSubscriptionsRateLimited();
      log('Subscribed', streamerSyms.length, 'nearest option symbols to dxLink. Waiting for caches to populate...');
    }
    ...
    for (const opt of filteredOptions) {
      ...
      const liveIv    = finiteNumber(greeks.volatility, opt['implied-volatility'], opt['iv']);
    ...
    // dxLink only returns openInterest on put-side Summary for equity options.
    // Cross-fill: if call OI is 0 but put OI exists (or vice versa), share it.
    for (const eg of Object.values(expMap)) {
      for (const strikeObj of Object.values(eg.strikes)) {
        const callOI = strikeObj.call?.['open-interest'] || 0;
        const putOI  = strikeObj.put?.['open-interest']  || 0;
        if (callOI === 0 && putOI > 0 && strikeObj.call) strikeObj.call['open-interest'] = putOI;
        if (putOI  === 0 && callOI > 0 && strikeObj.put)  strikeObj.put['open-interest']  = callOI;
      }
    }
    ...
    if (cached) {
      return sendJSON(res, 200, {
        quote: {
          ...
          closePrice: cached.prevDayClosePrice || cached.dayClosePrice || 0
        }
      });
    }
...
wss.on('connection', ws => {
  ...
  setTimeout(() => {
    ...
    ['SPX','VIX','/ES:XCME','/NQ:XCME','QQQ','SMH','AAPL','AMD','AMZN','GOOGL','META','MSFT','NVDA','TSLA'].forEach(sym => {
      ...
      if (dxSummaryCache[sym]) {
        const s = dxSummaryCache[sym];
        const prevClose = marketDataPrevCloseCache[sym] || s.prevDayClosePrice || 0;
        ws.send(JSON.stringify({ type:'FEED_DATA', data:[{
          eventType: 'Summary', eventSymbol: sym,
          openInterest: s.openInterest, dayVolume: s.dayVolume,
          dayOpenPrice: s.dayOpenPrice, dayHighPrice: s.dayHighPrice,
          dayLowPrice: s.dayLowPrice, prevDayClosePrice: prevClose
        }] }));
      }
    });
...
async function prewarmCache() {
  ...
  try {
    ...
    const indexSymbols = ['SPX','VIX','NDX','/ES:XCME','/NQ:XCME','SPY','QQQ','SMH','AAPL','AMD','AMZN','GOOGL','META','MSFT','NVDA','TSLA','COIN','HOOD','IWM','NFLX','PLTR'];
    indexSymbols.forEach(sym => addAutoSubscription(sym, ['Quote','Trade','TradeETH','Summary']));
...
server.listen(PORT, async () => {
  ...
  if (ok) {
    ...
    if (dxOk) {
      // Seed market-data prev-closes at startup
      try {
