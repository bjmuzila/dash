// Extracted verbatim from app/api/tpo-forecast/route.ts for the in-process API
// router. Only change: no next/server; pure fn taking URLSearchParams → {status,body}.
// Bundle: esbuild lib/tpo-forecast-compute.ts --bundle --platform=node --format=cjs --external:pg --outfile=server-v2/_lib-tpo-forecast.cjs
...
export async function computeTpoForecast(
  ...
  try {
    ...
    try {
      ...
      return { status: 200, body: { ok: false, status: "accumulating", nHistory: 0, need: LIVE_MIN,
    ...
    if (hist.length < LIVE_MIN) {
      return { status: 200, body: { ok: false, status: "accumulating", nHistory: hist.length, need: LIVE_MIN,
        note: "The forecast lights up once the recorder (or a one-time backfill) has enough sessions." } };
    }
    if (!todaySess || !ibDone) {
      return { status: 200, body: { ok: false, status: "pre_ib", nHistory: hist.length,
