// Extracted verbatim from app/api/confidence/route.ts so the in-process API
// router (server-v2/api-router.js) can run the exact same logic. The ONLY
// changes vs the route: no next/server import, the handler is a pure function
// taking URLSearchParams and returning { status, body, headers } instead of a
// NextResponse. Bundled to server-v2/_lib-confidence-route.cjs via:
