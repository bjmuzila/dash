// One-day promo: full annual CB Edge access for $300 with code NOPANTS.
//
// Celebration promo — both of the founder's kids are in school full time.
// Only 2 promo codes available, today only. Invoice-style layout (list price,
// discount line, total due today) + big CTA, rendered in the dashboard theme.
//
...
export interface NoPantsPromoOpts {
  ...
  listPrice?: number;
  /** Number of codes released. Defaults to 2. */
  spots?: number;
  /** Promo code. Defaults to NOPANTS. */
  code?: string;
}

export const NOPANTS_PROMO_SUBJECT = "Both kids in school — 2 spots at $300/yr (code NOPANTS)";
...
/** Plain-text fallback. */
export function noPantsPromoText(opts: NoPantsPromoOpts = {}): string {
  const cta = opts.ctaUrl || SIGN_UP_URL;
  const price = opts.price ?? 300;
  const listPrice = opts.listPrice ?? 1000;
  const spots = opts.spots ?? 2;
  const code = opts.code || "NOPANTS";
  return [
    "CB EDGE — REAL EDGE. REAL ORDERFLOW.",
    "",
    `LIMITED PROMO · ${spots} SPOTS REMAINING`,
    "",
    `$${price}/yr — full annual access to cbedge.net`,
    "",
    `Both of my kids are in school full time now. To celebrate the quiet house, I'm releasing ${spots} promo codes for full annual access at $${price}.`,
    "",
    `USE CODE: ${code}`,
    "",
    ...
    `Claim a spot: ${cta}`,
    "",
    `Today only. Max ${spots} redemptions — last time these sold out within minutes.`,
    "",
...
/** Branded HTML promo email. */
export function noPantsPromoEmail(opts: NoPantsPromoOpts = {}): string {
  ...
  const listPrice = opts.listPrice ?? 1000;
  const spots = opts.spots ?? 2;
  const code = escapeHtml(opts.code || "NOPANTS");
...
<title>${escapeHtml(NOPANTS_PROMO_SUBJECT)}</title>
</head>
<body style="margin:0;padding:0;background:#05060A;">
  <!-- preheader (hidden) -->
  <div style="display:none;max-height:0;overflow:hidden;opacity:0;">Both kids in school full time — ${spots} codes for full annual access at $${price}. Today only.</div>
        ...
        <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="max-width:560px;background:#0D1119;border:1px solid rgba(255,255,255,0.10);border-radius:16px;overflow:hidden;">
          ...
          <tr>
            <td align="center" style="padding:20px 28px 0 28px;">
              <div style="font:900 34px/1.05 -apple-system,Segoe UI,Roboto,Helvetica,Arial,sans-serif;color:#ffffff;letter-spacing:-0.01em;">CB EDGE</div>
              <div style="font:700 11px/1 -apple-system,Segoe UI,Roboto,Helvetica,Arial,sans-serif;letter-spacing:0.22em;text-transform:uppercase;color:#8ECAE6;margin-top:10px;">"Real Edge &middot; Real Orderflow"</div>
            </td>
          </tr>
          ...
          <tr>
            <td align="center" style="padding:22px 28px 0 28px;">
              <div style="font:800 13px/1 -apple-system,Segoe UI,Roboto,Helvetica,Arial,sans-serif;letter-spacing:0.14em;text-transform:uppercase;color:#219EBC;">Limited promo &middot; ${spots} spots remaining</div>
            </td>
          </tr>
          ...
          <!-- BODY -->
          <tr>
            <td align="center" style="padding:18px 34px 0 34px;">
              <div style="font:400 14px/1.7 -apple-system,Segoe UI,Roboto,Helvetica,Arial,sans-serif;color:#d4dde6;">
                Both of my kids are in school full time now — the house is finally quiet and the charts finally get my whole day. To celebrate, I'm releasing <strong style="color:#ffffff;">${spots} promo codes</strong> for full annual access at <strong style="color:#8ECAE6;">$${price}</strong>.
              </div>
            </td>
          </tr>
          ...
          <tr>
            <td align="center" style="padding:14px 34px 8px 34px;">
              <div style="font:400 12px/1.6 -apple-system,Segoe UI,Roboto,Helvetica,Arial,sans-serif;color:#6b7d8f;">
                Today only — ends tonight at midnight. Max ${spots} redemptions.<br>Last time these were gone within minutes.
              </div>
            </td>
          </tr>
