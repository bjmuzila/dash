...
// midnight-300.ts, minus the countdown/scarcity banner.
//
...
export function edge3AnnualText(opts: Edge3AnnualOpts = {}): string {
  ...
  return [
    ...
    "",
    `A full year of CB Edge for $${price} instead of $${listPrice.toLocaleString("en-US")}.`,
    "",
    `That is ${pct}% off the annual price — roughly $${Math.round(price / 12)}/month for every`,
    "page on the site: GEX, orderflow, the scanner, estimated moves, levels, the",
    "mobile build, and everything that ships next.",
    "",
    `USE CODE: ${code}`,
    "",
    `CB Edge Access (billed annually)   $${listPrice.toLocaleString("en-US")}.00`,
    `Today (${code})                    -$${off.toLocaleString("en-US")}.00`,
    `Total due today                    $${price}.00`,
    "",
    `Get the year: ${cta}`,
    "",
    "No countdown, no spot cap. Apply the code at checkout whenever you're ready.",
    "",
...
<body style="margin:0;padding:0;background:#05060A;">
  <!-- preheader (hidden) -->
  <div style="display:none;max-height:0;overflow:hidden;opacity:0;">A full year of CB Edge for $${price} instead of $${listPrice.toLocaleString("en-US")} — code ${code}, no deadline.</div>
        ...
        <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="max-width:560px;background:#0D1119;border:1px solid rgba(255,255,255,0.10);border-radius:16px;overflow:hidden;">
          ...
          <tr>
            <td align="center" style="padding:18px 28px 0 28px;">
              <div style="font:900 32px/1.1 -apple-system,Segoe UI,Roboto,Helvetica,Arial,sans-serif;color:#ffffff;letter-spacing:-0.01em;">A full year of CB Edge <span style="color:#219EBC;">for $${price}.</span></div>
              <div style="font:600 14px/1.6 -apple-system,Segoe UI,Roboto,Helvetica,Arial,sans-serif;color:#d4dde6;margin-top:12px;">
                The annual plan is $${listPrice.toLocaleString("en-US")}. Code <strong style="color:#8ECAE6;">${code}</strong> takes it to $${price} — about <strong style="color:#8ECAE6;">$${perMonth}/month</strong> for the whole site.
              </div>
            </td>
          </tr>
          ...
          <tr>
            <td style="padding:22px 28px 0 28px;">
              <div style="font:400 14px/1.9 -apple-system,Segoe UI,Roboto,Helvetica,Arial,sans-serif;color:#d4dde6;">
                <span style="color:#219EBC;font-weight:800;">&#8250;</span>&nbsp; Live GEX — chart, heatmap, walls, levels<br>
                <span style="color:#219EBC;font-weight:800;">&#8250;</span>&nbsp; Real orderflow tape and the flow scanner<br>
                <span style="color:#219EBC;font-weight:800;">&#8250;</span>&nbsp; Estimated moves, option chain, multi-greek<br>
                <span style="color:#219EBC;font-weight:800;">&#8250;</span>&nbsp; The phone build — same data, built for a phone<br>
                <span style="color:#219EBC;font-weight:800;">&#8250;</span>&nbsp; Every page that ships during your year
              </div>
            </td>
          </tr>
              ...
              <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0">
                <tr>
                  <td align="center" style="border-radius:12px;background:#219EBC;">
                    <a href="${cta}" style="display:block;padding:17px 24px;font:900 16px/1 -apple-system,Segoe UI,Roboto,Helvetica,Arial,sans-serif;letter-spacing:0.04em;text-transform:uppercase;color:#05060A;text-decoration:none;border-radius:12px;">Get the year for $${price}</a>
                  </td>
                </tr>
              </table>
            </td>
          ...
          <tr>
            <td align="center" style="padding:14px 34px 8px 34px;">
              <div style="font:400 12px/1.6 -apple-system,Segoe UI,Roboto,Helvetica,Arial,sans-serif;color:#6b7d8f;">
                No countdown, no spot cap.<br>Apply <strong style="color:#8ECAE6;">${code}</strong> at checkout whenever you're ready.
              </div>
            </td>
          </tr>
