// Follow-up to the NOPANTS promo: the first batch of codes was claimed within
// 30 minutes, so a second (final) batch is being released at the same price.
//
// Same invoice-style layout and dashboard theme as nopants-promo.ts, with a
// "SOLD OUT IN 30 MINUTES" proof banner up top and a hard final-call framing.
//
...
export interface NoPantsExtensionOpts {
  ...
  listPrice?: number;
  /** How many codes are being released in this second batch. Defaults to 3. */
  spots?: number;
  /** How many went in the first batch. Defaults to 2. */
  soldSpots?: number;
  /** How fast the first batch went, in minutes. Defaults to 30. */
  soldMinutes?: number;
...
export const NOPANTS_EXTENSION_SUBJECT = "Gone in 30 minutes — releasing 3 more at $300 (NOPANTS)";
...
/** Plain-text fallback. */
export function noPantsExtensionText(opts: NoPantsExtensionOpts = {}): string {
  const cta = opts.ctaUrl || SIGN_UP_URL;
  const price = opts.price ?? 300;
  const listPrice = opts.listPrice ?? 1000;
  const spots = opts.spots ?? 3;
  const soldSpots = opts.soldSpots ?? 2;
  const soldMinutes = opts.soldMinutes ?? 30;
  const code = opts.code || "NOPANTS";
  const deadline = opts.deadline || "Extended through tonight at midnight.";
  return [
    "CB EDGE — REAL EDGE. REAL ORDERFLOW.",
    "",
    `SOLD OUT IN ${soldMinutes} MINUTES`,
    "",
    `Both ${soldSpots} codes were claimed in under ${soldMinutes} minutes. Enough of you asked that I'm releasing ${spots} more — same price, same code, and that's the last of them.`,
    "",
    ...
    `Claim a spot: ${cta}`,
    "",
    `${deadline} Max ${spots} redemptions — when they're gone, they're gone.`,
    "",
...
/** Branded HTML extension/final-call email. */
export function noPantsExtensionEmail(opts: NoPantsExtensionOpts = {}): string {
  ...
  const deadline = escapeHtml(opts.deadline || "Extended through tonight at midnight.");
...
<title>${escapeHtml(NOPANTS_EXTENSION_SUBJECT)}</title>
</head>
<body style="margin:0;padding:0;background:#05060A;">
  <!-- preheader (hidden) -->
  <div style="display:none;max-height:0;overflow:hidden;opacity:0;">The first ${soldSpots} went in ${soldMinutes} minutes — ${spots} more codes at $${price}/yr, then it's done.</div>
        ...
        <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="max-width:560px;background:#0D1119;border:1px solid rgba(255,255,255,0.10);border-radius:16px;overflow:hidden;">
          ...
          <tr>
            <td align="center" style="padding:18px 28px 0 28px;">
              <div style="font:900 32px/1.1 -apple-system,Segoe UI,Roboto,Helvetica,Arial,sans-serif;color:#ffffff;letter-spacing:-0.01em;">${spots} more spots. <span style="color:#219EBC;">Then it's done.</span></div>
              <div style="font:600 14px/1.6 -apple-system,Segoe UI,Roboto,Helvetica,Arial,sans-serif;color:#d4dde6;margin-top:12px;">
                Both codes were claimed in under ${soldMinutes} minutes. Enough of you asked that I'm releasing <strong style="color:#8ECAE6;">${spots} more</strong> — same price, same code, and that's the last of them.
              </div>
            </td>
          </tr>
          ...
          <tr>
            <td align="center" style="padding:14px 34px 8px 34px;">
              <div style="font:400 12px/1.6 -apple-system,Segoe UI,Roboto,Helvetica,Arial,sans-serif;color:#6b7d8f;">
                ${deadline} Max ${spots} redemptions.<br>The first ${soldSpots} were gone in ${soldMinutes} minutes — when these are gone, they're gone.
              </div>
            </td>
          </tr>
