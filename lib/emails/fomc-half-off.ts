...
// inside a band.
//
// NO PROMO CODE ON PURPOSE. The discount is applied to the two checkouts by
// hand; a code chip here would send people hunting for a coupon that does not
// exist in Stripe. If a code is ever minted for this, put it back in the offer
// card the way edge3-annual.ts does.
//
...
export const FOMC_HALF_OFF_SUBJECT =
  "Half off the year — $250 instead of $500, two spots, until the Fed";
...
export function fomcHalfOffText(opts: FomcHalfOffOpts = {}): string {
  ...
  return [
    ...
    `${pct}% off the annual plan. ${spots} spots, closing at ${deadline}.`,
    "",
    "————————————————————————",
    "WHY NOW",
    "————————————————————————",
    ...
    ...([
      ...
      ["Discount · FOMC sale", `-$${off.toLocaleString("en-US")}.00`],
    ...
    `Get the year: ${cta}`,
    "",
    `Held for twelve months. When the ${spots} are gone it is back to $${listPrice.toLocaleString("en-US")}.`,
    "",
...
<body style="margin:0;padding:0;background:#05060A;">
  <!-- preheader (hidden) -->
  <div style="display:none;max-height:0;overflow:hidden;opacity:0;">A full year of CB Edge for $${price} instead of $${listPrice.toLocaleString("en-US")}. ${spots} spots, and they close at ${deadline}.</div>
          ...
          <tr>
            <td align="center" style="padding:22px 30px 0 30px;">
              <table role="presentation" cellpadding="0" cellspacing="0" border="0" style="border:1px solid rgba(33,158,188,0.45);border-radius:999px;background:rgba(33,158,188,0.12);">
                <tr>
                  <td style="padding:8px 18px;font:800 11px/1 ${SANS};letter-spacing:0.16em;text-transform:uppercase;color:#8ECAE6;">${pct}% off the annual plan &middot; ${spots} spots</td>
                </tr>
              </table>
              ...
              <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="margin-top:14px;border:1px solid rgba(33,158,188,0.30);border-radius:12px;background:linear-gradient(180deg,rgba(33,158,188,0.10),rgba(33,158,188,0.02));">
                ...
                <tr>
                  <td align="center" style="padding:20px 20px 16px 20px;">
                    <div style="font:700 10px/1 ${SANS};letter-spacing:0.16em;text-transform:uppercase;color:#8ECAE6;">Seats at this price</div>
                    <div style="padding-top:10px;">
                      <span style="display:inline-block;border:2px dashed rgba(33,158,188,0.7);border-radius:10px;padding:10px 24px;background:rgba(33,158,188,0.10);font:900 24px/1 ${SANS};color:#219EBC;letter-spacing:0.10em;">${spots} LEFT</span>
                    </div>
                    <div style="font:400 12px/1.7 ${SANS};color:#6b7d8f;padding-top:10px;">Closes at ${deadline}, or when they are taken.</div>
                  </td>
                </tr>
                ...
                <tr>
                  <td style="padding:16px 20px 10px 20px;">
                    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0">
                      ...
                      <tr>
                        <td style="padding-top:10px;font:400 13px/1.5 ${SANS};color:#9fb3c8;">Discount &middot; FOMC sale</td>
          ...
          <tr>
            <td align="center" style="padding:20px 30px 0 30px;">
              ...
              <div style="font:400 12px/1.7 ${SANS};color:#6b7d8f;padding-top:12px;">
                Held for <span style="color:#8ECAE6;font-weight:700;">twelve months</span>. When the ${spots} are gone it is back to $${listPrice.toLocaleString("en-US")}.
              </div>
            </td>
          </tr>
