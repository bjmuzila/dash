 ...
 *
 * Click any country or city bubble for a pinned detail card (top pages, recent
...
export default function Visitors() {
  ...
  // reaching into the map's internal aggregate.
  const { countries, cities, geoCoded } = useMemo(() => {
    ...
    for (const v of visits) {
      ...
      if (typeof v.lat === "number" && typeof v.lon === "number") {
        geo++;
        p.add(`${v.lat.toFixed(2)},${v.lon.toFixed(2)}`);
      }
    }
    return { countries: c.size, cities: p.size, geoCoded: geo };
  }, [visits]);
    ...
    <div style={homeShellStyle}>
      ...
      <div style={homeHeaderStyle}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
          ...
          <Stat label="cities" value={cities.toLocaleString()} />
          <button
            onClick={load}
            disabled={loading}
      ...
      <div style={{ flex: 1, minHeight: 0, overflowY: "auto", padding: "clamp(14px,2vw,22px)", display: "flex", flexDirection: "column", gap: 14 }}>
        ...
        <div style={{ fontSize: 14, color: T.textSecondary, opacity: 0.55, lineHeight: 1.6 }}>
          Click a country or a city bubble to pin its detail card. Scroll to zoom, drag to pan,
          double-click to zoom in. City positions are Cloudflare metro centroids from the visitor's
