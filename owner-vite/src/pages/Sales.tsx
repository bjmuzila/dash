...
export default function Sales() {
  ...
  // Entry rows from the visit log, for the Campaign Link Builder's "already
  // used" chips. Fetched ONCE per page load (not on ↻): the builder only reads
  // utm source/campaign names off it, and those change by the week, not the
  // minute. Best-effort — with no rows the builder simply offers no chips.
  const [campaignRows, setCampaignRows] = useState<CampaignSeedRow[]>([]);
  useEffect(() => {
    let alive = true;
    fetch("/api/page-visits?days=90&limit=20000", { cache: "no-store" })
      .then((r) => (r.ok ? r.json() : null))
      .then((j) => {
        if (!alive || !j?.visits) return;
        const rows = (j.visits as CampaignSeedRow[]).filter((v) => v.isEntry);
        setCampaignRows(rows.map((v) => ({ utmSource: v.utmSource, utmCampaign: v.utmCampaign, isEntry: true })));
      })
      .catch(() => { /* chips are a convenience */ });
    return () => { alive = false; };
  }, []);
