...
## 2026-07-31 — Owner Results → Confidence: CB contract pricing + auto-buy/auto-sell replay wired end to end (`server-v2/cb-contract-track.js` NEW, `server-v2/cb-contract-track.selftest.js` NEW, `server-v2/proxy-thetadata.js`, `server-v2/api-router.js`, `owner-vite/src/pages/Results.tsx`)

Finished the contract-pricing extension the Confidence tab had UI for but no data behind. New `server-v2/cb-contract-track.js` answers the checkpoint board in dollars: for each session it replays, off ThetaData's 1-minute tape, what the CB-strike 0DTE contract cost at 9:45 / 10:30 / 12:00 ET, auto-buys when that print was ≤ `$1.00` (`CB_AUTO_BUY_MAX`), and auto-sells the first time SPX traded within the 5–10 pt band of the CB (`CB_SELL_TRIGGER_PTS`, fired on the OUTER edge — price entering the band crosses 10 before 5, and a gap straight to 3 pts is still "within 10"; the actual distance is reported as `sellSignal.distPts` and flagged `tight` under 5). Side is the leg that gains as SPX travels TO the CB — SPX under it buys the CB call, over it the CB put — which is why the sub-$1.00 filter selects real magnet setups instead of ATM premium. Untriggered contracts settle at the last print (0DTE, usually a total loss); a live session marks to the last bar and sets `open` so the UI can show it as unrealized.

Deliberately a REPLAY, not a poller. The obvious build — a recorder that wakes at each checkpoint, snapshots a quote, then polls for the sell — has a hole this doesn't: any minute the process is down is a trade that silently never existed and can never be backfilled. Replaying from the tape on read is deterministic, backfills the whole history on first render, survives a redeploy mid-session, and needs no new table and no `lib/db.ts` rebundle. `proxy-thetadata.js` gained `fetchIndexIntradayTheta` (`/v3/index/history/ohlc`, flat `rowsFromV3` rows, ET wall-clock timestamps pinned via `_parseEtWallClock` — a UTC host would otherwise shift every bar; note index history wants root `SPX`, never the `SPXW` options root) so the "within N pts" trigger is evaluated on real 1m price action rather than the coarse `mvc_snapshots` cadence, with those snapshots kept as the fallback path when the index tier returns nothing.

`/api/confidence/checkpoints` now merges the per-cell fields (`right`, `contractPrice`, `autoEntry`, `sellSignal`, `sold`, `pnl`, `open`, `contractNote`) and per-checkpoint rollups (`contractTrades`, `sellHits`, `contractWins`, `contractWinRate`, `avgPnl`, `totalPnl`) plus a `contracts` meta block; `computeCheckpointData` also returns an internal `spxByDate` the handler strips before sending. Cost is one index call plus one option call per DISTINCT strike+side per day — the CB usually holds across checkpoints, so ~2–3 calls a session — and completed sessions cache for the life of the process (today's on `CB_TRACK_TTL_MS`), which is what keeps the tab's 60s poll off theta-terminal. Opt out with `?contracts=0`; capped at `CB_TRACK_MAX_DAYS` (40) newest-first so `?all=1` can't turn into 365 sessions of chain calls. Every failure path degrades: no Theta, no index tier, a strike outside the returned window, or the module absent entirely all leave the hit-rate half of the board rendering exactly as before, with a `contractNote` explaining the empty cell.

`Results.tsx` gained a CONTRACTS on/off toggle (sends `?contracts=0`), renders the three per-checkpoint columns only when the server actually returned tracking (a Theta outage collapses the table back to the plain board instead of showing nine em-dashes), shows C/P beside each price, clock-stamped entry/sell tooltips, `⏳`/`*` for still-open marks, a status legend, and Win Rate + Total P&L in the summary cards — which now render at zero trades too, so "nothing qualified" is distinguishable from "the tracker never ran". Also fixed a latent `C.muted` reference on that card (`C` has no `muted` key; now `MUTED`). New `server-v2/cb-contract-track.selftest.js` (`node server-v2/cb-contract-track.selftest.js`, no DB/network) covers the rules and the wiring — 28 checks, all passing: side selection, the $1.00 filter, sell-scan excluding the entry bar (which would otherwise buy and sell the same print for a flat 0.00 whenever a checkpoint fired with SPX already inside the band), signal-without-fill, settle vs mark-to-market, one-call-per-distinct-contract, 0DTE expiry on every request, rollup math, cache hits, and the Theta-outage degradation. `tsc --noEmit` clean on `Results.tsx`, `node --check` clean on all three server files. Not prod-build verified; needs deploy.
