...
## Friday 7/31/2026 — ES Candles: one shared /ws/gex socket, GEX-history backfill fix, payload diet, overlay-panel rebuild (`lib/gexSocket.ts` NEW, `lib/dedupeFetch.ts` NEW, `server-v2/_lib-db.cjs`, `server-v2/api-router.js`, `lib/snapdb.ts`, `app/es-candles/page.tsx`, `components/shared/DockToolbar.tsx`, `components/shared/TopBar.tsx`, `components/shared/ToolbarTicker.tsx`, `components/dashboard/{HomeGaugeRail,LiveGreeksGauges,EsCandlesFullPanel}.tsx`, `hooks/useEsCandles.ts`, `app-vite/index.html`, `app-vite/vite.config.ts`, `public/favicon.png` NEW, `public/apple-touch-icon.png` NEW)

Started from a network waterfall on `/app/es-candles` showing THREE `/ws/gex` WebSockets on one page load. New `lib/gexSocket.ts` owns a single refcounted connection (500ms close grace so StrictMode double-mounts and route changes don't drop-and-reopen; exponential backoff capped at 30s, replacing the flat 2–2.5s retry each consumer ran independently). Frames are `JSON.parse`d ONCE and handed to every subscriber. Late subscribers get the last frame of each *state* type replayed synchronously — mandatory, not a nicety: each per-consumer socket used to receive its own `snapshot` on connect, so a lazily-mounted route sharing an already-open socket would otherwise sit empty until the next server publish; one-shot notices (`regime-fit-updated`) are deliberately NOT cached. Six call sites converted (`TopBar`, `ToolbarTicker`, `useEsCandles`, `app/es-candles/page.tsx`, `HomeGaugeRail`, `LiveGreeksGauges`, `EsCandlesFullPanel`); `useWsLifecycle` is untouched and now decides whether a consumer *subscribes* rather than whether it *connects*.

**Root-caused the missing historical bubbles/heatmap: `api-router.js:4293` calls `libDb.normGexSymbol()`, which does not exist in `_lib-db.cjs`.** Every other libDb symbol that route uses resolves; that one is the only zero. It threw `TypeError: libDb.normGexSymbol is not a function` on the FIRST line of the handler, in every mode, and the route's `catch` turned it into `200 {error, rows:[]}` — so `res.ok` passed, `columns` was absent, and the client read it as "no data recorded yet". Live bubbles kept drawing (websocket path), historical never did. Hand-patched the esbuild bundle: added `normGexSymbol` (blank/`SPX`/absent → `$SPX`, byte-verified against `normSymbol` in `gex-history-writer.js` — they must agree or reads miss writes), registered it in the `__export` getter map AND `module.exports`, and threaded the `symbol` parameter through all eight `option_strike_gex_history` helpers, which the router was already calling with an arity they didn't accept (`getOptionStrikeGexSlotsWindowAny(sinceTs)` took 1 arg, called with 2) — so with SPY/QQQ writing to that table since 2026-07-27, `DISTINCT ON (minute, strike)` had been letting one symbol's row win per bucket across ALL symbols. `insertOptionStrikeGexRows` now writes the `symbol` column instead of falling to the `$SPX` default. Also fixed `getMvcSnapshots`' `sinceMs` branch selecting `strikeOIVol, spxPrice, esPrice` UNQUOTED against a table created with quoted camelCase — dead-on-arrival, never hit because nothing passes `?days=`. Added `console.error` to both catches on that route plus client-side warnings for `!res.ok` / `error` key / missing `columns`, since silence is what hid this.

Payload work, all opt-in so existing callers are byte-identical: `?top=N` on `mode=heatmap` returns only the N strongest strikes per column (the bubbles' own `cfg.topStrikes` ranking) **plus both strikes bracketing every sign change** — load-bearing, because the gamma flip is the net-GEX zero crossing and lives exactly where `|net|` is smallest, i.e. the first thing a top-N filter discards; 3000 randomized ladders gave 0/3000 flip mismatches with the guard vs 2982/3000 without, at 86% fewer bytes. Client gates it off when Heatmap / Flip X / Replay are on or on the ETFs (those read the ladder's shape, not its peaks) and carries it in `shapeKey`. `?lite=1` adds tuple/columnar encoding to `/api/snapshots/candles` and `/api/snapshots/mvc` (the latter also narrowing a `SELECT *` over ~22 columns to the four the CB line reads): measured 59% and 93% smaller on realistic row shapes. `lib/snapdb.ts` gained `_expandCandles` with a legacy-shape fallback so the client can deploy ahead of the backend. Separately cut `historyDays` 20 → 2 on `app/es-candles/page.tsx` and `EsCandlesFullPanel` (chart window `FIVE_DAYS_MS` → `TWO_DAYS_MS`, matching the 48h `option_strike_gex_history` retention and the `heatmapDays: 1|2` cap); the hook DEFAULT stays 20 so RelVol / IB Logic keep their avg5/avg14 baselines. Removed VSA entirely from the ES-candles page at the user's request (import, palette constants, `showVsa`/`vsaTuning` state, `vsaMap` memo, the per-bar recolor branch, the Overlays entry and the 5-slider tuning panel) — it was the only consumer on that page that wanted deep history, so the 2-day pull is now unambiguous. `lib/vsa.ts` left in place, unimported.

Overlay panel rebuilt: 2-column chip grid, `PanelSection` (small-caps label + hairline) replacing full-height headers, `PanelChip` deduplicating markup the checkboxes and the new CB toggle now share, Brightness folded into Bubble size, footer status text → dot so Save default / Reset split the width. `DockSlider` gained ▲/▼ steppers (one `step` per click, hold-to-repeat after 350ms, values snapped to the step's decimal precision since repeated float addition drifts and a non-multiple gets rounded back by the native input) plus `labelWidth` / `width="auto"` — without a shared label column each row sized to its own label and the steppers staggered. The outer wrapper moved from `<label>` to `<span>`: a `<button>` inside a label forwards its click to the labelled control and would yank focus onto the range input on every step. CB (MVC) step line moved out of Levels into the Bubbles group as a real toggle persisted in the bubble blob — it was a hardcoded `showMvcLine = true` with no control anywhere. Chart fixes: symbol switch now clears `didFitRef`/`lastFitDayRef`, re-enables the right scale's `autoScale` (lightweight-charts silently disables it on any price-axis drag and never restores it, so ES→SPY left the axis parked at 7500) and calls `applyDefaultView` directly, because effects flush in declaration order and the candle-data effect runs first with the new bars but the flag still true; spot/last-price line given an explicit neutral `priceLineColor` instead of inheriting the candle's green/red. Weekly EM bands added to the PDH/ON overlay (relabelled `PDH/ON+EM`) from `GET /api/levels?ticker=` — note that endpoint answers 200 with a literal `null` body when unpublished, and stores `up`/`down` as `toLocaleString`-formatted strings, so `Number("7,650.25")` is NaN and a naive parse would look fine on SPY/QQQ (<1000, no separator) while silently dropping the bands on ES only; `parseLevelNum` strips separators. Overlays menu hardened for small screens: explicit inline width + `boxSizing:border-box` (it relied on Tailwind `w-56`, which sets a CONTENT width and, if the class ever fails to generate in the Vite build, leaves the fixed-position panel with no width at all), viewport-clamped `left` re-run on resize, `maxHeight` + scroll, `minmax(0,1fr)` grid tracks. **The small-screen overflow was NOT reproduced** — a headless Chromium harness measured 0px overflow at 1920/1600/1440/1366/1280/1024/380 across both old and new configurations, so the hardening is reasoned rather than confirmed; the computed width of that panel on the affected laptop is the outstanding diagnostic. Daily EM was requested and NOT built: `/api/estimated-move` is a 501 stub, `/api/em/ticker-em-stats` returns averages of past weekly EMs, the only real daily EM (`/api/social-media/daily-input`) is `auth:'owner'` and SPX-only, and `levels-engine.js`'s `getTargetExpiration` filters `d >= 1 && d <= 10`, structurally excluding 0DTE. esbuild/`node --check` clean on all files; NOT prod-build verified and NOT deployed.
