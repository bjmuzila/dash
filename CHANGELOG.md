...
**The chart carries dots, not boxes.** Every play drops a single **entry dot** at its entry price on its trigger bar: green ▲ for a long, red ▼ for a short (direction, not outcome), solid with a halo when live, hollow + dashed ring when only armed, faded with ✓/✕ once resolved. **Click a dot** and that one play opens its **TradingView-style Long/Short Position box** — green profit zone entry→3R, red stop zone entry→structural stop, the entry line between them, dashed 1R/2R rungs that tick ✓ as they're tagged, a header chip reading `TURTLE SOUP · SHORT · 3.0R:R  LIVE` — plus a popup card with the same numbers in text. Click it again (or anywhere empty) to close. Drawing all six boxes at once was tried first and was unreadable; one box at a time is the whole point of the dot. New `▶ Plays` toolbar toggle (ON by default) and a `▶ Plays` card under the chart whose rows open the same popup, so the play is reachable without hunting for its dot.

**The risk math is deliberately a copy of the recorder's** (`app/api/ict-setups/route.ts` → `extractSetups`): entry = trigger-bar close, stop = the nearest swing pivot on the wrong side of entry gated on `confirmTs` (never `ts` — that was the lookahead that inflated every R on `/dev/results`), buffered by `0.15 × ATR(14)` with a ≥1-buffer floor so a degenerate stop can't divide by ~0. Targets are the 1R/2R/3R ladder off that risk, so a box tagging 2R on the chart is the same 2R the leaderboard counts. The header comment in both files says to change them together; if they drift, the boxes stop matching the win rates.

Four states, because the ask was "mark it when the trade becomes live **or possible**":
- **ARMED** (hollow dot, dashed box) — the setup exists but hasn't triggered: an EQH/EQL pool pierced by the live bar that hasn't closed back inside yet (the bar *before* a Turtle Soup confirms), a valid unmitigated OB or a post-BOS breaker zone with price walking into it, or price within reach of the OTE band.
- **LIVE** (solid dot + halo) — the trigger bar printed; box extends 8 bars past the live bar.
- **WON / STOPPED** — graded by walking the bars after entry the same way the recorder does (peak MFE in R; stop-and-target on the same bar resolves as the stop and banks no new rung, since there's no intrabar path). Ends at the resolving bar and stays up ~6 bars, then drops.

Scope is the **model plays only** — Turtle Soup, Judas, CISD, 2022 Model, Breaker, Inducement, valid-OB retest, OTE entry. BOS/CHOCH/displacement/every raw FVG fire far too often to mark; they stay as zone overlays. Cap of 4 plays, filled live → armed → at most 1 resolved, so a chatty detector (inducement fires ~50×/day on 5m) can't crowd out the live markup.

Three layout problems found by rendering it rather than by reading it: co-located dots (Turtle Soup → 2022 Model fire on the same bar at the same price by construction) drew on top of each other and only the top one was clickable — collisions now nudge **sideways**, never vertically, since y is the entry price; a play triggering on the live bar had no room to its right and collapsed into a sliver against the price scale, so a box under 132px now hangs back to the LEFT of its entry instead (the dot still marks the true bar; the box only has to carry the levels); and box headers auto-stagger down a row when they'd collide, with price tags flipping outside-left and dropping entirely when a zone is thinner than its text. Panning the chart ends in a click event, so the dot hit-test is gated on the pointer having moved <5px between press and release — otherwise every pan closed the popup.
