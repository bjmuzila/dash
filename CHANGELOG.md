...
## Sunday 9/13/2026 — Level Log ranges + snapshot polish, and a new Open bracket study (`cbedge-v3/src/pages/LevelLog.tsx`, `server-v2/core-hold.js`, `owner-vite/src/pages/Results.tsx`)

`/v3/level-log` gained **Monthly** and **All time** on the range switch (`LevelLog.tsx`, `levelLog/wallData.ts` — one `/api/walls-range` request each, the per-session fallback capped at `LEGACY_FALLBACK_MAX = 5`) with thinned date stamps in `levelLog/WallMigrationChart.tsx`, and its snapshots dropped the duplicated variant caption (`data-capture-hide`, which now returns its height via `trimHeight()` in `shell/snapshot.ts`) and gained a ticker logo in the caption band (new `badge` on `ShotOptions`/`CopyShotTarget`, `tickerLogoUrls()` exported from `pages/economicCalendar/ChipLogo.tsx`).

New owner study `/api/core-hold` + **Open bracket** tab on `/dev/results` (`server-v2/core-hold.js`, `server-v2/api-router.js`, `owner-vite/src/pages/Results.tsx`): the 09:29 / 09:35 / 09:45 / 10:00 put-wall→call-wall range frozen at the anchor, scored on closed-inside / never-left / median width (the control) and where the CORE actually sat — it is usually one of the two walls, so the above/below split runs only over interior-CORE sessions. Finally, `server-v2/state/retention-cleanup.js` exempts the fourteen `scanner-tickers.js` MAIN symbols from the `scanner_snapshots` prune (`RETENTION.scanner_keep_symbols`), so the level log's price line and the study's intraday arms keep accruing past 10 days for the names that matter.
...
**Twice this session `server-v2/api-router.js` was reverted by an edit made from a stale copy of the file, silently removing the whole server half while the 📎 button stayed on screen — if screenshots ever stop saving, grep that file for `customer_feedback_shots` first.**

## Thursday 9/10/2026 — Notes (v3): clip images readable at a glance, and a full-screen pop-out (`cbedge-v3/src/shell/notes.tsx`)

A clip note's image was capped at 120px and `object-cover` cropped, which in the 320px dock made two GEX-chart clips taken minutes apart indistinguishable; the resting cap is now 260px (click-to-grow to full height is unchanged) and a new expand button in the image's top-right opens `ClipLightbox`, a new component in the same file that portals to `document.body` at `z-[9999]` and covers the viewport — NOT the Card `ExpandStageHost`, whose `absolute inset-0` stage lives inside the page column and would still be inside the dock's `overflow-hidden` box, i.e. the exact problem. Escape / backdrop / ✕ close it, clicking the image toggles fit ↔ actual size with scroll (a wide-monitor capture is still unreadable shrunk to a laptop screen), body scroll is locked while it is up and restored on unmount, and a `useEffect` clears `lightboxId` if the note behind it is deleted so a popped-out clip cannot leave a dangling overlay. All token utilities (`border-line`, `bg-surface2`, `bg-bg/90`, `text-accent`, `text-faint`) — no hex. **NOT build-verified** — run `npx tsc --noEmit` + `npm run build` before push. Not deployed.
...
## Thursday 9/3/2026 — v3 board: Gauge Rail card, v2's home strip minus IB Direction (`cbedge-v3/src/board/gaugeRail/GaugeRailCard.tsx`, `cbedge-v3/src/board/catalog.tsx`)

v2's home-page segmented-LED strip, added to the v3 board catalog as `gauge-rail` (w12/h5, lazy). Five tiles: Gamma (Net GEX), Delta (DEX), Gamma % 0DTE (Vol), Net GEX Rate / min, 0DTE GEX Δ 15m — each a tick meter with the value under it and a 15-minute change line under that. IB Direction is deliberately dropped (Brandon's call): it was also the only tile on the rail that came from outside the option book, so dropping it makes this a single-source card. **Where the numbers come from is the one real departure from v2:** v2 reads the socket's `totals` blob, which v3's wire contract does not describe (`GexData.totals` is `unknown`), so the card sums the ROWS through the same accessors the GEX Chart card uses — `netGexOf`/`dexOf` on the OI+VOL basis — which is the identical definition `server-v2/greeks-ts-writer.js` records with, so the rail cannot disagree with the GEX Chart's Net GEX tile beside it. Seeded from `/api/snapshots/greeks` (today, $B, one request, no poll): without it the rate and Δ15m tiles would read `--` for the first fifteen minutes after the card is added. The per-tile 15-minute change LINE is still a one-minute ring buffer and still draws nothing until it reaches back a full fifteen minutes — never `value − prevValue`. The snapshot selector deliberately ignores the frame's `updatedAt` when testing for change, so the card re-renders on movement rather than on every socket push. Full `npm run check` green: typecheck, theme (0 new violations), build, budgets (3.2kb brotli), ws-scope, perf and all seven parity self-tests.

## Thursday 9/3/2026 — GEX Candles: bubbles land on their candles, a 1m size ladder, a gold leader, and a session-width frame (`cbedge-v3/src/board/gexCandles/{chart.ts,bubbles.ts,settings.ts}`, `cbedge-v3/src/design/tokens.css`)

Four fixes to the v3 GEX Candles card. **Alignment:** `drawBubbles` was finding a bucket's x by binary-searching `timeAtX` (`coordinateToTime`), which is a STEP function — the search converges on the step, i.e. the midpoint between two bar centres — so every mark sat half a bar off its candle; `chart.ts` now keeps `barTimes` and answers directly through `geo.xOfTime()` (containing bar's centre + sub-bar fraction × `barSpacing`), returning null off the ends instead of clamping, which also stopped bubbles floating in the whitespace past the newest candle. `timeWindow()` / `xAtTime()` deleted. **1m size ladder:** at a session zoom the horizontal spacing bound (~3.4px) was spent on both axes by a circle and all four rows landed on `minPx`, so `SizeProfile` gained a separate vertical budget (`capYPx`/`floorYPx`/`topCapYPx`, bounded by the rung's own `aspect`, drawn with `ctx.ellipse`) and the size position became `t = (1-rankMix)·ratio**sizeCurve + rankMix·rank`; both live on the RUNG PROFILE with `aspect: 1, rankMix: 0` everywhere but 1m, so 5m and coarser reduce to the previous expression exactly. **Leader colour:** the bucket's biggest wall now fills with a gold radial gradient (`--color-gex-lead` `#ffb300` / `--color-gex-lead-hi` `#ffd76a`, white highlight) built in the mark's own space so it stays concentric with an oblong mark, with the SIGN colour as its ring and glow; `--color-gex-pos-hot` / `-neg-hot` and `BUBBLES.topTint` deleted, peers unchanged (gold on every mark was mocked and rejected — a 2-4px fill plus a sub-pixel ring is an olive smudge). **Framing:** `frameRecent()` measured a 390-minute window backward from the newest bar, which on a morning is mostly yesterday with today crushed into the last inch; it now frames today's session, `from = min(sessionStartIdx, newestIdx - span/2)` so 09:30 pins the left edge and 16:00 the right once the day fills, and the live candle is centred before that. `ensureLatestVisible()` also re-frames on a too-wide or mostly-empty pane (the old "is the newest bar on screen" test passed on the broken view) and checks a third time at 600ms for a card that mounts in a hidden tab. Mockup sheets in `generated/2026-09-03-gex-bubble-{palettes,gold-variants}.png` and `generated/2026-09-03-gex-bubbles-variant-e.png`.

## Thursday 9/3/2026 — Phone build: v3 toolbar on /m/*, Chain tab removed, and three cards stripped to a phone read (`cbedge-v3/src/shell/Shell.tsx`, `cbedge-v3/src/mobile/{mobileNav.ts,MobileShell.tsx,pages/M{Gex,Heat,Spx,Chain}.tsx}`, `cbedge-v3/src/App.tsx`, `cbedge-v3/src/board/{gexChart/GexChartCard.tsx,multiGreek/MultiGreekCard.tsx,gexCandles/GexCandlesCard.tsx}`)

`/m/*` now draws the SAME `Toolbar` the desktop does (only the 64px rail comes off), with a new `mobile` flag that hides just the SPX chip and TickerPicker — nothing on a phone screen follows the board symbol any more, and a picker that moves a value no visible card reads is the control-that-lies this toolbar was rebuilt to stop being; `MobileShell` therefore dropped its own duplicate symbol row. Three new card props carry the rest: `GexChartCard({ simple })` pins SPX and shows OI+VOL / VOL only (no FLOW, no NET/C-P split, no DEX, no stat row), `MultiGreekCard({ pinnedFirst: 'SPX' })` makes panel one fixed read-only text so ＋ is the only way symbols come in, and `GexCandlesCard({ spxOnly })` pins SPX — making the SPX/ES switch the only symbol control — and derives the session from the tape (ES → ETH, SPX → RTH) instead of offering a button that could only ever be wrong; all three leave the stored desktop settings untouched, the way `railOn` already did. The Options Chain tab is out of `MOBILE_TABS`, `App.tsx` and both redirect maps — a dozen numeric columns read ACROSS is a horizontal scroll at 390px, not a page — and `MChain.tsx` is a compiling tombstone pending `git rm` (the device shell was unavailable). NOT YET BUILT — run `npm run check` in `cbedge-v3/`.

## Thursday 9/3/2026 — cbedge.net opens the v3 phone build on a phone (`app/page.tsx`)

`RootPage` now reads the user agent and sends a signed-IN visitor on a phone to `/v3/m/gex` instead of `/traders-dashboard`; signed-out phone visitors still get `LandingClient`, because that page is the sales page. It is a UA test (`Android.*Mobile` is Google's documented phone test and iPadOS reports as Macintosh, so tablets and laptops are excluded) rather than a width test because the decision is made server-side before any JS runs — the app's own `useIsPhone()` stays width + pointer — and it goes straight to `/v3/m/gex` rather than `/v3` so the desktop board never mounts for a frame first.

## Thursday 9/3/2026 — v3 Economic Calendar + Earnings page at `/v3/economic-calendar`, spec-first (`cbedge-v3/docs/parity/economic-calendar.md`, `cbedge-v3/src/pages/EconomicCalendar.tsx`, `cbedge-v3/src/pages/economicCalendar/{ChipLogo.tsx,board.ts}`, `cbedge-v3/src/{App.tsx,shell/Shell.tsx,data/econCalendar.ts,board/econCalendar/econTemplate.ts}`, `cbedge-v3/scripts/parity-check-econ{.mjs,.test.mjs}`, `cbedge-v3/package.json`, `app/v3/economic-calendar/route.ts`)

Ported v2's `/app/economic-calendar` (`components/pages/EconomicCalendar.tsx`, 1,303 lines) to `/v3/economic-calendar` spec-first: `docs/parity/economic-calendar.md` transcribes **176 checklist rows** — every rendered value with its endpoint and field, format, threshold, sort order and empty state — BEFORE any v3 file was opened, and the page was then built against it as two tabs over one feed (timed econ events with earnings woven into the day; a five-column earnings week board with week and Anticipated/All toggles). New `pages/economicCalendar/ChipLogo.tsx` (the three-stage logo ladder, `raw=1` kept on the proxy stage so a 302 cannot taint a capture canvas — **bump `LOGO_REV` in step with v2's**) and `board.ts` (the nine `BOARD` washes as `alpha()` over tokens, the chip geometry, `dayFull`/`dayDate`); `data/econCalendar.ts` gained only `fullDayLabelLong()` — the rest of that module was already a verbatim transcription and every helper is imported, nothing re-declared. Three decisions taken on the inventory (Brandon): type sizes collapse DOWN (12→11, 14→13, so the size order survives — v2 uses two sizes that are off v3's scale), all ten shared filters ship rather than v2's drifted eight, and the long day label (`MONDAY SEPTEMBER 1`) added beside the short form rather than replacing it. Four v2 defects fixed rather than transcribed, each recorded in Part Q: the date chip gated on `lastRefresh` but rendering `today`, `source` fetched and never read (the signal that would have caught the six-week silent feed outage), one search string shared across both tabs, and no `?tab=` in the URL. Verified by `scripts/parity-check-econ.mjs` — drives both pages through three scenarios against one backend and fails on anything v2 renders and v3 does not (25 calendar probes, 31 earnings probes; the two load-bearing set probes are every ticker each side LINKS to and every market cap each side puts in a TOOLTIP, since cap and EPS are on screen nowhere else) — plus `parity-check-econ.test.mjs`, 20 fixture assertions all green with seven injected regressions failing as intended; both wired into `npm run check`. Separately unblocked the pre-commit theme gate: six `font-size` literals in `board/econCalendar/econTemplate.ts` (the 1280×672 Discord poster) became five named constants, matching the twelve computed sizes already in that file, with the **rendered stylesheet verified byte-identical** — the poster does not change by one pixel, and the baseline was not raised. NOT YET BUILT — run `npm run check` in `cbedge-v3/` before pushing, and run the parity check once as owner and once as a plain subscriber (the feed-health and error banners are owner-only on both pages).

## Thursday 9/3/2026 — v3 phone build at `/v3/m/*`, `/v3` un-gated, and a "New version available" toast (`cbedge-v3/src/mobile/{mobileNav.ts,MobileShell.tsx,MobileTabBar.tsx,MobileRedirect.tsx,pages/M{Gex,Heat,Spx,Chain,Em,Econ}.tsx}`, `cbedge-v3/src/{App.tsx,shell/{Shell.tsx,UpdateToast.tsx},data/appVersion.ts,board/multiGreek/MultiGreekCard.tsx,board/gexCandles/GexCandlesCard.tsx}`, `app/v3/m/{route.ts,[tab]/route.ts}`, `middleware.ts`, `cbedge-v3/AGENTS.md`)

Added a six-tab phone build at `/v3/m/*` where every screen IS a home-board card or a v3 page rendered full-bleed inside `MobileShell` — no phone-only reimplementation of anything — with Heat mounting `MultiGreekCard` under a new `singleColumn` prop (front/0DTE column only, ＋ and its 1–4 ticker panels untouched), `GexCandlesCard`'s SPX/ES tape switch pulled out of the phone settings sheet into the header at every width, `Shell.tsx` branching to drop the rail and toolbar on `/m/*` while keeping all three providers, and `middleware.ts` losing `/^\/v3(\/.*)?$/` from `OWNER_PATTERNS` so `/v3` is now a normal paid route. Also added an update check (`data/appVersion.ts` compares the running entry chunk's content-hashed filename against the one `/v3` serves under `cache: 'no-store'`, polling 30s after mount, every 10 min while visible and on `visibilitychange`) behind a dismissible bottom toast (`shell/UpdateToast.tsx`), so a backgrounded phone tab stops silently running last week's build. NOT YET BUILT — run `npm run check` in `cbedge-v3/` before pushing.

## Wednesday 9/2/2026 — v3 Options Chain + Multi Greek: CB core keeps its sign, 50% strikes default, settings menu click fix (`cbedge-v3/src/pages/optionsChain/{heatSkins.ts,ChainMatrix.tsx,pickers.tsx,useChainData.ts}`, `cbedge-v3/src/board/multiGreek/MultiGreekCard.tsx`, `cbedge-v3/src/design/primitives/Controls.tsx`)

Replaced the flat gold CB fill with a directional wash (`levelFillBg()` cb branch, 112deg 26%/66% on the chain; `CB_WASH` 55%/82% on the Multi Greek ladder) so a negative core reads red instead of gold, dropped the ★ halo on both, and made the ladder's CB / CW / PW chip gate only the badges and ★ rather than the gold itself. Also raised the chain's non-SPX strike window from 30% to 50% (`useChainData.ts`) and fixed the % strikes menu swallowing clicks — `Popover` now ignores pointerdowns under `[data-popover-safe]` (new `POPOVER_SAFE_ATTR`) and the dropdown rows act on `onPointerDown`.

## Wednesday 9/2/2026 — v3 GEX Candles: ES futures back as an SPX/ES switch, owner background polling, timeframe reframe fix, zero-candle fix (`cbedge-v3/src/board/gexCandles/{GexCandlesCard.tsx,basis.ts,candles.ts,chart.ts,settings.ts,symbols.ts}`, `cbedge-v3/src/data/{api.ts,socket.ts}`)

Added an SPX/ES tape switch to the v3 GEX Candles card (SPX only; `settings.esCandles`, blob v7) — ES bars off `/api/snapshots/candles?lite=1`, the forming bar off the socket's `esCandles`/`es1mCandles` frame, every strike shifted by the ES−SPX basis from `/proxy/es-spx-basis` (new `basis.ts`) with v2's live esClose−spot fallback during cash hours and a status line when neither is usable. Also: `useQuery` gained `background` so the owner's candle and GEX polls keep running in a hidden tab; `chart.ts` re-checks the visible range after a reframe so the newest candle survives a timeframe change; `candles.ts` sanitises half-built bars (low/open 0) that drew one candle to zero; `socket.ts` fans the ES tapes out of the connect snapshot. Full detail in `md files/CHANGELOG.md`.

## Wednesday 9/2/2026 — v3 Replay: new /v3/replay hub, four tabs, and a bottom-docked transport for every replay surface (`cbedge-v3/src/pages/{Replay.tsx,OptionsChain.tsx}`, `cbedge-v3/src/pages/replay/{mgReplay.ts,MultiGreekReplay.tsx}`, `cbedge-v3/src/pages/optionsChain/{LadderModal.tsx,ReplayBar.tsx}`, `cbedge-v3/src/pages/analysis/lookup/{TickerLookup.tsx,Ladder.tsx}`, `cbedge-v3/src/design/primitives/ReplayDock.tsx`, `cbedge-v3/src/{App.tsx,shell/{Shell.tsx,notes.tsx}}`, `app/v3/replay/route.ts`, `cbedge-v3/scripts/parity-check-replay{.mjs,.test.mjs}`, `cbedge-v3/docs/parity/replay.md`, `cbedge-v3/package.json`)

**v3 only — nothing in this entry touches the live v2 app at `/app/*`.**

Ported v2's `/app/replay` to `/v3/replay` spec-first: `docs/parity/replay.md` transcribes 187 checklist rows from v2 BEFORE any v3 file was opened, three of the four tabs turned out to be mounts of components v3 already shipped (`LadderModal` gained an `embedded` shape, `TickerLookup` already took `embedded`/`initialReplay`, `OptionsChain` needed `initialReplay`/`initialReplayScope` threaded into `useChainData`), and only Multi Greek was a build — new `mgReplay.ts` (positional-cell parse, minute-bucket timeline across four sessions, step-hold frame pick, session strike axis) and `MultiGreekReplay.tsx`, with every wall, rank and ramp imported unchanged from `board/multiGreek/mgMath.ts`. Five decisions were taken deliberately rather than transcribed: the Δ double-sign bug (`++1.2B`) FIXED in `analysis/lookup/Ladder.tsx`, the unreachable "no recorded sweeps" empty state FIXED, v3's spot-guarded CW/PW and `MAX_EXP_COLS = 3` KEPT over v2's (so `/v3/replay` will deliberately disagree with `/app/replay` on two numbers), and four independent panel symbols kept as v2 has them.

Then, on seeing it built: every replay transport now docks to the bottom of the page via a new `ReplayDock` primitive wrapped around the page in `Shell.tsx` — transcribed from v2's ES Candles transport including the load-bearing part, that it is the last flex child of the page column and NOT `position: fixed`, so it shrinks the page instead of covering the strikes nearest the money. The dock is the orange plate now and all three bars dropped their own; the ⛶ Ladder modal keeps its transport inline, since a modal owns its own bottom edge. Also added `scripts/parity-check-replay.mjs` (119 probes across five surfaces — it clicks each tab, because only one is in the DOM at a time and a single harvest would score zero for three quarters of the checklist and pass) plus its self-test, and fixed a pre-existing `noUncheckedIndexedAccess` error in `shell/notes.tsx` that was blocking the gate. `npm run check` green.

## Monday 8/31/2026 - Phone SPX Candles: charted off SPX cash, bubbles on the desktop model, and the socket resilience behind it (`components/mobile/pages/MobileEsCandles.tsx`, `components/mobile/mobileNav.ts`, `hooks/{useGexBubbleHistory,useEsCandles}.ts`, `lib/{gexBubbleModel,gexSocket}.ts`, `cbedge-v3/src/design/{useIsPhone.ts,primitives/{Controls,Board}.tsx}`, `cbedge-v3/src/board/gexCandles/GexCandlesCard.tsx`)

`/app/m/es` now charts SPX cash via `useEtfCandles` instead of ES through the ES/SPX basis - which deletes `basisOk`, the gate that blanked every overlay off-hours - with a live tip on the newest bar off the socket's `spot` frame to cover the recorder's 60s write cadence, a two-day GEX bubble reach (`days` + a bar-day-derived minutes window in `useGexBubbleHistory`, `top` 8 -> 16), and the bubble layer rebuilt on the desktop's own rules through a new `lib/gexBubbleModel.ts` that imports `BUBBLES` from `es-candles/slotStore` rather than copying it (4 strikes a bucket, one forced each side of spot, `ratio**sizeCurve` sizing bounded by real bar spacing, boosted/ringed leader, fit-then-jitter). Also fixed a `levelLines`/`levels` ReferenceError that black-screened the route on the live build, anchored `sessionCandles`' 30h window to the newest bar held (it slid off the tape every weekend), hardened `lib/gexSocket.ts` against a zombie CLOSED socket ref / a hung CONNECTING handshake / a backoff that ran its full delay after resume, and made v3's GexCandlesCard phone-aware (new `useIsPhone`, `size="touch"` controls, `sheet` popover, single-column Board stack). Verified with `tsc --noEmit --noResolve` on every touched file - esbuild alone had missed the ReferenceError.

## Monday 8/31/2026 - Comped accounts 401'd on every API: ws-auth missing the comp_access join (`server-v2/ws-auth.js`)

`lib/db.ts`'s `getSessionWithUser()` (used by middleware, decides whether the page renders) defines `is_paid` as a live Stripe subscription OR a live `comp_access` grant; `server-v2/ws-auth.js`'s `getSessionForToken()` (gates every `/api/*`, all of `/proxy/*`, and the WS upgrade) had only the subscriptions join, so comped users passed middleware, rendered the page, then got a permanent 401 on every data call and no live feed - the ICT "Couldn't load today's plays: Error: HTTP 401" report. Added the comp join to both queries clause for clause from `lib/db.ts`, with `getAccessForUser` checking `is_comped` before subscription status so a churned-then-comped account isn't denied by its stale `canceled` row; revoked/expired comps still don't match and `is_owner` is untouched. Verified against a stubbed pool: comped-only and comped+canceled -> allowed, subscriber -> allowed, neither -> still `inactive`.
...
**GEX Candles rail (v3).** Dropped the strike column and the dollar column and killed the diverging layout: every bar now anchors to the same left edge and grows right, sign carried by colour alone, 184px → 96px. The chart's price axis already labels each row's height and the bar length already encodes size, so both columns were the same fact in a second notation; one baseline also beats two half-width scales pointing opposite ways. Exact strike and value survive as the row's hover title. Positioning, level tags and the priority thinning pass are untouched.

**GEX Chart (v3) — basis, split, DEX, expiry, core, ten cards.** The card had no toggles at all because v2 drives every one of them as a prop from its home page, which v3 does not have. It owns them now: a BASIS segment (OI+VOL · VOL · FLOW), a SPLIT segment (net bar, or call up / put down), a DEX chip, the expiry the bars are drawn from as a toolbar pill and a pane label, v2's CB badge on the biggest-|net| strike (`CB` / `CB·Vol` / `CB·Flow`), and v2's ten home stat cards above the chart behind a ⚙ Cards popover with per-card and master toggles.

`values.ts` is the point of the diff: renderer, tiles and header total share one definition of every level. Net is READ off the wire (`netGEX + netVolGEX`) because Key Levels, the rail and Multi Greek all do, so the chart can never be the one card that disagrees about where the core is; the per-side split is RECOMPUTED (γ × contracts × spot²) because there is no `callVolGEX` field — at the row's own `spotPrice`, which is what makes `call + put === net` to the cent instead of drifting as spot ticks. DEX draws v2's line on its own 60% scale with no gridlines (delta is orders of magnitude off gamma in dollars and would pin flat to zero on the bars' axis) and is suppressed outright when every value is 0.

`GexRow` gains `spotPrice`, `netDEX`, `volNetDEX`, `flowGEX` — all real `computeGexRows()` fields, the last three optional because `chainGex.ts` cannot always fill them; `mgMath`'s `Leg` gains `delta` so a non-socket ticker can still compute its DEX legs. `flowGEX` is left ABSENT rather than zeroed on that path, since a column of zeroes reads as a flat flow book and "absent" is what the fallback tests. New `--color-dex` token, a third hue on purpose. `SegGroup`, `Chip`, `Popover` and `PanelSection` were promoted from `gexCandles/controls.tsx` to `design/primitives/Controls.tsx` under the note in that file saying to move them the moment a second card wanted one; the old path re-exports them so no import changed.

**FLOW is unclickable when there is no flow GEX.** `SegGroup` options take a `disabled` flag — inert, dimmed, `cursor-not-allowed`, still carrying a tooltip that says why. Disabled rather than hidden: a control whose buttons come and go cannot be learned and a vanished option gives no reason. Gated on the ladder having ARRIVED (`flowSupported([])` is false, and empty is the state the card is in for the first second of every load, so an ungated test would grey the button on arrival and un-grey it a beat later). And a stored FLOW choice is never rewritten — when it is selected AND disabled it stays highlighted at 50% while the pane says it is drawing OI+VOL, so the preference survives a look at a ticker and comes back on the next SPX board.

The mock emits the new legs and mocks `/proxy/flow-history`, or VOL, FLOW, the DEX line and the Bull/Bear tile are all untestable without a backend — the one thing `npm run mock` exists to prevent.

**Typecheck-verified, not build-verified.** `tsc --noEmit` was run against the full v3 tree with the repo's own compiler options in a scratch install: zero new errors (the two `import.meta.env` reports are the missing `vite/client` types in that scratch tsconfig, and the `BubbleBucket` mismatch at `GexCandlesCard.tsx:408` reproduces on an unmodified checkout — pre-existing, untouched). Not run: `npm run build`, `check:ws`, `perf`. Not deployed.

## Saturday 8/29/2026 — Earnings tab: the market-cap floor was deleting the list it was meant to produce (`server-v2/earnings-calendar-recorder.js`, `server-v2/server-with-proxy.js`, `lib/econCalendar.ts`, `hooks/useEconCalendar.ts`, `components/pages/EconomicCalendar.tsx`, `components/dashboard/EconCalendarPanel.tsx`)

`earnings-calendar-recorder.js` only stored Nasdaq rows above `EARNINGS_MIN_MCAP` ($25B), which threw away every sub-$25B "most anticipated" name (CRDO, GTLB, PATH, CIEN, FIVE, OLLI, DLTH, DAKT, GCO, KNOP, BLRX, HMR) before a client filter could ever reach them — so the floor now defaults to 0, the sweep covers this week AND next (Sat 09:00 ET both-week rebuild plus a daily 06:30 ET current-week pass that drains the "time TBD" bucket), rows carry `sweep_ver` so the first boot re-widens both weeks by itself, and inserts batch 400/statement with per-day symbol de-dupe. The narrowing moved to `lib/econCalendar.pickAnticipated` (mcap ≥ $25B OR the maintained `ANTICIPATED_SYMBOLS` list, topped up to ~14/day) shared by all three surfaces, `/proxy/earnings-week` gained `?week=this|next|both` (default `both`, full Mon–Fri) and `earnings-week-run` finally forwards `&minMcap=`, and the earnings tab gained This wk / Next wk and Anticipated / All toggles plus a `fmtMcap` that no longer renders every sub-billion name as "$0B".

## Saturday 8/29/2026 — Level Log: wall-migration legend swatches land on their labels in the PNG (`lib/snapshot.ts`, `components/pages/LevelLog.tsx`)

The expanded wall-migration chart's PNG drew the four colour squares several px above "Put Wall" / "Call Wall" / "CORE" / "spot" — html2canvas paints each run at `textRect.top + baseline` from a probe built on the PAGE document under the body's line-height, so the `data-cap-center` rewrite's 3px of padding slack on a 16px chip cannot buy the drop back and the square, pinned to the box centre, stays put. `captureBaselineBias` became `captureFontProbe` (returns the raw `baseline` as well as the `bias`) and a new clone-only `alignCapSwatches` pass re-pins anything marked `data-cap-swatch` onto the cap band of the run beside it (baseline − 0.72em/2); the legend swatch in `LevelLog.tsx` opts in. Box, border and size untouched, live page untouched, and a probe that fails to lay out returns `baseline: 0` and switches the pass off. Verified against a real html2canvas render: swatch centre vs label cap centre 3.0px → 0.0px.

## Friday 8/28/2026 — Wall migration chart: clickable series, two roles instead of three levels, popout snapshot (`components/pages/LevelLog.tsx`)

`WallMigrationChart` in `components/pages/LevelLog.tsx` gained clickable legend chips (each series toggles off/on), dropped its caption paragraph, grew 190 -> 250px, and replaced the "hide the wall the CORE already is" mask — which made green and red blink out mid-session — with PostMarketTab's two-role model (CORE = heavier wall, OTHER = the lighter one, drawn per-side in green/red so the lines swap instead of vanishing). The popout added a snapshot button (same `SnapLogButton` + `lib/snapshot.ts` path), a 58px CB Edge watermark over the bottom-right of the plot, a weekday-over-date axis (MONDAY / 8/21), and `data-capture-hide` on its head; legend chips went inline-block + `data-cap-center` so their swatches stop riding high in the PNG.

## Friday 8/28/2026 — v3 terminal: four real cards, ES/NQ dropped, GEX bubbles calibrated in pixels (`cbedge-v3/src/board/gexCandles/*`, `cbedge-v3/src/board/{multiGreek,keyLevels,econCalendar}/*`, `cbedge-v3/src/board/{catalog,BoardPage}.tsx`, `cbedge-v3/src/{contract/frames.ts,data/api.ts,design/tokens.css}`, `cbedge-v3/scripts/mock-server.mjs`)

Rebuilt the v3 board's four cards against v2 line-by-line — GEX Candles (candlestick + GEX bubble overlay, RTH/ETH, interval picker, watchlist dropdown, forming-bar countdown, jump-to-latest), Multi Greek (four editable ticker ladders, ex-0DTE total, gold CB / star on later expiries, ATM ring + chip), the six premarket Key Levels tiles and the home Economic Calendar — then dropped ES and NQ from v3 entirely, which deleted the whole `/proxy/es-spx-basis` conversion path (every symbol now charts against its own strikes) and collapsed candles onto `/api/snapshots/etf-candles`; `src/board/esCandles/` and `src/board/lwChart.ts` are tombstoned and want deleting. Four defects fixed along the way: `staleMs` was a cache TTL that never refetched (added `pollMs` + a live forming bar off the `spot` frame), the bubble layer was calibrated in bar space so 1m drew a slab and 5m a dotted line (now stroked across a pixel span, gamma looked up by time, verified identical across a 60× bar-count change), `scripts/mock-server.mjs` still served the previous three endpoints so every card silently got `index.html` as JSON, and v3's ATM auto-scroll now actually centres (v2's `offsetTop` is measured from an unpositioned ancestor and lands a constant offset low). `npm run check` green: typecheck, build, budgets (77.9kb initial vs 109.4kb ceiling), all 8 ws-scope assertions. Owner-gated at `/v3/*`; not customer-visible. Not deployed.

## Thursday 8/27/2026 — ES Candles: SPX symbol, RTH/ETH switch, no side panel, any ticker; recorders go 1-minute and follow the live watchlist (`components/pages/EsCandles.tsx`, `app/es-candles/page.tsx`, `components/dashboard/es-candles/{EsChartCard,symbols}.tsx`, `server-v2/{api-router,candle-history,etf-candle-recorder,etf-gex-recorder,levels-engine,levels-auto-publish}.js`, `server-v2/state/retention-cleanup.js`)

/es-candles gained an SPX symbol (same `$SPX` gamma on cash-index candles, no basis), an RTH/ETH switch filtering the plotted `rows` after roll-up, an open ticker picker backed by a new `GET /api/es-candles/tickers`, and lost the GEX rail / 0DTE chain (`sidePanel="none"`) — with `ChartSymbol` relaxed to `string` and `symbolDef` synthesising unknown tickers, plus a live-dxLink fallback in `/api/snapshots/etf-candles`. Server side, `etf-candle-recorder` and `etf-gex-recorder` now cover the far-CB roster EVERY minute (new `fetchIntradayCandlesMulti` puts the whole roster on one dxLink connection; the GEX lane uses a bounded worker pool at ±25 strikes), both resolve their rosters per tick from the owner Watchlists page instead of frozen file lists — as do the picker route and `levels-engine`/`levels-auto-publish`, which were reading em-tickers' require-time `SYMBOLS` — behind `ETF_*_WIDE_MAX` caps, and `etf_candles` finally got a retention cut (`RETENTION_ETF_CANDLES_DAYS` 30). **NOT build-verified** — no repo `node_modules` in this session; all server files pass `node --check` and the TSX parses clean under esbuild. Run `npx tsc --noEmit` + `npm run build` before push. Not deployed.

## Thursday 8/27/2026 — Level Log: four recorded variants, a pop-out week chart, CORE means CORE, 1-minute sweeps (`components/pages/LevelLog.tsx`, `server-v2/scanner-variants.js`, `server-v2/scanner-recorder.js`, `server-v2/walls-recorder.js`, `server-v2/walls-reach.js`, `server-v2/server-with-proxy.js`)

`/level-log` gained two independent switches over FOUR server-recorded variants: expiry scope (`0dte` = `chain.expirations[0]`, vs `agg` = every other listed expiration summed per strike via `computeGexRowsMultiExpiry`) crossed with GEX basis (`oivol` = `netGEX + netVolGEX`, vs `vol` = `netVolGEX` alone). New `server-v2/scanner-variants.js` is the single source of truth for the four keys and the default pair. Both switches re-fetch — nothing is derived client-side, because a different basis is a different argmax, not a re-slice of the rows already loaded. Defaults are `0dte` + `oivol`, so a first load is byte-for-byte the log the page always gave.

Recording, not computing. `scanner-recorder.js` writes all four readings per ticker per sweep into a NEW `scanner_variants` table, and `walls-recorder.js` runs its whole slot pass once per variant, tagging `walls_log` / `wall_events` with `expiry_scope` + `basis`. **`scanner_snapshots` is untouched** and still gets exactly one row per symbol per sweep on the default variant — the same reasoning `forward-scanner-recorder.js` documents for its own separate table: `walls-recorder.sampleUniverse`, `walls-reach.getWatch`/`buildSessionRows`, `/proxy/scanner` and the forward sweep all do `SELECT DISTINCT ON (symbol) … ORDER BY ts DESC` and would have been handed an arbitrary one of four rows. `walls-reach.js`'s unfiltered `wall_events` read is now pinned to the default variant, or a level tested once would have reported four tests.

The two bases are free (same computed rows, different wall metric — new `basisNet()` / `levelsFor()`); only the aggregate leg costs upstream calls, so it is bounded (`SCANNER_AGG_MAX_EXPIRIES` 4, `SCANNER_AGG_MAX_DTE` 45) and rides its own sub-cadence (`SCANNER_AGG_EVERY_N_SWEEPS` 5). `SCANNER_VARIANTS_ENABLED=0` writes the legacy row only.

`/proxy/walls` takes `&scope=` and `&basis=` on the day view, the per-symbol log and the `&series=1` read. Both opt-in, both falling back to the historical pair, so a request naming neither returns what it always returned; `series=1` reads `scanner_snapshots` for the default and `scanner_variants` for the other three, so it cannot regress on a box where the new table is still empty.

**CORE said two different numbers on one page.** The Wall Migration chart carried a "role" model where CORE meant *whichever WALL carried more gamma at that slot* — MSFT read CORE 505 under ALL (the call wall) and CORE 500 under the CORE view (the recorded `cb`). Removed. The chart draws one line per RECORDED level, CORE last so an overlap is visible rather than hidden. Level colours moved to `LEVEL_COLORS.cb` (gold CORE) / `ES_CANDLE_UP` (green call wall) / `LEVEL_COLORS.pw` (red put wall) — deliberately not `LEVEL_COLORS.cw` (blue) or `HOME_THEME.green` (the status palette's light blue `#8ECAE6`), neither of which reads as GREEN beside a red put wall.

`WallMigrationChart` now takes an ARRAY of `DaySlice` instead of one day's log, so the inline single session and the new pop-out week view are the same drawing code. `⤢ Expand` opens it full width at 2.2x height over a scrim (Esc / click-outside to close) with a Today / 5-sessions range switch. Each session gets an equal width slice and they share ONE price scale — a wall holding its strike across the week draws as one flat run — and nothing is drawn across a session boundary. New `useWallDays` fetches in two waves: cheap level logs for more candidate weekdays than needed (holidays drop out), then a 1-minute tape only for the days that survived; levels render first, the tape sharpens the price line on a second pass. The popout is mounted only while open.

`SCANNER_INTERVAL_MINS` 5 → **1**. The walls grid still writes on its own 15-minute slot clock; this is how FRESH the sample under each slot is. A sweep that outruns the interval is skipped, not queued.

Schema, all additive and idempotent: `scanner_variants` (new); `walls_log`/`wall_events` gain `expiry_scope TEXT DEFAULT '0dte'` and `basis TEXT DEFAULT 'oivol'`, so every pre-existing row is correctly labelled as the reading it always was. The old narrow UNIQUE constraints on both tables are dropped **by column set, not by name** (created inline by `CREATE TABLE`, so the name is whatever Postgres generated) and replaced by variant-aware unique indexes — without that, three of every four writes would be swallowed by `ON CONFLICT DO NOTHING`. Non-0DTE and vol-only logs are recorded forward only; the page says so rather than showing an empty log that reads as a quiet session.

**NOT build-verified** — no repo `node_modules` in this session. All five server files pass `node --check`; `LevelLog.tsx` was type-checked clean under an isolated `tsc --strict` harness (the only diagnostic came from a stubbed local module). Run `npx tsc --noEmit` + `npm run build` before push. Not deployed.

## Thursday 8/27/2026 — Premarket / Post-Market: one page for every symbol (`components/pages/Premarket.tsx`, `components/pages/premarket/*`)

`/premarket`'s symbol picker went from a hand-typed `["SPX","SPY","QQQ"]` to the scanner's **MAIN watchlist** (14 names, imported from `lib/scannerTickers.ts` — static, because `/proxy/scanner-tickers` returns one flat array with no group labels), and the pill row became a `.dsel` select because fourteen pills wrap the page head. MAIN rather than an arbitrary list: it is the scanner's 2-minute hot lane, which is the roster `walls-recorder.js` samples per symbol, so every name offered has a real recorded wall log behind it.

The bigger change is that the non-SPX symbols now render **the same page**. They used to be routed to `components/pages/premarket/TickerBoard.tsx`, a second, smaller board carrying about a third of the panels — no regime strip, no level rail, no six Key Levels tiles with their prior-close migration lines, no scrolling GEX profile with DEX/vanna, no expected-range track, no gamma bell curve, no catalysts, and a Post-Market recap that was a summary rather than the recap. New `components/pages/premarket/chainGex.ts` is a THIRD source feeding the page's one destructuring (`useMobileGex` = SPX socket, `frozenGexOf` = a captured past session, `useChainGex` = any ticker off `/api/expirations` + `/api/chains` on a 1m poll), returning the same shape with RAW per-strike legs so the OI / OI+VOL / VOL switch recomputes identically. `TickerBoard.tsx` is tombstoned and no longer imported.

`PostMarketTab` gained a `symbol` prop that ROUTES rather than labels: `useIntradayLadder(…, symbol)` (new param; the route resolves an absent symbol to `$SPX` server-side, so a TSLA recap without it drew SPX's ladder, spot path and close under a TSLA heading), `useRecordedWalls(etDate, symbol)` (was hardcoded `"SPX"`) and `useNextExpiryStructure(…, ticker)` (had `ticker=SPX` baked into both URLs — a real bug: SPY/QQQ post-market boards had been printing SPX's next-expiry walls). Overnight context reads that symbol's own `etf_candles` via `useEtfCandles`; ES basis, the "ES 6,812" sub-lines and frozen sessions stay SPX-only and say so.

Scale was the quiet half: every price literal on the page was written for SPX. Added `kDp` (strike decimals off the ladder's step), `pxDp` (`spot >= 1000 ? 0 : 2`), `pxEps` = 0.015% of spot (was a literal `1`), `pinEps` = 0.15% (was `10`), `gapEps` = 0.004% (was `0.25`, one ES tick), and `tol` in PostMarketTab went `Math.max(3, spot*0.0005)` → `Math.max(0.01, …)`. Each evaluates to exactly the old constant on SPX. `GammaBellCurve` printed every price through `nf0()` (whole numbers) — now a decimals-aware formatter; GEX magnitudes still use `fmtB`.

`totals.vanna` is now `number | null`. `netVanna`/`netVolVanna` are published per strike by `server-v2/computation/vex-chex.js` and cannot be recomputed, so a chain with no per-contract vanna has NO vanna — `?? 0` summed into a confident `$0` that read as "nets out". `chainGex` reproduces `computeVexChexRow` exactly when the payload carries `vanna`, and both tabs print `—` otherwise. Deliberately not rebuilt from Black-Scholes (the server's `bsGreeks` returns 0 for T=0, so a client rebuild would put two differently-scaled tiles on screen). Also removed the one row that made a non-SPX board a different SHAPE (an extra `{SYM} change` stat), and the footer strip no longer drops its ES segment (reads `/ES` from the quotes batch the row above already fetches).

`gammaChartKit.ts`: the ±3% board is a percentage, and a strike ladder is not. ±3% of SPX is ~92 five-wide strikes; ±3% of a $257 AMZN is SEVEN. New `wideHalfOf()` keeps ±3% as the base and widens to the 60th-nearest strike (`WIDE_MIN_STRIKES`), capped at ±30% (`WIDE_MAX_BAND`); it caps AUTO, the pan clamp and the wheel. Widening the board alone was not enough — AUTO was hitting its own σ, not the board — so `AUTO_MIN_STRIKES = 20` + `strikesHalf()` put a floor on the window in the unit the eye counts. `MIN_HALF` became a cap on `min(MIN_HALF, 3 × gridStep)`. SPX is unchanged by construction in all three.

`GEX Profile by Strike` was centring with `i * 19 + 9.5 - el.clientHeight / 2` — a literal row height and a height read before layout. A `clientHeight` of 0 clamps to max scroll, so the card opened at the BOTTOM of the ladder; the live socket hid it (a new frame retried every second) but a chain-poll symbol's next attempt is 60s away. Now measures the row element, returns "not ready" and retries on rAF for ~20 frames, re-centres via `ResizeObserver`, re-pins on symbol change, and guards the un-pin against its own scroll write.

**NOT build-verified** — no repo `node_modules` in this session; every changed file was parse-checked with esbuild. Run `npx tsc --noEmit` + `npm run build` before push. Not deployed.

**Left for a decision:** `server-v2/premarket-baseline.js` has `ALLOWED_SYMBOLS = new Set(['$SPX','SPY','QQQ'])`. The page now sends `&symbol=` and the route understands it, so the "vs prior close" chip, the Key Levels migration lines and Biggest GEX Changes fill in for those three and show their empty state on the other eleven. The allowlist is deliberate — a miss starts a full settled-chain ThetaData sweep per symbol and nothing coalesces a loop over them — so widening it is a cost decision, not a typo. Untouched.
...
## 2026-08-23 — Earnings tab rebuilt as a week board; premarket cards on the app theme; 19 logos unstuck from a cached 404 (`components/pages/EconomicCalendar.tsx`, `components/shared/ChipLogo.tsx`, `next.config.js`, `components/pages/Premarket.tsx`, `components/pages/premarket/PostMarketTab.tsx`, `components/pages/premarket/HistoricalRecap.tsx`, `scripts/fetch-ticker-logos.mjs`, `public/logos/` +17)

The `/economic-calendar` **Earnings** tab was the calendar's row grid with the events removed — a full-width band per session that spent 2000px on four 46px chips — and is now a week BOARD: one card column per trading day (`repeat(auto-fit, minmax(210px,1fr))`, auto-fit deliberately, since globals.css's GLOBAL GRID COLLAPSE flattens fixed `repeat(N)` on phones but re-exempts auto-fit), each with a centred date strip (`1fr auto 1fr` grid — a flex row with the count on `margin-left:auto` centres nothing) and PRE/AFTER/TBD chip grids at `CHIP_LOGO = 42` (the largest logo that clears the tile padding without reflowing the four-across track). Cards are lifted off the background by a white-alpha `BOARD` rung set over `HT.panelBg` — five columns of a 45%-dark panel over `#05060A` read as one flat black rectangle. The Shot button now **copies to the clipboard** (`captureAndCopy`, download only as fallback, and the button says which) and on the earnings tab captures the **board alone** via `earnRef`, which carries its own header and a bottom-right CB Edge mark so the pasted PNG is self-contained. No `#8a9ab8` left in the file.

Nineteen earnings names rendered as ticker-TEXT chips because they were not in `public/logos` — mirrored now, `HEI.A` gets the `PBR.A` copy-of-root treatment in `MANUAL`, and `SEED` in `scripts/fetch-ticker-logos.mjs` went ~190 → ~490 HEAD-verified symbols. **They were still invisible after that**: `next.config.js` served `/logos/:path*` `immutable, max-age=1y` and Next matches headers on the PATH, so every pre-mirror 404 was cached for a year. Immutable is now gated on `?v` (`has`/`missing`), `ChipLogo` appends `?v=LOGO_REV` (bump on every mirror add), and unversioned requests get `max-age=300`.

`/premarket`'s scoped `.pmk` token block is interpolated from `homeTheme` instead of the mockup's own slate ramp — `--panel2` → `HT.panelBg`, `--card`/`--line` → `HT.border`, `--pos`/`--neg` → `ES_CANDLE_UP`/`ES_CANDLE_DOWN`, `--blue` → `LIGHT_BLUE`, `--amber` → `HT.orange`, `--r` 10 → 12 — plus derived alpha rungs (`--posWash`, `--blueFill1`, …) because PostMarketTab and HistoricalRecap are separate template literals whose hand-typed `rgba()`s would otherwise keep the old hues. Active states went cyan; `.prep`/`.prep.is-post` lost their coloured 1px ring; `--cw`/`--pw` deliberately NOT re-pointed at `LEVEL_COLORS`.

**Verified:** esbuild TSX parse clean on every touched file, `node --check` on `next.config.js` and the fetch script; the generated `.pmk` CSS was rendered in a headless Chromium against representative markup (`generated/2026-08-23-premarket-card-theme.png`) and every `var()` used across the four `.pmk` files was asserted to resolve with none orphaned; committed PNGs md5-matched and confirmed valid 250×250. These files are **CRLF** in the repo and are written back as CRLF. **NOT build-verified** — no repo `node_modules` in this session; run `npx tsc --noEmit` + `npm run build` before push. Not deployed. Full write-ups in `md files/CHANGELOG.md`.

## 2026-08-23 — Feedback is a support ticket system, customer + owner (`server-v2/api-router.js`, `app/feedback/page.tsx`, `components/shared/FeedbackThread.tsx` NEW, `app/owner/feedback/page.tsx` NEW, `owner-vite/src/pages/Feedback.tsx` NEW, `owner-vite/src/pages/registry.ts`, `owner-vite/src/lib/nav.ts`, `components/shared/OwnerSidebar.tsx`, `components/shared/UserMenu.tsx`)

`/feedback` was fire-and-forget; every submission is now a ticket with a thread on it — `server-v2/api-router.js` gained `GET/PATCH /api/feedback/:id` and `POST /api/feedback/:id/messages` plus a lazily-created `customer_feedback_messages` table and `user_read_at`/`owner_read_at` marks on `customer_feedback` (ensureX pattern, deliberately not `lib/db.ts`, which is behind `_lib-db.cjs`), with `?scope=mine` so the customer page is "my tickets" even for the owner and `isAuthor` on the thread read so the right side reads as "You". The inbox ships twice on purpose: `app/owner/feedback/page.tsx` for cbedge.net and `owner-vite/src/pages/Feedback.tsx` (Content → Feedback in `owner-vite/src/lib/nav.ts`) for owner.cbedge.net, which is where the queue is actually worked; full write-up in `md files/CHANGELOG.md`.
...
## 2026-08-21 — Post-Market recap: honest bars, full move log, premium panel, section 7 deleted (`components/pages/premarket/PostMarketTab.tsx`)

One file, four changes. **Section 3 "How the book was built"** had two real bugs: `maxAbsBar` was scaled over `evNear` (±12 strikes) while the panel renders `evBars` (±60), and the peak ghost was hard-clamped with `Math.min(46, …)` — so every far strike clipped to an identical full-length hatch, which read as "they all peaked at the same number." Scale now runs over the rendered rows (net and peak). Bars were also un-mirrored: all grow right off a shared left edge, with sign moved into a `+`/`−` chip and a signed dollar column (`.sgn` / `.netcol`), plus a `.pmk .evrow .track` override so the premarket profile's centre-axis gradient stops painting a meaningless zero line. Second bug: the AM/MID/PM buckets were hard-wired to `idxAtMin(12:00)` / `idxAtMin(15:00)`, and `idxAtMin` snaps to the NEAREST column — on a day the ladder recorder starts after noon it returned index 0, making the AM move `|vals[0]−vals[0]| = 0` for every strike, so the blue segment could not exist. `evCover` + `activeBuckets` now derive the buckets from the recording's real coverage, the legend renders only live buckets, and a warnbar names the window and what was dropped.

**Section 2** move log now scrolls the whole day (`.mvscroll`, ~9 rows, count in the header) instead of silently `.slice(-8)`-ing away the morning. **Section 6** replaced the replay scrubber — which re-derived numbers printed elsewhere — with **Where premium actually went**: top 5 legs by `volume × mark × 100`, shared bar scale, calls green / puts red to match `--cw`/`--pw`, and a warnbar rather than a fake `$0` when the frame has no marks. **Section 7 STRIKE PATHS deleted** after four rounds of redesign (mockups kept in `generated/`: `2026-08-21-strike-paths-4-ideas.*`, `-move-visible.*`, `-scrolling-window.*` — direction-coded lines, growth heat ladder, momentum sort, indexed-to-open, two-column, step chart, event-time axis, and an interactive 60-minute scrolling window with pinned strike/value columns). 25 rows at 2.5h wide and 22px tall, each scaled to its own peak, could not distinguish a strike that doubled from one that flatlined; none of the redesigns earned the vertical space. `strikePaths` memo and `.pathlist`/`.prow` CSS removed with it; `evNear` stays for `writtenVsTraded`. File 1,775 lines.

**Verified:** esbuild TSX parse clean after every edit. **NOT build-verified** — no repo `node_modules` in this session; run `npx tsc --noEmit` + `npm run build` before push. Not deployed.
...
## 2026-08-20 — Premarket: GEX level rail, POST-MARKET tab, scrolling profile, phone Prep tab (`components/pages/Premarket.tsx`, `components/pages/premarket/PostMarketTab.tsx` NEW, `components/pages/premarket/postMarketData.ts` NEW, `components/mobile/pages/MobilePrep.tsx` NEW, `components/mobile/mobileNav.ts`, `app-vite/src/App.tsx`, `app/app/m/prep/route.ts` NEW)

`/premarket` gained a one-axis GEX level rail above the Call Wall card (put wall / flip / core bullseye / spot / call wall on one shared price domain), a scrolling ±60-strike GEX profile that stays centred on spot until you scroll it yourself, and a PRE / POST tab switcher that auto-picks by the clock; the new Post-Market tab grades the day from the SAVED SPX wall log (`/proxy/walls`), diffs the 09:30 recorded ladder against the live one, and builds tomorrow's structure off the next expiry's chain. The shared fetching/grading layer was split into `postMarketData.ts` so the phone build could reuse it — the EM slot in the mobile tab bar is now **Prep** at `/m/prep` (`/m/em` stays routed). Full write-up in `md files/CHANGELOG.md`.

## 2026-08-19 — New page: Premarket Prep, live (`components/pages/Premarket.tsx` NEW, `app/premarket/page.tsx` NEW, `app/app/premarket/route.ts` NEW, `app-vite/src/App.tsx`, `components/shared/GlobalToolbar.tsx`, `components/pages/TradersDashboard.tsx`)

New `/premarket` board: regime header (net GEX, gamma flip, spot, one-line bias), six key-level cards (call wall / 0DTE magnet / spot / max pain / flip / put wall), per-strike GEX profile with spot + flip overlays, overnight context (ON range, gap vs prior close with an inside/outside-prior-range read, gap-fill target, biggest strike GEX changes, sector heat from `/api/scanner/market-quality`), and expected range + playbook + today's catalysts — all live off `useMobileGex` / `useEsCandles` / `useEconCalendar` / `/api/quotes-batch`, with "vs prior close" backed by the page's own 15:40–16:10 ET localStorage snapshot. It lives in `components/pages/` because anything under `app/` gets prerendered by Next and this tree needs a socket (that mistake failed the Docker build once); `app/premarket/page.tsx` is a force-dynamic redirect to `/app/premarket`, the toolbar gets a 🌅 Premarket item, and Traders Dashboard gets a header button plus a Quick Links default. Full write-up in `md files/CHANGELOG.md`.

## 2026-08-19 — GEX levels replay: ladder no longer jitters under a walking spot (`components/pages/Analytics.tsx`)

The `/replay` → GEX levels ladders (`TickerLookupCard` / `TlLadder`) anchored their window and auto-centre scroll on the strike nearest the LIVE spot, so every few replay frames spot crossed a strike, `tlWindow` re-sliced and the pane scrolled — a constant shudder. New `useTlAnchor()` holds the anchor strike until spot walks `TL_ANCHOR_SLACK` (5) strikes away, `TlLadder` takes an optional `anchor` prop and centres on that row (falls back to spot, so the live path is unchanged), and the nearest-strike scan is factored out into `tlNearestIdx()`; the ◀ caret, lit row and level chips still read the real spot. Full write-up in `md files/CHANGELOG.md`.

## 2026-08-17 (b) — Mobile ES Candles: price zoom moved to a gesture on the axis, and fixed (`components/mobile/pages/MobileEsCandles.tsx`)

The ± stepper added earlier today is gone — it covered candles, and it silently did nothing on a still chart: `setAutoScale(true)` does NOT re-run `autoscaleInfoProvider` (the price range is cached until something invalidates it), so the zoom only limped in on the next data tick. `applyYZoom()` now sends `priceScale().applyOptions({ autoScale: true, mode: PriceScaleMode.Normal })` — re-passing `mode` is the only public call that invalidates that cache. In its place: a transparent `zIndex: 4` strip sized to the measured `priceScale('right').width()`, press-and-slide (up = in, down = out, 150px per doubling, clamped 0.4x–12x), double-tap to reset, pointer-capture so a wandering thumb keeps the gesture, multiplier pushed through a ref inside pointermove so the axis tracks the thumb rather than a React render. Native `axisPressedMouseMove.price` / `axisDoubleClickReset.price` turned off — they freeze the range and would fight the strip; autoscale stays on under the multiplier so new bars re-centre. All of it verified in headless Chromium against the real lightweight-charts v5 bundle with synthesised pointer events. Full write-up in `md files/CHANGELOG.md`.
...
**The chart carries dots, not boxes.** Every play drops a single **entry dot** at its entry price on its trigger bar: green ▲ for a long, red ▼ for a short (direction, not outcome), solid with a halo while it's working, faded with ✓/✕ once it resolves. **Click a dot** and that one play opens its **TradingView-style Long/Short Position box** — green profit zone entry→3R, red stop zone entry→structural stop, the entry line between them, dashed 1R/2R rungs that tick ✓ as they're tagged, a header chip reading `TURTLE SOUP · SHORT · 3.0R:R  LIVE` — plus a popup card with the same numbers in text. Click again (or anywhere empty) to close. Drawing every box at once was tried first and was unreadable; one box at a time is the whole point of the dot. New `▶ Plays` toolbar toggle (ON by default) and a `▶ Plays` card under the chart whose rows open the same popup, so a play is reachable without hunting for its dot.

**No "possible / armed" state — a play appears only when it actually fires.** An armed tier was built first (pool pierced, waiting for the close back inside; price walking into an OB or the OTE band) and then cut, for two reasons. The owner's call was live-only. And the pierce test was wrong in a way that showed: `last.low < pool.price && last.close <= pool.price` is true for **any** pool price has simply left behind, so a swept-EQL long stayed armed at 28524 while price ran away to 28480 — a setup pointing the wrong way, sitting above a collapsing market. Both the tier and the bug are gone; `PlayState` is now `live | won | lost`.

**The risk math is deliberately a copy of the recorder's** (`app/api/ict-setups/route.ts` → `extractSetups`): entry = trigger-bar close, stop = the nearest swing pivot on the wrong side of entry gated on `confirmTs` (never `ts` — that was the lookahead that inflated every R on `/dev/results`), buffered by `0.15 × ATR(14)` with a ≥1-buffer floor so a degenerate stop can't divide by ~0. Targets are the 1R/2R/3R ladder off that risk, so a box tagging 2R on the chart is the same 2R the leaderboard counts. The header comment in both files says to change them together; if they drift, the boxes stop matching the win rates. WON/STOPPED is graded by walking the bars after entry the same way the recorder does (peak MFE in R; stop-and-target on the same bar resolves as the stop and banks no new rung, since there's no intrabar path), and a resolved play's box ends at the resolving bar and stays up ~6 bars before dropping.

Scope is the **model plays only** — Turtle Soup, Judas, CISD, 2022 Model, Breaker, Inducement, valid-OB retest, OTE entry. BOS/CHOCH/displacement/every raw FVG fire far too often to mark; they stay as zone overlays. Cap of 4 plays, live first, at most 1 resolved, so a chatty detector (inducement fires ~50×/day on 5m) can't crowd out the live markup.

Four layout problems found by rendering it rather than by reading it: the popup opened beside its own dot and **covered the box it describes** — it now goes to the LEFT of the dot (the box always runs from entry to the right edge) and to the vertical side the 3R leg doesn't occupy (below a long, above a short); co-located dots (Turtle Soup → 2022 Model fire on the same bar at the same price by construction) drew on top of each other and only the top one was clickable, so collisions nudge **sideways**, never vertically, since y is the entry price; a play triggering on the live bar had no room to its right and collapsed into a sliver against the price scale, so a box under 132px hangs back to the LEFT of its entry instead (the dot still marks the true bar; the box only has to carry the levels); and box headers auto-stagger down a row when they'd collide, with price tags flipping outside-left and dropping when a zone is thinner than its text. Panning the chart ends in a click event, so the dot hit-test is gated on the pointer having moved <5px between press and release — otherwise every pan closed the popup.

Verification: `tsc --strict --noUnusedLocals` clean on both files (checked in isolation with shims for `lightweight-charts` / the `@/hooks` imports — this sandbox has no repo `node_modules`), plus an offline harness over a 400-bar synthetic series asserting stop-below-entry-for-longs / monotonic target ladder / R:R = 3.00 / finite positive risk on every play, and no-throw on 0–5-bar and zero-volatility inputs. The overlay draw code was extracted verbatim and rendered headless (Chromium screenshot) in both the dots-only and one-box-open states to tune the layout. **NOT build-verified** — run `npx tsc --noEmit` and `npm run build` before push (`app-vite/scripts/check-routes.mjs` is unaffected: no new route, `/ict` already exists).

## 2026-07-31 — Owner Results: CB contract trade tracker on TastyTrade/dxLink + its own Contracts tab with the probe chart (`server-v2/cb-contract-track.js` NEW, `server-v2/cb-trade-recorder.js` NEW, `server-v2/cb-contract-track.selftest.js` NEW, `server-v2/api-router.js`, `server-v2/server-with-proxy.js`, `owner-vite/src/pages/Results.tsx`)

Built the contract-pricing half of the Confidence board the way every other contract in this app is priced: `/proxy/probe-rest?ticker=SPXW&expiry=<today>&type=<C|P>&strike=<CB>` — the TastyTrade/dxLink probe pipeline `/owner/probe` and `/api/watch` already ride. No Theta anywhere in it. New `server-v2/cb-contract-track.js` owns the rules: at 9:45 / 10:30 / 12:00 ET it reads the CB (Core Bullseye / MVC strike) live at that checkpoint out of `mvc_snapshots`, probes the CB-strike **0DTE** contract on the leg that gains as SPX travels TO the CB (SPX under it → the CB call, over it → the CB put — always the cheap/OTM side, which is why the sub-$1.00 filter selects real magnet setups instead of ATM premium), auto-buys when the probed mark is ≤ `$1.00` (`CB_AUTO_BUY_MAX`), then re-prices every open trade once a minute and auto-sells the first poll SPX is inside the 5–10 pt band of the CB (`CB_SELL_TRIGGER_PTS`, fired on the OUTER edge — price entering the band crosses 10 before 5, and a gap straight to 3 pts is still "within 10"; the distance at the fire is stored and flagged `tight` under 5). Anything still open at 16:00 is marked out at its last probed mark — 0DTE, there is no next session. `ticker=SPXW` deliberately, not `SPX`: `probeRestTT` resolves the chain under `chainTicker('SPXW') === 'SPX'` but keeps SPXW as the preferred root, which is the difference between the PM weekly and the AM-settled monthly on the one Friday a month TT returns both at the same date/strike/type.

Recorder, not backfill — and that is a property of the data source, not a shortcut. TastyTrade has no per-contract history; `server-v2/condor-marks.js` already documents the same wall (the condor daily series had to be rolled up from hourly TT ticks because the historical option endpoint 403s on this account). So new `server-v2/cb-trade-recorder.js` polls `POST /api/cb-trades {action:'tick'}` every 60s from 09:44–16:10 ET on weekdays (holiday-aware, `unref`'d timers, wired guarded in `server-with-proxy.js` so an optional module can never take the origin down on boot — the etf-candle-recorder lesson), and that tick opens due checkpoints, re-prices open trades, and settles at the bell in one idempotent pass. Consequence stated rather than hidden: **the table starts empty and fills forward from deploy; sessions the process was down for cannot be reconstructed.** A checkpoint whose grace window (`CB_CHECKPOINT_GRACE_MIN`, 20m) has passed is deliberately NOT opened late — a "9:45 entry" filled at 2pm is a fabricated trade, worse than a missing one.

Storage is `cb_trades` (one row per session+checkpoint, `UNIQUE (date, checkpoint)`) and `cb_trade_ticks` (the poll-by-poll mark/spot/distance curve), both created by the module through `libDb.getPool()` — the same escape hatch `/api/social-media/day-list` uses — so this ships with no `lib/db.ts` edit and no `_lib-db.cjs` esbuild rebundle. Every checkpoint gets a row **including the ones that never traded**: a `skipped` row carrying the probed price and the reason ("mark $1.35 over the $1.00 cap", "no MVC snapshot within the checkpoint window", "probe no-strike") is the only thing separating "nothing set up that day" from "the recorder was down", and without it the board silently rewrites its own history.

New `/api/cb-trades` (owner, with the standard `x-internal-token` bypass so the in-process recorder can post): GET `?since=20|?all=1|?date=` → `{trades, summary, config, checkpoints}`, GET `?ticks=<id>` → the poll curve, POST `{action:'tick'|'checkpoint'|'poll'|'settle'}` — the last three exist so a session can be repaired by hand without waiting on the clock. `/api/confidence/checkpoints` can still merge the same rows into its per-cell fields (`?contracts=0` to skip), kept so one request can answer both halves of a checkpoint for API callers.

UI: the dollar side got its **own Contracts tab** rather than more columns on Confidence — three extra columns per checkpoint on a table already carrying a strike, a distance and three tier ticks made both halves unreadable, so the Confidence tab is back to pure hit rates and fetches with `?contracts=0`. The Contracts tab is per-checkpoint roll-up cards over a deliberately short table — **Date · Time · Contract · Entry · Sell · P/L** — where Entry shows the disqualifying price in parens for a skipped row, Sell carries the trigger distance or `eod`, and P/L shows premium plus the ×100 dollars. Clicking the contract opens **the probe chart in a popup**: the same chart `/owner/probe` draws (gradient area under a cyan line, five gridlines, first/last ET stamps, dot on the latest point) but plotted from `cb_trade_ticks`, so the x-axis spans exactly the minutes the position was live instead of a rolling 1D/1W window. Metric toggles are Price / SPX / Dist to CB; the price view draws the entry as a dashed line (a rising curve reads as a winner even when it never got back to what was paid) and the distance view shades the 5–10 pt sell band, since "get out when price enters this stripe" is the whole strategy. Esc or the backdrop closes it.

Verified three ways. 32 offline checks (`node server-v2/cb-contract-track.selftest.js` — rules, scheduling, snapshot matching, and the probe boundary stubbed at `ctx.internalFetch`, asserting the SPXW/0DTE/CB-strike request shape). 17 integration checks run locally against a real Postgres covering the whole lifecycle — open, idempotent re-run, out-of-band poll, over-cap skip, put-side flip, in-band sell with booked P&L, EOD settle, read-back, rollups, confidence merge. And a jsdom render harness over `Results.tsx` (23 checks: tab wiring, the six columns, skipped/open row rendering, the popup, chart path sanity, metric toggles, Esc-to-close). That last one earned its keep: it caught `Number(null) === 0` slipping past `Number.isFinite` in the timestamp/number coercion helper, which was rendering every open and skipped row's P&L as a confident **"0.00"** instead of "—". Same bug was live in the server module's `num()` (a NULL `strikeOIVol` would have read back as a CB strike of 0); both are fixed and both now have a regression test. `tsc --noEmit` clean on `Results.tsx`; `node --check` clean on all server files. Not prod-build verified; needs deploy, and the first rows appear at 9:45 ET the next trading day.

## 2026-07-31 — CB contracts priced off dxLink, REST as fallback; bare cards (`server-v2/cb-stream.js` NEW, `server-v2/cb-stream.selftest.js` NEW, `server-v2/cb-contract-track.js`, `owner-vite/src/pages/Results.tsx`)

Held positions are now priced by a dxLink Quote subscription instead of a REST probe every 60s. The motivation is accuracy, not cost: the board leads with the day's PEAK, and a 60-second sample cannot see a spike between two polls — peak was a sampling artifact. Streaming sees every NBBO update, so the high-water mark is the real one.

Storage did not grow. A held 0DTE contract quotes hundreds of times a minute; a row per event would be ~1M rows a session for a chart that draws 375 points. New `server-v2/cb-stream.js` keeps an in-memory OHLC accumulator per symbol per minute and `drain()` hands back only COMPLETED 60-second bars — the forming minute is withheld deliberately, since persisting a partial bar would let its high be revised after it was written and the caller uses that high to move a watermark. `cb_trade_ticks` gained `mark_open`/`mark_high`/`mark_low`/`src`; `mark` still holds the close under its old name so existing rows read unchanged.

The failure model is the reason this is layered rather than swapped. A dead REST poll is loud — it throws and stamps `last_error`. A dead SUBSCRIPTION is silent: events stop, and a frozen row looks exactly like a contract that is not moving, which is the worst possible failure on a board whose job is to be trusted. So `isFresh()` requires a recent EVENT, never merely a connected socket, and any symbol past `CB_STREAM_STALE_MS` (90s) falls straight back to the REST probe. If dxLink never loads, never connects, or goes quiet, the tracker behaves exactly as it did before this file existed. Spot is still REST-sourced (only the contract is subscribed), refreshed at least every 5 minutes so distance-to-CB cannot drift. Reconnects resubscribe every held symbol — a subscription that silently did not survive is the likeliest fault here — and `settle` tears the socket down at the bell after flushing the last partial bar.

The chart now draws the minute high/low as a shaded envelope behind the line, which is the visible payoff: the line is one price per minute, and the peak can live entirely inside the band. REST-probed rows have high = low = mark, so the band collapses on its own for anything the stream did not cover.

Also, per the owner: the Confidence and Contracts summary cards dropped their coloured `borderTop` accent and now use the bare frosted `CARD`, matching the ICT cards on the same page — the accent strip made them read as a different card family. `accent` still colours the stat text.

New `server-v2/cb-stream.selftest.js` (10 checks, stubbed client, no socket): connected-but-silent is not reported as working, a stale symbol falls back, the forming minute is withheld, a completed bar carries the intra-minute high a 60s poll would have missed, draining twice does not replay, a reconnect resubscribes, and `stop()` flushes rather than losing the close. Plus 47 offline, 27 integration against Postgres, 38 render. `tsc --noEmit` and `node --check` clean. Not prod-verified — worth watching `diagnose`'s new `stream` block on the first live session to confirm `fresh` tracks the held count.

## 2026-07-31 — CB contracts: hold to the bell, auto-sell removed (`server-v2/cb-contract-track.js`, `owner-vite/src/pages/Results.tsx`)

Owner's call: the position stays alive the whole session and the exit rule is gone. A checkpoint that opens a contract now re-prices it every minute until 16:00 and never sells early. The old rule closed on the first poll SPX came within 5-10 pts of the CB, which capped the record at the moment the CB was reached and hid everything the contract did afterwards — precisely the part worth studying when the CB is behaving as a magnet.

What replaces it is measurement, not a decision. `best_price`/`best_ts` and `worst_price`/`worst_ts` now carry the day's peak and trough **with the times they printed** — a $4.20 peak at 11:03 and the same peak at 15:58 are very different facts about the same session. `closest_dist` still records how near SPX ever got to the CB; it just triggers nothing. `sellCheck` became `distanceToCb` and the `CB_SELL_*` env vars are gone: as long as a sell-shaped export exists, something can start calling it again, so the selftest asserts `t.sellCheck === undefined`. The only thing that closes a row in a normal session is the 16:00 settle, and that is the session ending rather than an exit signal — 0DTE, no next session. `signal_ts`/`signal_dist` stay as dead columns so existing rows keep their history; nothing writes them.

Summary stats had to change with it. `sellHits` was meaningless once nothing sells, replaced by **`peakedUp`** (how often the contract ever printed above what was paid) and **`avgPeakGain`** — the honest version of "the move was there", without pretending a rule captured it. The table's **Sell** column became **Peak**, showing the high-water mark and the clock time it hit; the popup gained Peak and Close stats and the chart marks the peak instead of an exit, with the 5-10 pt band removed from the distance view.

Two bugs the render harness caught, both created by this change and both the same shape — a number that had been safe to infer stopped being safe. The server now writes `pnl` on OPEN rows (mark-to-market), so the page's `pnl ?? live` test stopped starring anything and every live mark rendered as booked; the unrealized flag is now `status === "open"`. Same root cause in the footer totals, where `pnl != null` had selected settled rows and now caught held ones too — folding live marks into the booked win rate and the net dollar figure. Both now key off `status`.

Cost note: positions no longer exit early, so a full day is up to ~375 polls per open position (3 positions ≈ 1,100 probes/session, versus a few hundred before). That is the same order as the existing watchlist recorder and well inside the Theta/TT governor, but it is a real increase.

47 offline checks, 27 integration against real Postgres (new: a poll INSIDE the old band must not close anything, a position that keeps tracking after the CB is reached and left behind, and the bell closing both held positions together), 38 render checks. `tsc --noEmit` and `node --check` clean.

## 2026-07-31 — Contracts tab: card typography back onto the dashboard scale (`owner-vite/src/pages/Results.tsx`)

The Contracts roll-up cards had invented their own type scale — a 20px headline win rate and 13px labels — next to the ICT and Confidence cards, which sit at the page's actual scale of **17 for the card title, 14 for everything else**, `HOME_THEME.text` for text, and `var(--font-mono)` on numbers only. One card shouting at a different size reads as a different app bolted on.

Collapsed every size I had introduced (11/12/13/15/16/18/20) back onto 14/17 across the whole Contracts tab — cards, table headers, buttons, the probe popup, the diagnosis dump — leaving only the chart's SVG axis labels at 11, which is `ProbeChart`'s own scale on `/owner/probe` and is what this chart is a copy of. Grepping the pristine page confirmed the baseline: every 12/13 in it came from the earlier contract work, never from the dashboard itself.

The render harness now asserts it (35 checks, up from 33): every rendered inline `font-size` on the tab must be 14px, 17px or `inherit`, and no 18/20 anywhere. Eyeballing is what let it drift in the first place.

## 2026-07-31 — CB contracts: buy the closest strike to the money OVER $1.00, not the CB strike under it (`server-v2/cb-contract-track.js`, `server-v2/api-router.js`, `owner-vite/src/pages/Results.tsx`)

Rule change from the owner — and it inverts the filter. The old rule bought the CB strike when it was **at or under** $1.00. The new one treats $1.00 as a **floor**: price the CB strike, and if it is under a dollar, step one strike at a time TOWARD THE MONEY and buy the first one that prices **over** $1.00. Owner's example, now a test: CB 4500 with SPX below it and the 4500 call at $0.35 — try 4495, then 4490, then 4485, take the first over a dollar. The reasoning is sound: a sub-$1.00 0DTE contract is close to a lottery ticket, so SPX can travel 20 points toward the CB and the thing barely reprices — the sell rule then fires on a contract that never moved. Walking in until the premium clears a dollar buys something with enough delta to actually express the trade. **No upper bound** (owner's call): the walk stops at the FIRST qualifier, so it naturally lands just over $1.00 unless the CB is already near the money.

The walk stops AT the money and never crosses into ITM — "closest to the money", not "through it" — and it is bounded at `CB_WALK_MAX_STEPS` (24) probes per checkpoint, though a qualifier is normally 1–5 steps away. Strikes are not assumed: each step is priced through `/proxy/probe-rest`, which snaps to the nearest listing and reports `resolvedStrike`, so a step landing in a gap still prices a REAL contract and a 25-wide far-out chain just resolves several steps onto the same strike (deduped) instead of inventing three that don't exist. `qualifies()` is strictly greater than the floor — "over 1.0" means over.

This forced a split the schema didn't have: `cb_strike` (the target) is now stored separately from `strike` (the instrument actually bought), plus `cb_price` (what the CB itself cost — the reason the walk happened) and `walk_steps`. **Every distance — the sell trigger, `closest_dist`, `probe_dist` — measures to the CB, never to the traded strike.** The CB is the thesis; the strike is just how it's expressed, and collapsing the two would have quietly re-pointed the 5–10 pt sell band at whatever contract the walk happened to land on.

A skipped row now says how far it walked and what the best price it saw was — `walked 8 strikes from the CB to 6615 — none priced over $1.00 (best $0.70)` — which is a different failure from "the probe is down" and has to read differently. `cb_price` stays null when the CB strike itself couldn't be priced rather than reporting the walked-to price as the CB's, since that would be a quiet lie about why it walked. `diagnose` now runs the REAL walk and returns the full trail (every strike tried, what each priced at), so "why did it skip" is answerable from the Diagnose button.

UI: the Contracts table shows the traded strike with `←CB 6650` beside it whenever the walk moved (omitted when the CB itself cleared the floor), and the popup gained **CB target** (strike @ its own price) and **Walk** (`3 strikes toward the money`) stats. Showing only the traded strike is how you end up reading a 6635C as if the board had said the CB was 6635.

47 offline checks (up from 32 — the walk's ordering, direction per side, the stop-at-the-money guard on both sides, the strict floor, duplicate-resolution dedupe, and the owner's 4500 example end to end through a stubbed chain) and 26 integration checks against real Postgres (the walk writing `cb_strike`/`cb_price`/`walk_steps`, distance measured to the CB not the strike, a whole-chain-too-cheap skip, the put side walking up, and the null-`cb_price` case), plus 33 render checks. `tsc --noEmit` and `node --check` clean.

## 2026-07-31 — CB contract tracker: made the failure visible (`server-v2/cb-contract-track.js`, `server-v2/api-router.js`, `owner-vite/src/pages/Results.tsx`)

Follow-up to the tracker below, from the first live session: a row showed "0 polls recorded — not enough for a chart" and its contract never moved. The tracker was reporting three completely different situations with one misleading sentence, and swallowing the only fact that would have explained any of them.

`pollOpen` did `if (!p.found) continue;` on a probe miss. That is how a dead probe becomes an *invisible* one — the row simply stops moving, `polls` stays where it was, and nothing anywhere says why. Now a failed poll stamps a new `last_error` column with the reason and the attempt time (cleared on the next poll that prices), and `pollOpen` returns an `errors` count. `probeContract`'s reasons went from `probe miss` to the actual answer — `probe no-expiry — chain has 2026-08-01, 2026-08-03` — because the difference between "wrong expiry" and "chain is empty" is a five-second fix versus an afternoon of guessing. It also now refuses a strike ≤ 100 outright: `probeRestTT` snaps to the nearest strike in the chain, so a CB of 0 would have returned a confident price for the cheapest contract on the board.

Also fixed a real leak the live run exposed: the 16:00 settle only fires if the process is alive between 16:00 and the recorder's 16:10 window close. Miss that slot — a deploy, a restart — and yesterday's position stayed `open` forever, because `pollOpen` and `settle` are both scoped to today's date and nothing would ever touch it again. New `settleStale` runs at the top of every tick and marks out any `open` row from a prior session at its last known mark with `exit_reason='stale'`.

New `POST {action:'diagnose'}` / `GET ?diag=1` answers "why is nothing updating?" in one call: whether the recorder has ticked in this process at all (and how long ago), what CB each checkpoint resolves to out of `mvc_snapshots`, a LIVE probe of the current one with its raw status, and per-row tick counts. Surfaced on the Contracts tab as **Run now** (forces one idempotent tick and reports `opened / priced / probe errors`) and **Diagnose** (dumps the JSON in a popup). The popup now branches by status instead of one sentence about polls: a `skipped` row gets "Not taken" plus the reason and the price it was probed at — never a chart, since it was never a position — a row whose last poll failed gets an amber banner naming the error, and only a real position with too few ticks gets the polls message.

25 integration checks against a real Postgres (up from 17 — new coverage for the error stamp and its recovery, the nonsense-strike refusal, `settleStale` including that it leaves today alone, and all three diagnose paths) and 30 jsdom render checks (up from 23 — the skipped-row explanation, the failed-poll banner, Run now, Diagnose). `tsc --noEmit` and `node --check` clean.

## 2026-07-25 — New /options dashboard page + toolbar tile swap + Vite SPA route (`app/options/page.tsx` NEW, `components/shared/GlobalToolbar.tsx`, `app-vite/src/App.tsx`)

New `app/options/page.tsx` is a `"use client"` placeholder scaffold on `PageShell` + `Card` (`variant="budget"`, `accent={LIGHT_BLUE}`) — an intro card plus a responsive auto-fit grid of three dashed "coming soon" slots (Chain / Filters / Summary), with zero raw hex/rgba (every color from `HOME_THEME`). Replaced the dead `{ href: "/lookup", …, comingSoon: true }` entry in `GlobalToolbar`'s `NAV_ITEMS` with a live `{ href: "/options", label: "Options", emoji: "🎛️" }` (no `comingSoon` → navigates and drag-reorders; saved `cb-toolbar-nav-order-v1` arrangements drop the item to default position once since that order keys on `href`), and registered the page in the customer SPA — `app-vite/src/App.tsx` gained `const Options = lazy(() => import('@/app/options/page'))` + `<Route path="/options" element={S(<Options />)} />`, unprefixed since `BrowserRouter basename="/app"` supplies `/app`; without step 2 the page renders on Next at `/options` but `/app/options` silently falls through the catch-all to `/traders-dashboard`. Not build-verified; needs deploy. The deploy attempted this session failed in an UNRELATED target: `owner-vite`'s `src/pages/ControlPanel.tsx` imports `@bklitui/ui/charts`, absent from `owner-vite/package.json` and unaliased in its `vite.config.js` (only `@` → `./src`), so Rollup can't resolve it in a clean Docker build even though local `node_modules` has it — see `owner-vite/charts-ui-patch.tar.gz` + `components.json`, both touched at the same time as `package.json`.

## 2026-07-25 — Free /explore pages showed baked-in "populates at end of day" empty states — forced request-time render + per-fetcher cache (`app/explore/[slug]/page.tsx`, `components/explore/DelayedLiveView.tsx`)

`/explore/[slug]` was prerendered at build time (`generateStaticParams` + `export const revalidate = 900`), but the Docker build stage never sees `.env.local` (secrets are mounted at runtime via compose `env_file`), so `DATABASE_URL` is undefined during `npm run build`: every delayed-live query threw, hit the graceful-empty branch, and the "Graded IB results populate here at the end of each trading day." / "Completed Market Profiles populate here…" / GEX / EM empty states were baked into the prerendered HTML and served to visitors permanently (verified live on `/explore/initial-balance`, `/explore/gex`, `/explore/tpo` — the numbers still visible there come from the static "Preview · sample data" teaser, not `DelayedLiveView`). `Confidence7dTracker` on `/explore/confidence-score` was empty for the same reason. Replaced `revalidate`/`generateStaticParams` with `export const dynamic = "force-dynamic"` so the blocks render where the DB actually exists, and wrapped all four `DelayedLiveView` fetchers (`gexSnapshot`, `emStats`, `ibRows`, `tpoRow`) in `unstable_cache(..., { revalidate: 900, tags: ["explore-delayed"] })` so the DB sees one query per 15-min window per block, not one per visitor. Symbol keys confirmed correct against the recorders (`ib_daily_results` = `'ES'` via `server-v2/ib-results-recorder.js`, `tpo_profiles` = `'ESU'` via `server-v2/tpo-profiles-recorder.js`). Not build-verified; needs deploy. If IB/TPO stay empty after redeploy, the tables themselves are unpopulated — check recorder logs.

## 2026-07-24 — Mult-greek Δ15/30/open card 401 fix + font size match to Options Chain (`server-v2/mult-greek-gex-recorder.js`, `server-v2/mult-greek-snapshot-recorder.js`, `app/mult-greek/MultGreekClient.tsx`)

Diagnosed the click popout's Δ 15/30/Open cells stuck on "building…"/"—": `PROXY_AUTH_REQUIRED=1` gates every `/proxy/*` route, but `mult-greek-gex-recorder.js`'s `fetchJson()` and `mult-greek-snapshot-recorder.js`'s `fetchJson()` called the local `/proxy/api/tt/...` routes with no `x-internal-token` header, so the recorders 401'd themselves before ever reaching TastyTrade — `mult_greek_gex_ring`/`mult_greek_gex_open` never got written (confirmed live via `[mult-greek-snapshot] expiry resolution failed — skip (... 401)` in prod logs; TT refresh-token/client creds independently verified valid via manual `/oauth/token` call, so not an auth-provider issue). Fixed both `fetchJson()`s to send `x-internal-token: process.env.INTERNAL_API_TOKEN` (snapshot recorder already had an `internalHeaders()` helper it just wasn't using). Also set the ladder-cell `fontSize` in `MultGreekClient.tsx` to `10` to match `app/options-chain/page.tsx`'s cell size (was 12, briefly 8). Not yet deployed to prod.

## 2026-07-24 — Multi-Greek page overhaul: per-ticker 4-closest-expiry NET GEX columns, CB/CW/PW header walls, combined click popout + backend 15m/30m/open GEX-change recorder (`app/mult-greek/MultGreekClient.tsx`, `server-v2/mult-greek-gex-recorder.js` NEW, `server-v2/server-with-proxy.js`, `app/api/mult-greek-gex-change/route.ts` NEW)

Rebuilt `MultGreekClient.tsx`: dropped the DEX/CHEX/VEX columns for each ticker's OWN 4 closest expirations of NET GEX (per-ticker calendar via `expByTicker`/`pickCols` — fixes TSLA showing a blank SPX-Thursday column; `loadAll` fetches `range=all` then gap-fills each ticker's missing column expiries with explicit `?expiration=` calls to bypass the proxy's SPX-live-1-expiry / REST-3-expiry caps), added CB (max |GEX|) / CW (top +GEX) / PW (most −GEX) wall chips in each panel header (always shown) plus toggle-gated ladder badges, and merged the hover card into one click popout (book stats + NET GEX 15m/30m/open change) that portals to `document.body` and closes on any off-card click. New `server-v2/mult-greek-gex-recorder.js` (server-v2 root, not gitignored `state/`) snapshots per-strike NET GEX for SPX/SPY/QQQ/IWM × 4 expiries every 60s RTH into `mult_greek_gex_ring`/`mult_greek_gex_open`, served via new `/proxy/mult-greek-gex-change` + `app/api/mult-greek-gex-change/route.ts`; the card diffs its live value against these (client-ring fallback for non-roster tickers). `node --check`/esbuild clean; NOT prod-build/live verified; needs deploy (new recorder + auto-created tables).
...
`LiveToday` now renders only Live Read (`LiveGauges`), IB Read · 4 families (`RuleClusterBoard`), and the Probability Engine — dropping `IbLevelCanvas`, `RuleBoard`, the "Today" levels card, the owner historical-stats block + `IbDailyResults`; added `showLive`/`showStages` props to `IbProbabilityEngine` (passed `false`) so it shows only the 10:30-locked gauges. Fixed the stale LAST 5 SESSIONS tape in `RuleClusterBoard` by sourcing it from the live `/api/ib-results?symbol=&limit=5` feed (reversed to oldest→newest) instead of the static `ds.days` export, with the export as fallback. esbuild parse clean; not full prod-build verified.

## 2026-07-23 — Cards: removed the light-blue radial highlight from card/tile surfaces app-wide → solid frosted fills (matches squeeze `cardBase`)

Flattened the "grayish" radial highlight on card backgrounds across every page. Source of truth: `components/shared/homeTheme.ts` (`classicCardAccentStyle` and `statTileStyle` now use flat `HOME_THEME.panelBg` instead of `radial-gradient(circle at 50% 0%, rgba(126,211,252,…), …)`) and `components/shared/ownerTheme.ts` (`homePanelStyle` → `OWNER_THEME.panelBg`) — this covers all shared `Card`/tile call sites. Also stripped the same hardcoded radial highlight (keeping each card's existing solid fill) from inline card surfaces in `app/home/HomeClient.tsx`, `app/docs/page.tsx`, `app/market-matrix/page.tsx`, `app/scanner/page.tsx`, `app/gex2/page.tsx`, `app/es-candles/page.tsx`, and shared components `components/dashboard/SnapshotPanel.tsx`, `ScannerHomePanel.tsx`, `EmCustomer.tsx`, `ChainStatsBar.tsx`. Left intact deliberately: the cyan toolbar/dock glow (`homePanelStyle`/`homeGlossPanelStyle`/`DOCK_THEME` + flow/quote-pill menu panels) and per-card colored top-border strips. Not build-verified.

## 2026-07-21 — /test "Condition" tab: SPY/QQQ market-condition cards (WAVE + 0-DTE GEX) + dxLink 1-min price line + SPY/QQQ candle recorder; full-site 502 outage fix (`components/dashboard/GexRegimeCard.tsx`, `app/test/page.tsx`, `server-v2/candle-history.js`, `server-v2/etf-candle-recorder.js`, `server-v2/server-with-proxy.js`)

New `components/dashboard/GexRegimeCard.tsx` recreates the SPX regime tile for SPY/QQQ — WAVE = cumulative net call−put premium from `/proxy/flow-history`, GEX Structure = per-strike net gamma `(|cΓ|·cOI−|pΓ|·pOI)·spot²·0.01·100` from `/api/chains` (green=support / red=magnet), plus a condition header (label/stars/bullets) derived from WAVE dominance + gamma-path cleanliness — mounted as a new "Condition" tab in `app/test/page.tsx`. Price line uses real per-symbol dxLink 1-minute candles (the flow tape's `spot` is the SPX index on every underlying, so it plotted SPX on both cards): new isolated collector `server-v2/candle-history.js` (`fetchIntradayCandles`, short-lived dxLink connection reusing exported `DxLinkClient`/`getQuoteToken`) exposed at `GET /proxy/candles-intraday?symbol=&interval=1m&fromMs=` (today-only, client pins `fromMs` to ET midnight); `server-v2/etf-candle-recorder.js` persists SPY/QQQ 1m bars into a new `etf_candles` table every 60s during RTH.

OUTAGE FIX: the recorder first lived at `server-v2/state/etf-candle-recorder.js`, but `.gitignore`'s trailing `state/` rule silently untracks anything under `server-v2/state/`, so the new file never pushed → missing from the image → the hard `require('./state/etf-candle-recorder')` threw `MODULE_NOT_FOUND` on boot → container crash-loop → Cloudflare 502 across the whole site. Fixed by moving the recorder to `server-v2/` root (tracked, like `candle-history.js`) and wrapping both new requires in `server-with-proxy.js` in `try/catch` so an optional feature module can never take the origin down again. tsc `--noEmit` clean on the component, `node --check` clean on all server files; not full prod-build verified.
...
`route.ts`: 30s in-memory TTL cache on the heatmap branch + window cap 2880-min (matches retention). `lib/db.ts`: 2-day prune on `option_strike_gex_history` insert; `EodGexRecord.total_gex_ex0dte`. `server-v2/eod-gex-recorder.js`: new nullable `total_gex_ex0dte` column = combined net GEX across all expirations EXCEPT 0DTE, computed live off the TT chain (concurrency-capped, once/day) for $SPX/SPY/QQQ plus the historical AM/catch-up path; upsert `COALESCE`s so a null never wipes a good value.

## 2026-07-20 — IB Stat Prompter: width→break-odds + 8am/2pm candle stats + streak reframe + widthBucket backfill; IB hover trimmed to today-only (`components/scanner/StatPrompterTab.tsx`, `components/scanner/IbStatsTab.tsx`, `scripts/build-bar-stats.mjs`, `app/scanner/ib-embed/page.tsx`, `app/es-candles/page.tsx`)

`StatPrompterTab.tsx`: new `width-break-odds` (Session shape) partitions ES/NQ sessions by IB width bucket into one-side / both / neither with a stacked bar + observed point ranges; two new Bar-stats prompts `cand-8am-both` (08:00–09:00 ET reference hour → both/one/neither + cumulative both-completion timing) and `cand-2pm-sides` (14:00–15:00 ET → four-way partition into the close); reframed the Streaks view to explicit `N in a row → N+1` continuation with a cumulative "reach from fresh" survival column; added `backfillWidthBuckets()` on load that derives the null `widthBucket` from each day's `atr`/`avgIB` (same thresholds as `lib/ibStats.ts`) so every width prompt now populates off the existing `ib-*.json` export (was empty — exporter shipped bucket null). `scripts/build-bar-stats.mjs`: emits `refCandles.{am8,pm2}` aggregates (both/one/neither counts + 30-min both-completion histogram + median/p75/first-break clock) and relaxed the all-hours grid sanity check to warn-not-fatal on the daily 17:00–18:00 ET futures maintenance-halt shortfall while still failing on fragmentation/off-grid (was refusing to write ES ETH). `IbStatsTab.tsx` RuleBoard: dropped the NOT-IN-PLAY section + chip helper so the live board shows only PENDING (forming) + IN PLAY; added an `embedToday` prop that forces `isOwner` off, collapsing the tab to `symTabs` + `LiveToday`. New bare route `app/scanner/ib-embed/page.tsx` renders `<IbStatsTab embedToday />`; the ES-candles IB dock hover now iframes `/scanner/ib-embed?embed=1` (LayoutShell strips global chrome on `embed=1`) instead of the full `/scanner?tab=ibstats`, so the preview shows only today's IB section — "Open ↗" still links the full board. All files transpile-verified via esbuild; not full prod-build verified.

## 2026-07-19 — /obook Order Book "Tenor Split" page + live-wired API + /test tab (`app/obook/page.tsx`, `components/shared/ObookView.tsx`, `app/api/obook/route.ts`, `app/test/page.tsx`)

New `/obook` route renders an Order-Book tenor-split read (combined C/P premium, delta-weighted flow, front-month vs intermediate-term tenor cards, and per-expiry net-directional bars) from a shared `components/shared/ObookView.tsx`; `app/obook/page.tsx` wraps it in `PageShell`, and it's also mounted as an "Order Book" tab in `app/test/page.tsx` (new `TestTab` value + tab-bar entry + Overview card + render branch). `app/api/obook/route.ts` computes the whole `ObookData` shape from the live `/proxy/flow-history?underlying=X` tape (`FlowOrder[]` — buy/sell aggressor, OTM flag, per-print `spot`) via `forwardGet`, split front vs intermediate; the first pass wrongly queried the unused `flow_calls` table (always 404'd) — repointed to the same source the Flow Inventory tab uses. Only recorded roots (SPX/SPY/QQQ) have a tape (else 404 → QQQ sample); delta-flow is a moneyness estimate (no per-print greeks); metric tiles use `statTileStyle` and tenor panels use `classicCardStyle` (dropped the theme-removed top-accent strip). Typechecked in isolation via `tsc` stubs; not full-build/prod-verified.

## 2026-07-19 — Scanner TPO tab: white fonts, title colors, marked-up Signals & Alerts, profile reorder (`app/scanner/page.tsx`)

Restyled the TPO tab (`app/scanner/page.tsx`, `TpoStructuresScanner`/`AmtPanel`/`AmtSignalRow`/`TpoLetterProfile`): ~60 muted `rgba(255,255,255,0.xx)` and `NEUTRAL` font colors (subtitles, tile labels, table headers, legends, hover tooltip, canvas date/axis labels, info-level badge, flat glyph) → `HOME_THEME.text`; borders/backgrounds untouched. Card titles recolored (AMT + Structure stats cyan, TPO profile LIGHT_BLUE, Open business orange, Today's profile green). Rebuilt the "Signals & Alerts" header into an accent-bar block with a live/armed count pill (green when `liveCount>0`, else orange). Reordered the render so the TPO profile Card sits above the `<AmtPanel>` (signals) instead of below. Not build-verified.

## 2026-07-18 — Customer pages → Vite SPA: /app (traders-dashboard, analytics) live-wired (`app-vite/`, `app/app/*`, `lib/serveSpaShell.ts`, `public/app/`)

New `app-vite/` Vite+React+TS SPA compiles the EXISTING Next client pages as-is via an `@`→repo-root alias plus `next/link|navigation|dynamic` shims (`app-vite/src/shims/`), mounting `app/traders-dashboard/page.tsx` and `app/analytics/page.tsx` in one React Router shell (`app-vite/src/App.tsx`); served at `/app/*` by per-page route handlers (`app/app/{route,traders-dashboard,analytics}/route.ts` → `lib/serveSpaShell.ts` reading `public/app/index.html`), with the built bundle committed to `public/app/`. Verified by a real `vite build` (46 modules, zero page edits); `app-vite/MIGRATION.md` documents the page-by-page port process; pages mount bare (no global nav yet) and it is unverified in prod. Backend (API/`/proxy`/`/ws`/`server.js`/`middleware.ts`) untouched; owner pages handled separately on the owner subdomain; only `/home` + `/home-fast` need their SSR `/proxy/gex` seed swapped for a client `/api/home-snapshot` fetch before they can move.
...
## 2026-07-27 — New standalone `options-vite` placeholder app (`options-vite/` NEW, at repo root next to `home3-vite`)

Scaffolded `options-vite/` (Vite 5 + React 18 + TS + react-router-dom, dev port 5183) as an unwired layout shell for the Options dashboard: `src/pages/PageView.tsx` owns the grid (toolbar / ticker selector + daily-yearly `HeatmapPanel` on the left, `SunburstPanel` + page-supplied side/feed slots on the right; heatmap + sunburst render on every route), `src/TickerContext.tsx` holds the one global symbol every card reads via `useTicker()`, `src/components/TickerSelect.tsx` is the main dropdown with a nested top-left Favorites/Watchlist list picker + search (`src/data/tickers.ts`), and `src/pages/registry.tsx` maps routes (`/`, `/chain`, `/flow`, `/greeks`) to their three slot components and auto-populates the toolbar nav. All panel bodies are `src/components/Placeholder.tsx` skeletons — no fetches, sockets, or chart libs. Palette in `src/theme.ts` is ported from `home3-vite/src/dashboard/theme.ts`. `tsc --noEmit && vite build` clean; verified in a headless Chromium run (ticker switch propagates to all six panels, route change preserves heatmap + sunburst).

## 2026-07-27 — ES Candles: "Flip X" comet overlay + bubble-panel persistence / Save default (`app/es-candles/page.tsx`)

New `showFlipCross` overlay ("Flip X" in the Overlays dropdown) on `app/es-candles/page.tsx` draws the gamma flip across the session. The per-minute flip is derived from `minuteColsRef` — the same 1-min GEX columns that feed the bubbles — using the app's canonical rule (`findGEXFlip`): per-strike net-GEX SIGN crossing, linearly interpolated between bracketing strikes, picking the crossing nearest that column's stored SPX spot, then SPX→ES via `basisAt()`. The first implementation used a CUMULATIVE-sum zero crossing and printed 45–125 pts above the real flip (confirmed against the header FLIP readout: 7470+ drawn vs 7423.85 actual) — that quantity is "equal gamma above and below", not the flip, and it is window-dependent because `option_strike_gex_history` only stores a band of strikes around spot; replaced with the sign-crossing method, which is local and window-immune. NOT a basis bug — the bubbles use the same `basisAt()` and land correctly. Rendered as a comet: constant 1.3px stroke (deliberately no taper — only recency is being encoded, and alpha carries it), per-segment alpha ramp 0.10→0.88, glowing head + halo at the newest on-screen reading, plus ONE `▲/▼ INTO ±GEX <level>` chip for the most recent close-vs-flip crossing. Per-cross rings / center dots / direction arrows were built and then removed: on a chop day the flip is crossed 10+ times and the marks buried the candles and bubbles over old bars that carry no tradeable read. Backfills for the whole session at no extra cost — `minuteColsRef` is already populated from `/api/snapshots/option-strike-gex-history` on load (48h server-side retention, not RTH-gated, so it covers ETH too). Bubble panel persistence: `bubbleMins` (1m/5m bucket) and `showGexBubbles` now persist into the same `es-candles-bubble-cfg-v1` blob through new read-modify-write helpers `readBubbleBlob()` / `writeBubbleBlob()` — the old slider write did a bare `setItem(JSON.stringify(next))` and would have clobbered any key it didn't know about. Added a second, deliberate store `es-candles-bubble-default-v1` with **Save default** / **Reset** buttons and a "saved ✓" flash: only Save default writes it, Reset restores it (falling back to factory values when unset), and the on/off toggle is intentionally excluded from a pinned default since it's working state. Diagnosed but NOT fixed this session: no MVC/CB step line overnight or in ETH — `server-v2/mvc-auto-snapshot.js` gates every write behind `isRTH()` (Mon–Fri 09:30–16:00 ET) so `mvc_snapshots` has zero rows outside RTH, and separately the client's step-line `flush(prevX)` ends the last run at the final snapshot's x rather than extending to the right edge, so even the last known RTH level doesn't carry forward. esbuild-clean; not prod-build verified.

## Tuesday 7/28/2026 — components/shared/ChainReplay.tsx
Smoothed the replay spot-price readout: added an eased tween (`animSpot` ref + rAF, capped 450ms, only active while `playing`) so the spot value glides between recorded frames instead of snapping every 6-8 events; scrubbing the range input snaps instantly (no tween) to avoid overlapping animations racing toward different targets. Replaced the per-row injected dashed spot line with a single absolutely-positioned overlay whose `top` is measured live off the bracketing strike rows via `getBoundingClientRect()` (through new `rowsContainerRef`/`rowRefs`) rather than a guessed `ROW_H` pixel constant, fixing cumulative drift that had the line landing several strikes off from the actual spot value.

## Wednesday 7/29/2026 — Options Chain OI tab + day-over-day ΔOI recorder (`server-v2/oi-daily-recorder.js` NEW, `server-v2/server-with-proxy.js`, `app/options-chain/page.tsx`)

New **OI** mode beside GEX/DEX/CHEX/VEX on `app/options-chain/page.tsx`. Open interest is settled overnight by the OCC and republished pre-open — it does NOT tick intraday — so a "change since 9:30" is flat by construction; the real signal is today's settled OI vs the PREVIOUS trading day's. New `server-v2/oi-daily-recorder.js` therefore takes ONE snapshot per trading day at 9:32 ET (`OI_DAILY_RUN_AT_MIN`, holiday-gated, minute-poll scheduler so a restart backfills) across `SCANNER_TICKERS` × `OI_DAILY_EXPIRY_DEPTH` (6) expiries, trickled at `OI_DAILY_DELAY_MS` (250ms), upserting into a self-created `oi_daily(date,symbol,expiry,strike,call_oi,put_oi,spot,ts)` with 45d retention. `getOiChange()` diffs the two most recent snapshot DATES that exist for the symbol (not calendar today/yesterday — survives holidays and missed runs) via a FULL JOIN, so a strike fully closed out overnight (no `cur` row, since zero/zero strikes aren't written) still surfaces its unwind instead of being dropped. Exposed as `GET /proxy/oi-change?symbol=&expiries=` + `POST /proxy/oi-daily-run` (upserts ⇒ safe to re-fire); `/proxy/*` is a blanket next.config rewrite to the origin, so no allowlist change. Client: `GreekCell` gained `oi` (net, always settled book — ignores the Vol-Only toggle), `oiSides()`/`oiSideValue()` implement calls-above-ATM / puts-below-ATM (both at the pivot row) and drive cell render AND heat scale/column totals/⅀ Total together, `fmtCount`/`fmtChg` replace `fmtMoney` in OI mode (contracts, not dollars), and the ΔOI snapshot is keyed by symbol in state + gated through `oiSnapshot` so a ticker switch can't paint the previous symbol's OI under the new chain. Δ15/30/60 columns and the pin glyph are suppressed in OI mode. Review pass fixed 7 defects pre-ship, notably: a manual single-symbol `/proxy/oi-daily-run` claiming `_lastRunDate` and silently cancelling that morning's full sweep; `runSweep` returning `ok:true` after writing zero rows (dead upstream ⇒ day lost with no retry, since all per-ticker errors are swallowed); and a check-before-await `_running` guard allowing concurrent sweeps. **Also fixed a pre-existing layout bug affecting every greek tab:** the chain scroll container's `padding: "8px 10px 10px"` meant its `position:sticky; top:0` header stuck to the CONTENT edge, leaving an 8px band where rows scrolled visibly ABOVE the header — verified live on prod via CDP (container top y=151, header stuck y=159, gap 8px; after moving the spacing to `marginTop:8` on the grid, gap 0). Not deployed — VPS `git pull` is blocked by an uncommitted `/proxy/strike-growth/gex-pct` endpoint that exists only on prod and must be recovered into main first.

## Wednesday 7/29/2026 — Sector wheel: S&P 500 hub label + on-wheel winner/loser callouts, and a clipboard Snapshot button (`components/dashboard/SectorSunburst.tsx`, `components/shared/CopySnapButton.tsx` NEW, `app/traders-dashboard/page.tsx`)

`SectorSunburst.tsx`'s hub now prints `S&P 500` above the net % (the sector's shortest form — `TECH`, `FINS` — when zoomed in), and the biggest movers of whatever is on screen are named on the rim with a tick line back to their bar (top/bottom 3 in the card, 5 each popped out; bars pulled in from `R*0.40`/`R0=R*0.56` to `R*0.33`/`R0=R*0.54` to open the label band, placement greedy by |move| so a colliding smaller mover is skipped rather than overprinted, and a called-out bar no longer also prints its ticker inside itself). New `components/shared/CopySnapButton.tsx` renders a ref'd subtree to PNG via a dynamically-imported `html2canvas` (already a dep) and writes it to the clipboard with a download fallback, wired into the `app/traders-dashboard/page.tsx` header over a new capture wrapper — the gradient `<h1>` carries `data-snap-plain` because `background-clip:text` rasterizes invisible, and `onclone` flattens it to solid cyan for the capture only. Also fixed two pre-existing defects: the wheel's stacked layout had `flexWrap:"wrap"` on a **column** flex container, so past ~430px card width (roughly the live right column at `maxWidth={1200}`) the Top/Bottom block wrapped into a second column on top of the wheel — wrapping is now enabled only in the popped-out row layout; and `savePrefs`'s patch type omitted `links`, making `updLinks` a type error at the call site. `tsc --noEmit` clean; verified by headless Chromium render (card / zoomed / popped-out) and a clipboard round-trip. Not deployed.

## 2026-07-30 — Strike History: on-chart hover readout (`app/strike-history/page.tsx`)

`Panel`'s hover branch now draws two chips alongside the existing crosshair + dot: a value tag pinned to the dot (panel-colored, `HOME_THEME.bg` fill, flips left of the cursor when `hx + 8 + vW` would cross `VB_W - M.r` so it can't collide with the last-value label) and a `hh:mm` ET tag on the x axis, clamped to `[M.l, VB_W - M.r - tW]` and painted after the hour gridline labels so it covers them. Chip widths are estimated at 5.9px/char since SVG has no pre-paint text metrics. Added an optional `hoverFmt` prop (defaults to `fmt`) so the tag can outrun the axis ticks — spot `toFixed(2)`, net GEX / vol GEX `fmtM(v, 1)`, skew `fmtVp(v, 2)`. The synced four-panel readout under the stat tiles is unchanged. esbuild parse clean; not deployed.

## Saturday 8/1/2026 — Test Lab "GEX Map" tab: four unified GEX/DEX readouts + per-strike DEX persistence (`app/test/GexMapTab.tsx` NEW, `app/test/page.tsx`, `server-v2/api-router.js`, `server-v2/gex-history-writer.js`, `server-v2/scripts/dump-strike-gex-window.js` NEW)

New `app/test/GexMapTab.tsx` fuses the GEX profile, strike×time heatmap, strike rail, spot-riding GEX bubbles and DEX into four readouts (Tape Field / Polar Reticle / Spine / Gamma Terrain) shown 2×2 with click-to-expand, wheel+drag+pinch pan/zoom, a session and expiry chooser, and the `GexHeatmap`/options-chain palette (`#29b6f6` / `#ff4757`, GexChart's 28% magnitude lift, `ratio**1.4` alpha easing), all fed by one new `/api/gex-map` route in `server-v2/api-router.js` that ships a slot-aligned ladder + spot + per-column flip and refuses to blend expiries; `server-v2/gex-history-writer.js` now persists `net_dex`/`net_vol_dex` in the same row as gamma (values `computeGexSummary` always emitted but nobody stored), with `greek_snapshots` kept as the fallback for older sessions. Also added `server-v2/scripts/dump-strike-gex-window.js` — a schema-introspected CSV rescue for `option_strike_gex_history` before its unscoped 48h `DELETE` erases a session, since nothing on disk holds a per-strike ladder past 2026-07-20. Verified against a real Postgres with the live DDL across four schema states plus `tsc --noEmit` and headless render/interaction tests; not deployed.

## Sunday 8/2/2026 — Toolbar: section sub-strip, Bzila alerts merged into the CB Edge logo, dead account-menu links (`components/shared/SectionSubStrip.tsx` NEW, `components/shared/sectionNav.ts` NEW, `components/scanner/scannerNav.ts` NEW, `components/shared/GlobalToolbar.tsx`, `components/shared/BzilaAlerts.tsx`, `components/shared/UserMenu.tsx`, `components/pages/Scanner.tsx`, `components/pages/TestLab.tsx`, `components/pages/StrikeHistory.tsx`, `components/scanner/ScannerTabsBar.tsx`)

Promoted the Scanner and Test Lab tab bars into the app chrome as a single non-wrapping row welded to the bottom of the `GlobalToolbar` pill. Design was picked from four rendered mockups (split-caret dropdown / hover mega-panel / in-flow sub-strip / searchable jump-list) then a second round crossing the mega-panel's grouping with the sub-strip's tab-pill styling; the shipped variant is the one-row strip with hairline cluster dividers and short labels. New `components/scanner/scannerNav.ts` and `components/shared/sectionNav.ts` are plain data modules (no "use client", no React, no next/*) so the toolbar imports the tab lists without dragging page components or `next/navigation` into every bundle — `ScannerTabsBar.tsx` re-exports from `scannerNav` so existing import sites are untouched. `sectionNav` is a registry: each section declares `rootPath`, owned `paths`, `defaultTab`, tabs (with `short` + `icon`), clusters, and a window event; adding a third section is one array entry. `SectionSubStrip` renders whichever section matches `usePathname()` and unmounts everywhere else. Fitting is a measure-then-adjust pass writing `data-mini` straight onto the pill nodes (state would mean a render per shed pill and a measurement feedback loop): labels shed right-to-left to icon+tooltip, then the row scrolls horizontally rather than clipping. Tab clicks fire a per-section `CustomEvent` (`cb:scanner-tab` / `cb:testlab-tab`) because React Router does not remount a page for a query-string-only change — the URL would move and the visible tab would not. Removed the on-page `ScannerTabsBar` from `/scanner` and `/strike-history`, the whole Overview landing block from both pages (`ScanMeta`/`SCAN_META`/`ScannerOverview` ~115 lines; `OverviewCardDef`/`OVERVIEW_CARDS`/`OverviewCard`/`OverviewTab`/`FlaskIcon` ~215 lines) and their tab ids — the strip is on screen from every route in the section, so a landing page whose only job was linking to sibling tabs had nothing left to do; `/scanner` opens on GEX Scanner, `/test` on Squeeze, and `/test?tab=gexmap` deep-links now work (it had no URL support before). The strip renders on mobile too — it is those sections' only tab bar now, so hiding it would have made every non-default tab unreachable. Also folded `BzilaAlerts` into the CB Edge logo via a new `renderTrigger` prop (all polling / seen-tracking / panel logic unchanged; only the button markup is swappable): the logo glows orange with a pulsing Bzila-face badge when an alert lands, and `/feedback` — the logo's old job — moved into the panel footer and the account menu. Free/signed-out viewers still get the plain `/feedback` link, so `renderTrigger` is called with `canSee:false` rather than the component returning null. Removed the permanently-dimmed Order Flow coming-soon tile from `NAV_ITEMS` (still in the hamburger) and dropped `NAV_RESERVED_PX` 700→650 since the standalone bell's 40px is no longer reserved. Fixed three defects: `UserMenu`'s `INFO_LINKS` used `next/link`, so inside the Vite SPA (basename `/app`) Help & Docs / Disclaimer / Risk Disclosure / Terms / Privacy resolved to `/app/docs` etc., hit no route, and fell through the catch-all to Traders Dash — switched to native `<a>`, the same fix `/whats-new` already carried but that this block had missed; the strip's `overflow:hidden` (needed for the max-height transition) was shaving the active pill's 1px accent border and its 1px hover lift, fixed by padding 9/10→12/13, `STRIP_H` 58→76 and making the inner row `overflow:visible` until the fit pass genuinely needs `overflow-x:auto`; and the strip's `z-index:49` beat the `z-index:1` wrappers that every popover hanging out of the pill lives in (account menu, notes panel, NQU quotes dropdown), so it painted over all three — dropped to `z-index:0`, still above page content because the whole band is `z-index:50`. Separately fixed a pre-existing typo: the alert panel's "View reactions" link pointed at `owners.cbedge.net` (plural), which does not resolve — `DNS_PROBE_FINISHED_NXDOMAIN`; every other Owner link in the repo uses the singular host. `tsc --noEmit` clean against React 19 / Next 15 types. `components/shared/ScannerSubStrip.tsx` is now a one-line re-export marked deprecated, `ScannerTabsBar.tsx` is unreferenced, and `.scanner-overview-grid` / `.test-tab-select` / `.test-tabs` in `globals.css` are dead — all safe to delete. Not deployed.

## Monday 8/3/2026 — Vol GEX Flow: symbol scoping on `option_strike_gex_history` reads (`server-v2/server-with-proxy.js`, `gex-vol-flow-today.sql`)

The home page's **Vol GEX Flow** line drew a hard sawtooth (~-55B on a ~250B level, alternating almost every 1m bucket). Root cause: `option_strike_gex_history` is a SHARED table — `server-v2/etf-gex-recorder.js` writes SPY and QQQ 0DTE into it every 60s during RTH through the same `gex-history-writer.writeGexSnapshot()`, stamped with their own `symbol` but under the SAME `'YYYY-MM-DD'` expiry string SPX uses — and `handleGexVolFlow` filtered on timestamp/RTH/expiry only. So the aggregate summed SPX + SPY + QQQ, and every minute bucket the ETF writer's throttle skipped dropped that slice back out of the total, drawing market-looking movement that was pure composition noise. It also skewed the expiry picker's row counts and the front-expiry derivation, both of which are `COUNT(*)`-based.

Fix: `handleGexVolFlow` takes `?symbol=` (default `'$SPX'`, the writer's `DEFAULT_SYMBOL`) and threads it through **both** queries — the `expiries` GROUP BY and the main bucket aggregate. `symbol` also went into the `DISTINCT ON (bucket_ms, symbol, expiry, strike)` key and its `ORDER BY`: without it, two underlyings sharing a strike (SPY 630 / QQQ 630) silently overwrite each other and one snapshot's row vanishes from the sum. Added `symbol` to the 20s `_volFlowCache` key so an ETF request can't serve an SPX cache hit, and to the response payload.

`handleGexHistory` (`/proxy/gex-history`, the per-strike Δ baselines behind the heatmap's Vol GEX Speed column) had the identical hole — `WHERE date = $1 AND expiry = $2` and nothing else, so SPY/QQQ rows landed in the SPX baseline set on every 0DTE. Same `?symbol=` param, same default.

Both existing indexes already lead with `symbol` (`idx_osgh_symbol_ts (symbol, timestamp)` and `idx_osgh_symbol_lookup (symbol, date, expiry, strike, timestamp DESC)`), so the unfiltered queries could never use the leading column — these reads should get **faster**, not slower. `gex-vol-flow-today.sql` mirrors the scope across all four sections via a `:'sym'` psql variable defaulting to `$SPX` (`-v sym=SPY` to override).

**Diagnosed, NOT fixed:** the series itself is mislabeled. `netVolGEX` in `server-v2/computation/gex-calculator.js` is `callGamma*callVolume*spot^2 - putGamma*putVolume*spot^2`, and `callVolume`/`putVolume` are the chain snapshot's CUMULATIVE session volume — so `net_vol_gex` is a running day total revalued by current gamma, not a flow. Hence the monotone open-to-now ramp (9.33B at 09:32 to 304B at 10:23), Session Low always being the open, and "Sign Flips" reading 0 on a call-heavy 0DTE tape, which it structurally almost always will. The real flow is the `dVol` the endpoint already computes and the panel already shows as "Δ Last Bucket"; plotting THAT as the Baseline series is what `VolGexFlowPanel.tsx`'s own header comment ("net vol GEX is a POLARITY measure") describes. Left alone this session — it changes what the chart means, not whether it's correct.

`node --check` clean; diffed line-by-line, CRLF preserved. Not deployed.

## Monday 8/3/2026 (b) — Options Chain: VOL tab beside OI (`components/pages/OptionsChain.tsx`)

Added `"vol"` to `GREEK_MODES`, so the greek toggle now reads GEX / DEX / CHEX / VEX / OI / VOL. VOL reuses the OI tab's entire shape — `oiSides()` ladder (calls above ATM, puts below, both on the pivot), same 84/92px column widths, same near-white-on-heat number — but is a different quantity from a different source: today's LIVE traded contract count off `GreekCell.callVol`/`putVol`, no recorder, no `/proxy/oi-change` fetch, no day-over-day diff.

New `volSideValue(cell, strike, atm)` mirrors `oiSideChange()`: call volume above ATM (positive → blue), put volume below (negative → red), call − put at the pivot. It is what `valueAt()` returns for `greekMode === "vol"`, so the heat scale, the per-column header totals and the ⅀ Total column all read the same number the cell draws from — the tint and the figure can't disagree. `visibleTotal` got the matching branch.

New `VolLine` renders each side UNSIGNED (`fmtCount` → `12.4K`), unlike `OiChgLine`'s signed `fmtChg`: volume is a level, not a change, so a leading `+` on every figure is noise — side is already carried by row position, the C/P letter on the pivot, and the tint. Untraded strikes print `·` rather than `0`.

Introduced `isCountMode = isOiMode || isVolMode` and moved every "this tab counts contracts, not dollars" branch onto it: cell padding, `alignItems`, the four grid-track width ternaries, the ⅀ Total cell's SignVal bypass, and the Δ-column lockout (`isChangeCol` / `isCh` in both the header-total and row-total passes). The 15/30/60m Δ columns carry volume-GEX **dollars** — on VOL that is a different unit in the same grid, same reason OI already excluded them. `hasDelta15` also moved to `!isCountMode` (no behaviour change, it was already GEX-gated). Replay tooltip now reads `DEX/CHEX/VEX/OI/VOL are not recorded`; the greek tabs stay pinned to GEX while replaying.

Type-narrowing verified: `valueAt()` and `visibleTotal` both early-return on `"oi"` then `"vol"`, so the trailing `cell[greekMode]` narrows to the four real greek keys — confirmed against an isolated `tsc --strict` harness. Full-file `tsc` shows only module-resolution errors (no repo `node_modules` in this session). **NOT build-verified** — run `npx tsc --noEmit` + `npm run build` before push. Not deployed.

Open question flagged to the owner: the tab label `VOL` sits near the existing `Vol Only` data-mode toggle, which is an unrelated control (how GEX is computed, not what the grid shows). Relabel to `TVOL` if that reads as ambiguous on screen.

## Monday 8/3/2026 (c) — Journal: focus panes, theme-sourced palette, chart axes, 3x2 fill (`components/pages/Trading.tsx`, `app/globals.css`)

Four rendered mockups were put up for the `/trading` density complaint (digest rows / dollar-ranked triage / focus panes / neutral canvas); the owner picked focus panes. `Trading.tsx` now splits Leaks, The clock, Charts and Journal into a `PANES` segmented nav (`DOCK_THEME` tiles, active in `HOME_THEME.cyan`) showing one pane at a time with the header totals pinned above, and `insightCard()` renders header / body / verdict BANDS off shared `bandHeadStyle`/`bandBodyStyle` instead of one continuous padding block — the collapsible table headers and the calendar were folded onto the same band, retiring their cyan/orange/purple titles.

Palette: an intermediate teal/rose pass was reverted as aded — gain/loss is now `REFRESH_GREEN` + `HOME_THEME.red` imported from `homeTheme.ts`, verdict tones are `HT.red`/`HT.orange`/`T.green`, the heat map and legend derive their tints through a local `rgba()` helper off `T`, and a page-local `DIM` gray was deleted. **Zero color literals remain in the page — every hex left in the file is inside a comment.** The card surface keeps the theme's `classic` card (`panelBg`, `HOME_THEME.border`, radius 18) with only a deeper shadow; `globals.css` gained a `.no-card-lift .journal-focus.card-hover:not(.no-card-lift):hover` rule that wins back specificity using the dashboard-wide `.card-hover:hover` values verbatim (modals carry `no-card-lift` themselves and stay still).

Charts got real axes. New `ChartFrame` wraps all three primitives — Y labels in a 50px gutter, gridlines behind the plot, X labels in a strip below — exploiting the fact that the viewBox height equals the rendered CSS height, so a tick's y in DATA space is its `top` in PIXELS and gridlines can be plain divs routed through the same `y()` the marks use. `niceTicks()` snaps to 1/2/2.5/5x10^n (the 2.5 rung stops a 0-14k range collapsing to three labels), `fmtAxis$` compacts to `2.5k`, `axisX()` always emits the last label and drops a neighbour within 9% so it can't overprint. `MiniBars` per-bar value labels are dropped past 24 bars; **fixed a latent bug in `MiniLabeledBars` — it drew from a hardcoded baseline, so an all-negative series would have rendered flipped above the zero line** (now `y(0)`->`y(v)`), which also gave back the 14px its label row was stealing. Chart height 120->170.

Layout: the Leaks grid was `repeat(auto-fit, minmax(330px,1fr))`, which on a 2000px monitor resolved to SIX columns and left the bottom two-thirds of the screen dead. Now a fixed `repeat(3,1fr)` x `repeat(2, minmax(300px,1fr))` with `flex:1`/`minHeight:0` on both the grid and the pane column (the wrapper had no height to fill, so the row tracks alone would have done nothing); header card pinned `flexShrink:0`. Content scaled to match — bar tracks 9->12px, bar rows 12->13px on a 96px label column, headline values 29->34px, verdict 12.5->13.5px — and the body band centres vertically, without which the taller card just relocated the dead space above the verdict. `globals.css` adds a 900-1400px step-down to 2 across and clears the fixed row track at both breakpoints.

Verified by rendering: the patched chart components were run in a Vite harness against realistic series (tick alignment, crosshair, hover dot and tooltip all checked), and the grid was shot at 2000x1153 and 1280x860. Zero TSX parse diagnostics, zero page errors, balanced CSS. **NOT build-verified** — no repo `node_modules` in this session; run `npx tsc --noEmit` + `npm run build` before push. Not deployed.
