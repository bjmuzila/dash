...
export async function GET(req: NextRequest) {
  ...
  // Headless Chromium resolves the font stack to a different face than a desktop
  // browser does, and the pills' optical-centring nudge is measured against the
  // font — same markup, same html2canvas, text ~3px high without this. The
  // constant lives beside the browser one in econSnapshot.ts; ECON_SNAP_NUDGE_EM
  // overrides it so a re-measure does not need a code change.
  const pillNudgeEm = Number.isFinite(Number(process.env.ECON_SNAP_NUDGE_EM))
    ? Number(process.env.ECON_SNAP_NUDGE_EM)
    : PILL_NUDGE_EM_HEADLESS;

  const html = buildSnapshotHTML(events, quote, logoDataUrl, earnings, tickerLogos, { pillNudgeEm });
