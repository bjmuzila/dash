...
// "All users" = signed-up accounts + waitlist, deduped. The two legacy CSV
// lists are NOT part of it any more: stale addresses bounce, hit spam traps and
// draw "Report spam" clicks, which sinks cbedge.net's sender reputation and
// sends every broadcast to the spam folder. They stay available as explicit
// picks, and send() asks for confirmation when either is ticked.
const AUDIENCE_OPTIONS: SegOption[] = [
  { value: "all", label: "👥 All users (no old lists)" },
...
// Legacy CSV lists: deliverability risk, confirm before sending to them.
const LEGACY: Audience[] = ["old_emails", "old_emails2"];
...
export default function Emails() {
  ...
  // Plain-text version from the loaded template. Sent alongside the HTML so the
  // email is multipart (HTML-only mail is a spam signal). Blank = the server
  // derives one from the HTML.
  ...
  async function send() {
    ...
    try {
      const payload: Record<string, unknown> = {
        subject: subj, html,
        // Template's own plain-text version; blank → server builds one from html.
        text: textBody.trim(),
      ...
      <Card accent="cyan">
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 18 }}>
          ...
          <div>
            ...
            <textarea
              value={body}
              onChange={(e) => { setBody(e.target.value); if (!e.target.value.trim()) setTextBody(""); }}
            ...
            />
            {body.trim() && (
              <div style={{ fontSize: 12, color: HOME_THEME.muted, opacity: 0.7, marginTop: 6 }}>
                {textBody.trim()
                  ? "Plain-text version: from the template (sent with the HTML)."
                  : "Plain-text version: built automatically from the HTML."}
              </div>
            )}
            {body.trim() && (
              <div style={{ marginTop: 10 }}>
          ...
          {hasLegacy && (
            <div style={{ fontSize: 14, color: HOME_THEME.red, fontWeight: 600, padding: "10px 12px", borderRadius: 10,
                          border: `1px solid ${HOME_THEME.red}55`, background: `${HOME_THEME.red}14` }}>
              ⚠️ Old email list selected. Stale addresses bounce and get marked as spam, which can send
              ALL your emails to the spam folder. You'll be asked to confirm.
            </div>
          )}
          ...
          <div style={{ display: "flex", justifyContent: "flex-end" }}>
            <DockButton
              ...
              style={{
                ...
                padding: "0 22px",
                fontSize: 14,
                color: hasLegacy ? HOME_THEME.red : HOME_THEME.cyan,
                border: `1px solid ${hasLegacy ? HOME_THEME.red : HOME_THEME.cyan}59`,
                background: hasLegacy
                  ? `${HOME_THEME.red}1f`
