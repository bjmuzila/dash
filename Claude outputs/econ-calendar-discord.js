...
async function fetchSnapshotHtml(base) {
  ...
  return {
    ...
    pres: n('x-econ-pres'),
    earn: n('x-econ-earn'),
    // "ok" / "cache" / "unavailable" — see the route. Zero rows because the
    // upstream feed is down is NOT the same as zero rows because it is a quiet
    // day, and only the second one deserves a silent skip.
    feed: res.headers.get('x-econ-feed') || 'ok',
    feedWarning: res.headers.get('x-econ-warning') || '',
  };
}
...
async function collectOnce(base, opts = {}) {
  ...
  try {
    ...
    // A dead feed is an ERROR, not a quiet day. Treating it as "nothing on today"
    // is how a broken calendar goes unnoticed for a week: it looks exactly like
    // August. Throwing here means the tick retries inside the grace window, the
    // card shows the reason, and nothing claims the day.
    if (feed === 'unavailable') {
      throw new Error(`calendar feed unavailable${feedWarning ? ` — ${feedWarning}` : ''}`);
    }

    if (!econ && !pres && !earn && !cfg.postEmpty && !opts.force) {
      console.log(`[econ-cal] nothing on the calendar today (feed:${feed}) — skip`);
      await store.markRun(JOB_ID, { status: 'skipped', error: `empty day (feed:${feed})` });
