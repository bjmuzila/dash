...
// "All users" combined list: signed-up accounts, then waitlist, deduped.
//
// The two legacy CSV lists (old_emails / old_emails2) are deliberately NOT in
// "All users" any more. Stale addresses bounce, some are spam traps, and people
// who don't remember signing up hit "Report spam" · all of which drags the
// cbedge.net sender reputation down and lands EVERY broadcast (subscribers
// included) in the spam folder. They can still be sent to on purpose by ticking
// "Old emails" / "Old emails 2" explicitly; the composer warns before it does.
function allUsersList(lists: {
  signedUp: string[]; waitlist: string[];
}): string[] {
  const seen = new Set<string>();
  for (const email of [...lists.signedUp, ...lists.waitlist]) {
...
export async function GET(req: NextRequest) {
  try {
    ...
    // "All users" = signed-up accounts + waitlist, deduped. The legacy CSV
    // lists are excluded on purpose (deliverability · see allUsersList()).
    const all = allUsersList({ signedUp, waitlist });
...
// Plain-text alternative built from the HTML, used only when the composer sent
// no text part (a hand-typed or pasted body). HTML-only mail is a spam signal,
// so every broadcast goes out multipart. Links keep their URL in parentheses so
// the {{UNSUBSCRIBE_URL}} placeholder survives for applyUnsubscribeText().
...
export async function POST(req: NextRequest) {
  try {
    ...
    // Always send a plain-text part: the template's own text version when the
    // composer passes it, otherwise one derived from the HTML.
    ...
    if (picked.has("custom")) {
      ...
      // Signed-up accounts + waitlist, deduped. Legacy lists are NOT included
      // (deliverability · see allUsersList()); tick them explicitly to send.
      const recipients = await listAllUsersForBroadcast();
      const signedUp = recipients.map((r) => r.email);
      const waitlist = await listWaitlistEmails().catch(() => []);
      to = allUsersList({ signedUp, waitlist });
    } else {
