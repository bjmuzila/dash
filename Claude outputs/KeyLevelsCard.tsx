...
import type { GexFrame } from '@/contract/frames'
import { chainGexUrl } from '../chainGex'
...
import { LevelsFor, type LevelsSource } from './levelsSource'
...
// The ladder itself — LevelsSource, the SPX socket reader and the /api/chains
// reader — moved to ./levelsSource.tsx on 2026-09-18, unchanged. The camera's
// Stats probe needs the same rows-and-spot without any of the drawing below,
// and two copies of the SPX/everything-else split is how the two would end up
// disagreeing about which feed answers for a ticker.

export function KeyLevelsCard() {
  const { symbol } = usePageSymbol()
  const body = (s: LevelsSource) => <LevelsBody symbol={symbol} {...s} />
  return <LevelsFor symbol={symbol} render={body} />
}
...
function LevelsBody({
  ...
  //
  // 📸 The Stats row — six lines of TEXT, not a picture. The derivation and the
  // target both live in ./statsShot.ts now, because the camera has to be able
  // to take this shot from any page without walking you to the home board: when
  // this card is not mounted, shell/CopyShot.tsx mounts KeyLevelsStatsProbe,
  // which publishes the identical target off the identical function.
