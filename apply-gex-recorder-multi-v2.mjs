#!/usr/bin/env node
/**
 * apply-gex-recorder-multi-v2.mjs — supersedes apply-gex-recorder-multi.mjs.
 *
 * v1 had 19/21 anchors resolve; the two that failed both SPANNED the
 * "Dealer gamma by DTE" block (sweepChainByExpiry / pmDteGamma /
 * eod-dte-gamma-recorder) that was added to this file in parallel. Their
 * anchors reached across the insertion points, so they could no longer match.
 * Nothing was written, which is why the file is still intact.
 *
 * Rebuilt here:
 *   - the computeLiveGexEx0dte edit now STOPS at that function's closing brace,
 *     so the Dealer-gamma section that follows is irrelevant to it
 *   - the PM-pass edits are two single-line anchors (the upsert call and the
 *     console.log), so the `void pmDteGamma(...)` line between them is untouched
 *
 * The Dealer-gamma code is left exactly as-is. Note it is NOT affected by the
 * multi-expiry clobbering bug this change set fixes: sweepChainByExpiry builds
 * its byStrike map inside the per-expiry callback, so each expiry keeps its own
 * row rather than overwriting a shared one.
 *
 * Verified: replaying all 21 edits on the pre-parallel file reproduces the
 * intended result byte-for-byte.
 *
 * All-or-nothing: if any anchor fails to resolve, nothing is written.
 * Safe to run twice.
 *
 * Run from the repo root:  node apply-gex-recorder-multi-v2.mjs
...
const EDITS = [
  ...
  [
    ...
    "computeLiveGexEx0dte rewrite + computeLiveGexRowsMulti/getLiveGexRowsMulti (REBUILT: stops at the function's closing brace, before the Dealer-gamma-by-DTE section)"
  ],
  [
  ...
  [
    ...
    "PM pass: compute gex0dte + pass it to upsertEodGex (REBUILT: single-line anchor, leaves the pmDteGamma call alone)"
  ],
  [
    ...
    "PM pass: log the 0DTE OI+Vol total (REBUILT: single-line anchor)"
  ],
  [
