...
| Verified on the laptop | `npm run check` passes end to end: casing, typecheck, theme, build, budgets, ws-scope, perf, and all four parity self-tests |
| Step 4 — DONE | `scripts/parity-check-analysis.mjs` + `.test.mjs`, wired in as `check:parity:analysis:self`. 120 probes. Ten of them compare COMPUTED COLOURS, which is the only mechanism in the repo that can catch a token resolving to the wrong colour — `check-theme.mjs` bans literals and structurally cannot see it. The self-test drives the two regressions this port actually shipped (four stubbed cards; Ticker Levels reduced to fixed pills) plus a v3-palette page whose values are all correct, and asserts that only the colour probes fire on that one |
