...
const INK = HOME_THEME.text;             // the only text color here

/**
 * The white washes this page is built out of, named once.
 *
 * Every one is `alpha()` over a token rather than a typed rgba(): the value
 * keeps tracking --color-fg, and check-theme.mjs bans the literal outright
 * (non-negotiable #1). The two GRID values are the chart's zero line and its
 * ordinary gridlines — a two-step contrast that says which line is the axis.
 *
 * These are CSS strings, so on an SVG element they go through `style`, NEVER
 * through a `stroke=` / `fill=` presentation attribute: those are parsed as a
 * <paint> and do not resolve var() or color-mix(), so the shape would render
 * with no paint at all and nothing would warn.
 */
const GRID = alpha(T.text, 0.07);        // gridlines, month rules
const GRID_ZERO = alpha(T.text, 0.28);   // the zero line, in % mode
const CROSSHAIR = alpha(T.text, 0.35);   // hover rule
const FUTURE_WASH = alpha(T.text, 0.025); // the part of the year not reached yet
const WASH_03 = alpha(T.text, 0.03);     // stat tile plate
const WASH_04 = alpha(T.text, 0.04);     // an unselected chip
const WASH_06 = alpha(T.text, 0.06);     // the Clear button
...
function StatTile({
  ...
  return (
    ...
    >
      ...
      {sub ? (
        <div style={{ fontSize: "var(--text-xs)", color: INK, marginTop: 3 }}>{sub}</div>
      ) : null}
    </div>
  );
}
...
.sea-railgrp{font-size:var(--text-3xs);font-weight:800;letter-spacing:.14em;text-transform:uppercase;
...
.sea-railitem{display:block;width:100%;text-align:left;padding:7px 10px;border-radius:8px;
  ...
  font:inherit;font-size:var(--text-xs);font-weight:600;cursor:pointer;line-height:1.3}
...
.sea-railitem[aria-current="true"]{font-weight:800;color:${HOME_THEME.cyan};
  border-color:${alpha(HOME_THEME.cyan, 0.4)};
  background:linear-gradient(90deg,${alpha(HOME_THEME.cyan, 0.26)},${alpha(HOME_THEME.cyan, 0.04)})}
.sea-compare{border:1px solid ${alpha(HOME_THEME.cyan, 0.4)};
  background:linear-gradient(180deg,${alpha(HOME_THEME.cyan, 0.12)},${SEA.card2});transition:border-color .15s}
...
.sea-caret{color:${HOME_THEME.cyan};font-size:var(--text-xs);display:inline-block;transition:transform .15s}
...
.sea-comparecta{margin-left:auto;flex:none;padding:5px 12px;border-radius:999px;
  background:${HOME_THEME.cyan};color:var(--color-sea-on-accent);font-size:var(--text-xs);font-weight:800;letter-spacing:.06em;
...
@media (max-width:860px){
  ...
  .sea-railgrp{padding:0 4px 0 8px;white-space:nowrap;align-self:center;font-size:var(--text-3xs)}
    ...
    <>
      ...
      >
            ...
            return (
              <button
                ...
                style={{
                  ...
                  background: on
                    ? `linear-gradient(180deg, ${alpha(SEASON_COLOR, 0.2)}, ${alpha(SEASON_COLOR, 0.05)})`
                    : WASH_04,
        ...
        >
          ...
          <div style={{ display: "flex", gap: 18, fontSize: "var(--text-xs)", fontWeight: 700 }}>
        ...
        <details className="sea-compare" style={{ marginBottom: 14, borderRadius: 12 }}>
          ...
          >
            ...
            <span style={{ fontSize: "var(--text-base)", fontWeight: 800 }}>Compare any year</span>
            {overlaySeries.length ? (
                ...
                {overlaySeries.map((o) => (
                  ...
                  >
                    <svg width={16} height={6} aria-hidden>
                      <line x1={0} y1={3} x2={16} y2={3} style={{ stroke: o.color }} strokeWidth={2} strokeDasharray={o.dash || undefined} />
                    </svg>
                    {o.short}
                  </span>
                ))}
              ...
              <span style={{ fontSize: "var(--text-sm)", fontWeight: 600, color: INK }}>
          ...
          <div style={{ padding: "0 14px 14px" }}>
            <div style={{ display: "flex", gap: 10, alignItems: "center", flexWrap: "wrap", marginBottom: 12 }}>
              <span style={{ fontSize: "var(--text-xs)", color: INK }}>
            ...
            <div style={{ display: "flex", gap: 6, flexWrap: "wrap", alignItems: "center", marginBottom: 10 }}>
              <span style={{ width: 46, flexShrink: 0, fontSize: "var(--text-xs)", fontWeight: 800, color: HOME_THEME.green }}>
                Cycle
              </span>
              {CYCLE_DEFS.map((c) => {
                ...
                return (
                  <button
                    ...
                    style={{
                      ...
                      background: slot ? alpha(slot.color, 0.18) : WASH_04,
            ...
            {DECADES.map(([dec, years]) => (
              <div key={dec} style={{ display: "flex", gap: 6, flexWrap: "wrap", alignItems: "center", marginBottom: 6 }}>
                <span style={{ width: 46, flexShrink: 0, fontSize: "var(--text-xs)", fontWeight: 800, color: INK, fontVariantNumeric: "tabular-nums" }}>
                  {dec}s
                </span>
                {years.map((y) => {
            ...
            <p style={{ margin: "10px 0 0", fontSize: "var(--text-xs)", color: INK, lineHeight: 1.6, maxWidth: "72ch" }}>
            ...
            <p style={{ margin: "8px 0 0", fontSize: "var(--text-xs)", color: INK, lineHeight: 1.6, maxWidth: "72ch" }}>
            ...
            >
              ...
              {ticks.map((t) => (
                <g key={`y${t}`}>
                  <line
                    ...
                    y1={y(t)}
                    y2={y(t)}
                    style={{ stroke: t === 0 && mode === "pct" ? GRID_ZERO : GRID }}
                    strokeWidth={1}
                  />
                  <text
                    x={PAD.left - 8}
              ...
              {MONTHS.map((m, mi) => (
                <g key={m.label}>
                  ...
                  />
                  {mi % (innerW < 470 ? 2 : 1) === 0 ? (
                    <text x={x(m.day) + 4} y={CHART_H - 10} fontSize={11} fontWeight={700} style={{ fill: INK }}>
                      {m.label}
                    </text>
                  ) : null}
                </g>
              ...
              {overlaySeries.map((o) => {
                ...
                return (
                  <g key={o.id}>
                    <path d={d} fill="none" style={{ stroke: o.color }} strokeWidth={1.8} strokeDasharray={o.dash || undefined} opacity={0.95} />
                    <circle cx={x(endI)} cy={y(o.values[endI])} r={3} style={{ fill: o.color }} />
                    ...
                    <line
                      ...
                      opacity={0.35}
                    />
                    <text x={PAD.left + innerW + YEAR_LABEL_DX} y={y(o.values[endI]) + 4} fontSize={11} fontWeight={800} style={{ fill: o.color }}>
                      {o.short}
                    </text>
                  </g>
                );
              ...
              <path d={seasonPath} fill="none" style={{ stroke: SEASON_COLOR }} strokeWidth={2} opacity={0.85} />
              <path
                d={yearPath}
                fill="none"
                style={{ stroke: YEAR_COLOR }}
                strokeWidth={2.4}
                strokeLinejoin="round"
              />

              {/* end-of-2026 marker */}
              <circle cx={x(last)} cy={y(yearSeries[last])} r={4} style={{ fill: YEAR_COLOR }} />
              ...
              {hover != null ? (
                <g>
                  ...
                  />
                  <circle cx={x(hover)} cy={y(seasonSeries[hover])} r={3.5} style={{ fill: SEASON_COLOR }} />
                  {overlaySeries.map((o) =>
                    hover < o.values.length ? (
                      <circle key={o.id} cx={x(hover)} cy={y(o.values[hover])} r={3} style={{ fill: o.color }} />
                    ) : null,
                  )}
                  {hYear != null ? (
                    <circle cx={x(hover)} cy={y(hYear)} r={3.5} style={{ fill: YEAR_COLOR }} />
                  ) : null}
                </g>
              ) : null}
            </svg>
...
const quickBtn = (on: boolean): React.CSSProperties => ({
  ...
  borderRadius: 6,
  border: `1px solid ${on ? HOME_THEME.cyan : HOME_THEME.border}`,
  background: on ? alpha(HOME_THEME.cyan, 0.18) : WASH_04,
