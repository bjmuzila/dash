    ...
    <Page title="Whale Archive">
      ...
      <div className="grid grid-cols-1 gap-2 xl:grid-cols-[minmax(0,1fr)_320px]">
        <div className="flex min-w-0 flex-col gap-2">
          ...
          {/* ── prints ───────────────────────────────────────────────────── */}
          {/* Matches the right rail's height (lookup + size + buckets + repeats):
              the grid row is sized by the rail, the card fills it, and the table
              scrolls inside an absolutely-positioned box so its row count can
              never push the card taller than the rail. */}
          <Card
            className="flex-1"
            title="Prints"
