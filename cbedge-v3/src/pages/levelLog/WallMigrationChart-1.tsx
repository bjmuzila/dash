...
/**
 * The price ticks the axis prints — plain grey gridline numbers every 5 or 10
 * points. Wide ranges (multi-day) step up in multiples of 10 so the column
 * never holds more than ~24 numbers; sub-5-point ranges (cheap tickers) fall
 * back to a round step off the range so the axis is never empty.
 */
function priceTicks(lo: number, hi: number): number[] {
  ...
  if (span < 10) {
    ...
    step = [1, 2, 2.5, 5, 10].map((m) => m * mag).find((v) => v >= raw) ?? 10 * mag
  } else if (span <= 60) {
    step = 5
  } else {
    const ladder = [10, 25, 50, 100, 250, 500, 1000]
    step = ladder.find((v) => span / v <= 24) ?? 1000
  }
  const out: number[] = []
...
export function WallMigrationChart({
  ...
  /** The only numbers on the price axis: grey round ticks, nothing else. */
  const axisTicks = priceTicks(lo, hi)
