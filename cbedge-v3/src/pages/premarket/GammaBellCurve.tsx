...
} from "@/pages/premarket/gammaChartKit";
...
export default function GammaBellCurve({
  ...
  const geo = useMemo(() => {
    ...
    for (let i = 1; i < binsIn.length; i++) {
      // i starts at 1 and is bounded by binsIn.length, so both indices are in range.
  ...
  const onPointerMove = useCallback((e: ReactPointerEvent<SVGSVGElement>) => {
    ...
    for (let i = 0; i < binsIn.length; i++) {
      // i is bounded by binsIn.length, so this index is always in range.
