...
export const etMinutes = (ts: number) => {
  ...
  return h !== undefined && Number.isFinite(h) ? h * 60 + (m || 0) : -1;
};
...
export function structureFromChain(items: unknown[], expDate: string, spot: number): NextStructure | null {
  ...
  // rows.length >= 5 is guaranteed by the guard above, so rows[0] is in range.
...
export function useRecordedWalls(date: string, symbol = "SPX", throughMin?: number) {
  ...
  const byLevel = useMemo(() => {
    ...
    for (const lvl of ["call_wall", "put_wall", "cb"] as WallLevel[]) {
      ...
      // rows is non-empty per the check above.
      const openRow = rows.find((r) => r.reason === "open") ?? rows[0]!;
      ...
      out.set(lvl, {
        ...
        open: openRow.strike ?? null,
        last: lastRow.strike ?? null,
