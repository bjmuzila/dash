...
/** A 3-vector / row of the 3×3 system, and the augmented (3×4) working matrix. */
...
function solve3(A: Mat3, b: Vec3): Vec3 | null {
  ...
  for (let i = 0; i < 3; i++) {
    let piv = i;
    // i, r and piv all range over 0..2 and m always has exactly 3 rows/4 columns,
    // so these indices can never be out of range — the `!`s just satisfy
    // noUncheckedIndexedAccess, they are not real guards.
...
export function lsqGaussian(rows: Bin[]): { a: number; mu: number; sigma: number; lsq: boolean } | null {
  ...
  // `use.length < 5` returned above, so first/last always exist.
  const first = use[0]!;
  const last = use[use.length - 1]!;
  const span = last.k - first.k;
  const ok = Number.isFinite(sigma) && Number.isFinite(mu) && Number.isFinite(a)
    && sigma > 0.3 && sigma < span * 2
    && mu > first.k - span && mu < last.k + span
    && a > 0 && a < peak * 12;
...
export function wideHalfOf(chain: ChainRow[], spot: number): number {
  ...
  // dists.length >= 1 is checked above, and the index is clamped to
  // [0, dists.length - 1], so it is always in range.
...
export function useGridStep(wide: Bin[]): number {
  return useMemo(() => {
    ...
    const gaps: number[] = [];
    // i runs 1..wide.length-1 and wide.length >= 2 is checked above, so both
    // wide[i] and wide[i - 1] always exist.
...
export function foldBins(raw: Bin[], maxBars = MAX_BARS): Bin[] {
  ...
  // raw.length > maxBars >= 0 is checked above, so raw is non-empty.
  const a0 = raw[0]!.k;
...
export function strikesHalf(wide: Bin[], spot: number, n: number): number {
  ...
  below.sort((a, b) => a - b);
  above.sort((a, b) => a - b);
  // arr.length is checked truthy before indexing, so the clamped index is
  // always in range.
