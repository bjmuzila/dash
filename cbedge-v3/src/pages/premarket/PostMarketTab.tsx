...
import { EV_ROW_H, POSTMARKET_CSS } from "@/pages/premarket/postMarketTab.css";
// Re-exported so `import PostMarketTab, { POSTMARKET_CSS }` still resolves.
// Premarket.tsx now takes the stylesheet from the .css module directly, which is
// what lets it lazy() this file.
export { POSTMARKET_CSS };
...
export default function PostMarketTab(p: PostMarketProps) {
  ...
  const lastPathPt = path.pts.length ? path.pts[path.pts.length - 1] : undefined;
  const closePx = spot > 0 ? spot : (lastPathPt ? lastPathPt.px : 0);
  const rthHi = useMemo(
  ...
  const evWindow = useCallback((half: number) => {
    ...
    // `b` is always a valid index into `perStrike`: it starts at 0 and is only
    // ever reassigned to `i`, the current index of the same array.
  ...
  const evCover = useMemo(() => {
    ...
    return { from: etMinutes(first.ts), to: etMinutes(last.ts) };
  }, [cols]);
   ...
   *   · The composition label follows what is lit, so it always describes the
   *     windows actually drawn beside it.
   *
  ...
  const evRows = useMemo<EvRow[]>(() => {
    ...
    return evBars.map((b) => {
      ...
      // `cuts` always has one more entry than `activeBuckets` (built as
      // `[0, ...bounds.map(...)]` with `bounds = activeBuckets.map(...)`), so
      // cuts[i] and cuts[i + 1] are always in range here.
      ...
      const allSegs = total > 0
        ? moves.map((mv, i) => {
            // `moves` is built by mapping over `activeBuckets`, so the two
            // arrays are always the same length.
            const bucket = activeBuckets[i]!;
            return { share: mv / total, color: bucket.color, label: bucket.label };
          })
        : [];
  ...
  const wallPath = useMemo(() => {
    ...
    const raw = cols.map((c) => {
      ...
      // Inside the loop `c.cells` is non-empty (we're iterating over it), so
      // `cw`/`pw` are always real entries by then — the `=== undefined`
      // checks only cover the type checker, not a real empty-cells case.
      for (const x of c.cells) {
        if (cw === undefined || x.net > cw.net) cw = x;
        if (pw === undefined || x.net < pw.net) pw = x;
      }
      return {
        ts: c.ts, spot: c.spot,
    ...
    const pts = raw.map((p, i) => ({
      ...
      // `sideRaw` is built by mapping over `raw`, so it is always the same
      // length and `sideRaw[i]` is in range here.
      coreSide: sideRaw[i]!,
    }));
  ...
  const writtenVsTraded = useMemo(() => {
    ...
    if (firstStrike === undefined) return [];
    // `strikes.length > 1` guarantees indices 0 and 1 both exist.
    const step = strikes.length > 1 ? Math.abs(strikes[0]! - strikes[1]!) : 5;
    const minutesAt = new Map<number, number>();
    for (const c of cols) {
      if (!(c.spot > 0)) continue;
      const near = strikes.reduce((b, k) => (Math.abs(k - c.spot) < Math.abs(b - c.spot) ? k : b), firstStrike);
      if (Math.abs(near - c.spot) <= step) minutesAt.set(near, (minutesAt.get(near) ?? 0) + 1);
    }
    return strikes.map((k) => {
      const e = series.get(k);
      const added = e ? Math.abs((e.vals[e.vals.length - 1] ?? 0) - (e.vals[0] ?? 0)) : 0;
  ...
  const centerEv = useCallback(() => {
    ...
    // `n = evRows.length` is checked above, so every index used below (0,
    // n - 1, and i while i < n) is provably in range.
          ...
          {grades.map((g) => (
            <div className="sc" key={g.key}>
              <div className="nm">
                ...
                {/* One tone → one class, through the shared map. The violet
                    case used to be an inline style here and nothing at all in
                    HistoricalRecap, which is exactly how the two tabs ended up
                    grading the same PINNED reaction in two different colours. */}
                <span className={pillClass(g.tone)}>{g.status}</span>
              </div>
          ...
          <div className="col">
            ...
            {wallPath ? (
              <>
                ...
                {/* wallPath.pts always has >= 3 entries — see the `raw.length < 3` guard above. */}
                <div className="heatx">
                    ...
                    <div className="mrow" key={r.strike}>
                      <div className="mleft">
                        <div className="mbar" style={{
                          ...
                          background: "linear-gradient(270deg,var(--violet),color-mix(in srgb, var(--color-violet) 30%, transparent))",
                        }} />
                      </div>
              ...
              <>
                <div className="premlist">
                  {premiumRows.map((r) => {
                    ...
                    // premiumRows[0] exists here — the empty case renders the
                    // warnbar branch above instead of this .map.
