...
import { PageVisitBeacon } from '@/data/pageVisit'
...
// /chain (the quote-book chain, src/pages/Chain.tsx) was REMOVED 2026-09-21 —
// unrouted and off the rail. /options-chain is untouched. The page files are
// orphaned, not imported anywhere, and can be deleted.
...
export default function App() {
  return (
    <BrowserRouter basename="/v3">
      ...
      <MobileRedirect />
      {/* The visit beacon. One row per route the user lands on, including the
          phone build — v3 shipped with NO tracking at all, so every paid
          customer went dark the moment they crossed over from the Next pages.
          Inside the router because it reads the location; above the Shell
          because it renders nothing and must not depend on chrome. */}
      <PageVisitBeacon />
      <Shell>
