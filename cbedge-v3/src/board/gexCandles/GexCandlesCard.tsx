...
function useEsChart(
  ...
  const onMount = useCallback((frame: ChartHandle): (() => void) => {
    ...
    void mountEsChart(frame.el, {
      ...
      // NON-NEGOTIABLE 5, and this is the card it matters most on: the chart's
      // overlay runs a steady rAF loop, so it takes `handle.visible` — the
      // per-frame predicate — rather than the `onVisibility` edge. The frame
      // was already handing us this and we were dropping it on the floor, so
      // the bubble layer redrew the whole band for a card scrolled out of view.
