...
export function densifyChainRows(chain: ChainRow[], step = 5): ChainRow[] {
  ...
  const minStrike = first.strike;
  const maxStrike = last.strike;
  const start = Math.floor(minStrike / step) * step;
  const end = Math.ceil(maxStrike / step) * step;
  const spotPrice = first.spotPrice ?? first.spot ?? 0;
...
export function findGEXFlip(chain: ChainRow[], spotPrice?: number): number | null {
  ...
  for (let i = 0; i < sorted.length - 1; i++) {
    ...
    if (!rowA || !rowB) continue; // i+1 < sorted.length by the loop bound, always defined

    const a = oiVol(rowA);
    const b = oiVol(rowB);

    if (a === 0) { crossings.push(rowA.strike); continue; }
    if (b === 0) { crossings.push(rowB.strike); continue; }

    // Sign change → linear interpolation for sub-strike precision
    if ((a > 0 && b < 0) || (a < 0 && b > 0)) {
      const sA = rowA.strike, sB = rowB.strike;
  ...
  const best = (spotPrice == null || !Number.isFinite(spotPrice))
    ? crossings[0]! // crossings.length checked just above
...
export function findCallWall(chain: ChainRow[]): number | undefined {
  ...
  let max = chain[0]!; // chain.length checked above
  for (const row of chain) {
    if ((row.callGEX ?? 0) > (max.callGEX ?? 0)) max = row;
  }
  return max.strike;
}
...
export function findPutWall(chain: ChainRow[]): number | undefined {
  ...
  let max = chain[0]!; // chain.length checked above
  for (const row of chain) {
    if (Math.abs(row.putGEX ?? 0) > Math.abs(max.putGEX ?? 0)) max = row;
  }
  return max.strike;
}
...
export function computeGEXProfile(
  ...
  // put leg near ATM and inflating it in the wings — which walked the zero
  // crossing off the level the bars actually cross at.
  //
  ...
  // decay instead of 1 — which flattens gamma and drags the crossing.
  ...
  for (let i = 0; i < values.length - 1; i++) {
    const a = values[i]!, b = values[i + 1]!; // i+1 < values.length by the loop bound
    if ((a >= 0 && b < 0) || (a < 0 && b >= 0)) {
      const li = levels[i]!, li1 = levels[i + 1]!; // levels/values share length N, built together above
      const z = li1 - ((li1 - li) * b / (b - a));
      if (Number.isFinite(z)) brackets.push({ lo: li, hi: li1, z });
    }
  }
