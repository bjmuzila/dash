...
// on top of whatever the page publishes. A row whose surface is live behaves
// exactly as it always did. A row whose surface is not gets MADE ready on the
// click: the menu mounts an off-screen PROBE for a row that needs data and not
// pixels (Stats — text, so it is taken where you stand and does NOT walk you to
// the home board), or navigates to the route and signals any surface that has
// to arrange itself, waits for the real target to publish, and only then
// shoots it.
//
...
//
// The prepare signal below is module-level rather than context, because the
// thing that has to answer is on the page the click is navigating TO — it is
// not mounted when the click happens and so cannot be under a provider the
// click can read.
...
/**
 * A beat after a freshly navigated-to surface resolves, for its first data and
 * its first paint. A live row waits for nothing — it is already on screen, and
 * a probe publishes only once its numbers are real.
 */
...
export function CopyShotMenu() {
  ...
  /**
   * The off-screen probe slot. Holds a component for exactly as long as one
   * shot needs it — mounted by `prepare`, dropped by its `done`. See the probe
   * field in shotAtlas.ts and KeyLevelsStatsProbe.
   */
   ...
   *
   * A PROBE row needs nothing but its own component mounted — no navigation, no
   * board card, no waiting on a page. Everything else: route first (whatever
   * has to arrange itself lives on the page being navigated to), then the
   * prepare signal, then wait for the publication. All of it undone in `done`,
   * which `run` calls whatever happens.
   */
  const prepare = useCallback(
    async (a: AtlasShot): Promise<Prepared | null> => {
      ...
      try {
        ...
        if (a.probe) {
          const load = a.probe
          const Comp = lazy(load) as unknown as ComponentType
          setProbe(() => Comp)
          undos.push(() => setProbe(null))
        }
        ...
        await wait(a.probe ? 0 : a.route ? SETTLE_MS : 0)
        return { target, done }
      } catch (e) {
        done()
        throw e
  ...
  return (
    <div className="relative shrink-0">
      {/* The probe slot. `hidden` rather than absent-from-the-DOM because these
          render null anyway; they are here to hold hooks open for a second, not
          to draw. Suspense fallback is null for the same reason. */}
      {Probe && (
