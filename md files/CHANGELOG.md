...
Every other kind came down. react went UP, by 4.6kb, in the one command whose
whole job is to stop budgets drifting above reality.

The cause: `writeRatchet()` wrote `tightened(kind)` unconditionally.
`tightened()` is `actual x (1 + ratchet.slack)` - a target for a budget that has
drifted too HIGH. react's chunk is 50.6kb against a 53.7kb ceiling, only 6%
headroom, already tighter than the 15% slack line, so its "tightened" value
(58.3kb) sits above the budget in the file. Note the SLACK report never named
react - the reporting half already knew it was fine; only the writing half
ignored that.

Fixed: `next[kind] = want == null ? now : Math.min(now, want)`. A ratchet turns
one way. A kind already tight keeps its number, and the change log only lists
values that actually moved.

Also restored `react` to 55000 in `budgets.json`, undoing the raise the buggy
run wrote.

Files: `cbedge-v3/scripts/check-budgets.mjs`, `cbedge-v3/budgets.json`
...
### Full-window layout, uniform bars, faint joins, screenshot

The card now fills the browser window (plot height follows the window,
re-rendered on resize) and the explanatory footer is gone — the legend and the
head carry it. Bars are one weight each (CORE 3.2px, walls 2.4px) and the
handoff from one stay to the next is drawn as a faint vertical (28% alpha), so
the level's path reads without the joins shouting. The y-range is taken from
the tape and from stays of ≥2 slots, so a one-slot blink 40 points away no
longer flattens the quarter. `📷 Screenshot` composes head + legend + plot +
date rail on a canvas (no html2canvas — the site CSP allows no third-party
scripts), copies it to the clipboard and downloads it.
