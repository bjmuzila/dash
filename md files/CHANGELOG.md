...
## 2026-08-13 - /levels: 12 across, plus a table and a rails view

Edited: `app/levels/page.tsx`.

**Twelve across.** The index was `repeat(auto-fill, minmax(92px, 1fr))`, so a wide
monitor fitted ~19 columns and every cell was exactly as wide as its own text —
a wall of tiny type. A fixed column count means the cells GROW with the window
instead of multiplying. Type went up with them (ticker 11→12, CB 11→13, Δ%
9→10) and the roster lands in ~14 rows. Steps to 9 / 7 / 5 / 3 columns as the
window narrows so a cell never falls under ~110px.

**Two collapsed sections under the board**, both closed on arrival and both
saving their open state:

- **Ladder table** — every number in a column: spot, CB, Δ CB, Δ%, CW, PW, an
  inline range bar, net GEX, DTE. The index answers "where is this one"; a table
  answers "which of them is furthest from its core", which you can only ask by
  running an eye down a column. Header cells that map onto one of the page's
  three sorts drive it, so the table and the board can never disagree about
  order.
- **Aligned range rails** — every ticker squashed onto the SAME 0–100% axis, put
  wall left, call wall right, gold tick for the core, white mark for spot.
  Because the axis is shared the marks line up into a distribution: a column
  bunched right means the whole tape is leaning into call walls, which no
  per-ticker view can show. Sorted by position rather than by the page sort —
  the ordering IS the readout, and A–Z would scatter it.

Both read the same filtered `view` the board does, and a click on any row or
rail pins that ticker to the dock exactly like a cell. Prefs key `-v5`.
