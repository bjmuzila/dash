...
import { useNotes, useCreateNote, useDeleteNote, useToday } from '../hooks'
import { ApiError, type Note, type Person } from '../api'
 ...
 * Both write the same notes rows through the same endpoint, so there is no
...
export default function Journal() {
  ...
  // Only for names. /today is already in the cache by the time anyone reaches
  // this screen, so this costs nothing and saves a second endpoint that would
  // exist purely to turn a user id into a first name.
      ...
      <div style={section()}>
        ...
        {days.map(([day, notes]) => (
          <div key={day} style={{ marginTop: 18 }}>
            ...
            {notes.map((n) => (
              <Entry key={n.id} note={n} mine={n.created_by === user?.id} people={people}
 ...
 */
function Entry({ note, mine, people, onDelete }: {
  note: Note; mine: boolean; people: Person[]; onDelete: () => void
}) {
  const [confirming, setConfirming] = useState(false)
  // Who wrote it, but only when it wasn't you. On a household journal that is
  // the difference between reading your own week back and reading someone
  // else's; printing your own name on every entry you wrote is just noise.
  const author = mine ? null : people.find((p) => p.id === note.created_by)?.displayName
