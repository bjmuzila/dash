      ...
      {BLOCKS.map((b) => {
        ...
        return (
          <div key={b} style={section()}>
            ...
            <div>
              {items.map((r) => (
                <RoutineRow key={r.id} routine={r} onToggle={() => toggle.mutate(r.id)} />
              ))}
            </div>
...
function RoutineRow({ routine, onToggle }: { routine: Routine; onToggle: () => void }) {
      ...
      {open && (
        <div style={{ padding: '0 0 14px 32px' }}>
          ...
          <div style={{ display: 'flex', gap: 8, marginTop: 12, flexWrap: 'wrap' }}>
            ...
            {/* Anyone in the household can retire a habit — there is no owner to
                check against any more, and a shared habit that only one person
                can remove is a habit nobody can remove once they stop opening
                the app. */}
          ...
          <div style={label({ marginTop: 9, letterSpacing: '0.06em' })}>
            Removing hides it and keeps the history — the streak isn't lost
          </div>
        </div>
      )}
    </div>
