...
/**
 * Which calendars appear on Today, and whether the rest of the household sees
 * them.
 *
 * This picker IS the privacy control. Everything else in this app belongs to the
 * household by definition, but a Google account does not — it is one person's,
 * and it holds their work diary. "Share with the household" exposes only the
 * calendars ticked here, never the whole account, so someone can put the family
 * calendar on everyone's Today without handing over the rest of their week.
 */
export default function CalendarPicker({ status }: { status: CalendarStatus }) {
  const connected = status.configured && status.connected
  const { data, isLoading, isFetching, refetch } = useCalendarList(connected)
  const save = useSaveCalendarSelection()

  if (!connected) return null
  if (status.needsReconnect) return <Muted>Reconnect to choose calendars.</Muted>
  ...
  return (
    <div style={{ marginTop: 18 }}>
        ...
        {calendars.map((c) => (
          <label key={c.id} style={row({ cursor: 'pointer', padding: '11px 0' })}>
            ...
            <span style={{ ...body(14), flex: 1, minWidth: 0, wordBreak: 'break-word' }}>
              ...
              {/* Read-only calendars still show their events — this only
                  explains why "Add to calendar" on Today never offers them. */}
              {!c.writable && (
      ...
      <label style={{ ...row({ alignItems: 'flex-start', marginTop: 6, cursor: 'pointer' }) }}>
        ...
        <span style={{ ...body(14) }}>
          Show these on everyone's Today
          <span style={{ ...body(13), color: T.muted, display: 'block', marginTop: 3 }}>
            Only the calendars ticked above — the rest of the household never sees the rest of
            your account, and they don't need to connect anything of their own.
          </span>
        </span>
      </label>
    </div>
