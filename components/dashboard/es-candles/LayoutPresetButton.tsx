...
export default function LayoutPresetButton({ inline = false }: { inline?: boolean } = {}) {
  ...
  /**
   * The panel's contents, with no chrome of their own.
   *
   * Split out so the SAME markup can be a portalled dropdown (the standalone
   * button, still used anywhere this is a bar control) or a pane inside the
   * chart cog's section rail (`inline`), where there is no popover to open —
   * a preset list opened from inside a menu was one of the nested floating
   * layers the rail exists to remove.
   */
  const body = (
    <>
      {/* Save-as row */}
  ...
  // Pane mode: the host frames it, so no wrapper, no trigger, no portal.
  if (inline) return <div style={{ display: "flex", flexDirection: "column", gap: 8, minWidth: 0 }}>{body}</div>;
