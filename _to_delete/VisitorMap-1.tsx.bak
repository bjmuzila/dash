/*
 * PORTED FROM THE LIVE OWNER CONSOLE, 2026-09-16, THEN REPAINTED.
 *
 * owner-vite/src/components/VisitorMap.tsx, copied whole. Structure, behaviour
 * and every piece of its reasoning are the console's: the Natural Earth
 * projection, the square-rooted choropleth, the one-dot-per-visitor fan-out, the
 * two-channel dot language, the pinned cards, the legend.
 *
 * What changed is the paint. Every colour now comes from ./mapTheme, which is
 * built from Voltick's tokens, and nothing in this file mints a hex. The other
 * edits are imports: the country tables are a copy beside it, and CustomerName
 * is a local stub that opens the sandbox's own customer card.
 *
 * So: structural changes belong upstream in the console, and colour changes
 * belong in mapTheme.ts. Edit the drawing here and the two screens fork, and
 * whoever compares them next will believe the wrong one.
 *
 * It draws whatever rows it is handed. Here they are synthetic (sampleVisits.ts)
 * and nothing on this page calls the backend.
 */
...
import { ALPHA2_NAME, FEATURE_ID_TO_ALPHA2 } from "./countryMaps";
...
interface VisitorDot {
   ...
   *  This — not `signedIn` — is what fills a dot in. A free registered
   *  account is a RING in the same hue: identified, but not a customer, and the map has to
...
//
// The ramp itself, its stops and the reason for its ceiling are in mapTheme.ts,
// along with the empty-country fill, the border and the surface colour the dots
// ring themselves with.
...
//
//   SOLID        paying subscriber (active or trialing)
//   RING         signed-in account that is NOT paying
//   QUIET RING   anonymous visitor, known only by IP
//
// The account hue used to mean "signed in", which put a free registration and a paying
...
// against the ramp. A solid fill over the busiest (palest) country is weak at
...
// holds — so no dot goes missing at any step of the ramp. (The quiet ring alone would
...
// BUBBLE_FILL, BUBBLE_STROKE and MEMBER_FILL are in mapTheme.ts.
const MEMBER_STROKE = SURFACE;
/** Free registered account: the account hue, but hollow — the same dark disc the
 *  anonymous dot uses, ringed in the account hue instead of the quiet one. Reads as "we know who
...
function PlaceCard({ place, onClose }: { place: SelectedPlace; onClose: () => void }) {
  ...
  const Row = ({ label, value }: { label: string; value: ReactNode }) => (
    <div style={{ display: "flex", justifyContent: "space-between", gap: 10, fontSize: 14, padding: "4px 0" }}>
      <span style={{ color: HOME_THEME.textQuiet, minWidth: 0, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{label}</span>
    ...
    >
      <div style={{ padding: "10px 12px", borderBottom: `1px solid ${HOME_THEME.border}`, display: "flex", alignItems: "flex-start", gap: 8 }}>
        <div style={{ minWidth: 0, flex: 1 }}>
          ...
          {place.sub && (
            <div style={{ fontSize: 11, color: HOME_THEME.textQuiet, marginTop: 2, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
              {place.sub}
            </div>
          )}
        </div>
      ...
      <div className="owner-scroll" style={{ overflowY: "auto", padding: "8px 12px 12px" }}>
        ...
        {acct && (
          <>
            ...
            {acct.signedIn ? (
              ...
              <div style={{ fontSize: 14, color: HOME_THEME.textQuiet, padding: "4px 0", lineHeight: 1.5 }}>
            ...
            {recent.map((r, i) => (
              <div key={i} style={{ padding: "5px 0", borderBottom: i === recent.length - 1 ? "none" : `1px solid ${HOME_THEME.border}` }}>
                <div style={{ display: "flex", justifyContent: "space-between", gap: 8, fontSize: 14 }}>
                  ...
                  <span style={{ color: HOME_THEME.textQuiet, fontFamily: "var(--font-mono)", flexShrink: 0, fontSize: 10 }}>
                    {new Date(r.createdAt!).toLocaleString("en-US", { month: "short", day: "numeric", hour: "numeric", minute: "2-digit" })}
                  </span>
                </div>
                {r.ip && (
                  <div style={{ fontSize: 10, color: HOME_THEME.textQuiet, fontFamily: "var(--font-mono)", marginTop: 1 }}>
        ...
        {place.sample.length >= SAMPLE_CAP && (
          <div style={{ fontSize: 10, color: HOME_THEME.textQuiet, marginTop: 8 }}>
...
export function VisitorMap({ rows }: { rows: VisitorMapRow[] }) {
  ...
  const bubbleLayer = useMemo(() => bubbles.map((b) => (
    <circle
      ...
      cy={b.cy}
      r={b.r / sizeK}
      // Solid ONLY for a paying subscriber. A free account is the same hollow
      // disc as an anonymous visitor, ringed in the account hue instead.
    ...
    <div style={cardStyle}>
      ...
      >
        <div style={{ minWidth: 0 }}>
          ...
          <div style={{ display: "flex", alignItems: "baseline", gap: 9 }}>
            ...
            <span style={{ fontSize: 14, color: HOME_THEME.textQuiet }}>
      ...
      <div ref={wrapRef} style={{ position: "relative", width: "100%", minHeight: MIN_HEIGHT }}>
        ...
        {hover && (
          ...
          >
            ...
            {hover.kind === "visitor" && (
              <>
                <div style={{ fontSize: 14, color: HOME_THEME.textQuiet, marginTop: 3 }}>
      ...
      >
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          <span style={{ fontSize: 14, color: HOME_THEME.textQuiet }}>0</span>
          <span
            style={{
              width: 128,
              height: 8,
          ...
          />
          <span style={{ fontSize: 14, color: HOME_THEME.textQuiet, fontFamily: "var(--font-mono)" }}>
          ...
          <button
            ...
            title={
              stats.dots.length === 0
                ...
                : `${stats.dots.length} visitors · ${stats.subscriberDots} paying (solid), ${stats.signedInDots - stats.subscriberDots} free accounts (ring), ${stats.dots.length - stats.signedInDots} anonymous (quiet ring) · ${stats.cityDots} on a city coordinate across ${stats.placeCount} locations, ${stats.approxDots} dashed on a country centroid · zoom in to separate visitors sharing a place`
            }
            style={{
              marginLeft: 6,
              display: "flex",
              ...
              background: showBubbles && stats.dots.length ? ACCOUNT_SOFT : "transparent",
              color: HOME_THEME.text,
              fontSize: 14,
          ...
          {stats.approxDots > 0 && (
            <span
              style={{ fontSize: 14, color: HOME_THEME.textQuiet }}
        ...
        <div style={{ display: "flex", gap: 14, flexWrap: "wrap" }}>
          {stats.ranked.slice(0, 5).map((c) => (
            <span key={c.code} style={{ fontSize: 14, color: HOME_THEME.textQuiet }}>
              {c.name}{" "}
