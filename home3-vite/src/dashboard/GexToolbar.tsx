...
export default function GexToolbar(props: {
  ...
  onRefresh: () => void
  status: string
  chartSymbol: CandleSymbol; onChartSymbol: (v: CandleSymbol) => void
}) {
  const p = props
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '10px', flexWrap: 'wrap', flexShrink: 0 }}>
      <Seg options={[{ id: 'gex', label: 'GEX' }, { id: 'escandles', label: 'Candles' }]} value={p.view} onChange={p.onView} />
      {p.view === 'escandles' ? (
        <Seg
          options={[{ id: 'ES', label: 'ES' }, { id: 'NQ', label: 'NQ' }, { id: 'SPY', label: 'SPY' }, { id: 'QQQ', label: 'QQQ' }]}
          value={p.chartSymbol}
          onChange={p.onChartSymbol}
        />
      ) : (
        <>
