cat > /tmp/db-detail.mjs <<'ENDOFSCRIPT'
#!/usr/bin/env node
...
cd /opt/dashboard
docker compose cp /tmp/db-detail.mjs dashboard:/tmp/db-detail.mjs
docker compose exec -T dashboard node /tmp/db-detail.mjs
