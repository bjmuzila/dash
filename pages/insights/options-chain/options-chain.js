(() => {
  ...
  (function() {
    ...
    function setChainRange(pct) {
      _rangePercent = parseFloat(pct) || 10;
        ...
        .then(function(json) {
          ...
          if (!allSyms.length) {
            ...
            var checkReady = function() {
              ...
              if (_wsDataStartTime && elapsed >= _minWaitForWsMs) {
                finalizeLoad(preStrikes, token, 'LIVE');
              } else if (!_wsDataStartTime && elapsed >= _minWaitForWsMs) {
                console.log('[Chain] timeout waiting for WS, rendering with static data');
                finalizeLoad(preStrikes, token, 'LIVE (Static)');
              } else {
          ...
          // Subscribe via REST — WS path blocks non-SPXW symbols
          ...
          var checkReady = function() {
            ...
            if (_wsDataStartTime && elapsed >= _minWaitForWsMs) {
              finalizeLoad(preStrikes, token, 'LIVE');
            } else if (hasRestSnapshot && elapsed >= _minWaitForWsMs) {
              finalizeLoad(preStrikes, token, 'LIVE');
            } else if (!_wsDataStartTime && elapsed >= _minWaitForWsMs) {
              console.log('[Chain] timeout waiting for WS, rendering with static data');
              finalizeLoad(preStrikes, token, 'LIVE (Static)');
        ...
        strikeRows.forEach(function(item) {
          ...
          if (item.call) {
            ...
            if (r.callSym) _liveData[r.callSym] = {
              ...
              oi:    safeInt(item.call['open-interest']) ?? 0,
          ...
          if (item.put) {
            ...
            if (r.putSym) _liveData[r.putSym] = {
              ...
              oi:    safeInt(item.put['open-interest']) ?? 0,
    ...
    function renderTable() {
      ...
      console.log('[Chain] renderTable before filter: spot=' + spot + ', _strikes.length=' + _strikes.length + ', sortedStrikes.length=' + sortedStrikes.length + ', range=' + _rangePercent + '%');
      if (spot > 0) {
        var lowerBound = spot * (1 - pctRange);
        var upperBound = spot * (1 + pctRange);
        console.log('[Chain] filtering to ' + lowerBound.toFixed(2) + ' - ' + upperBound.toFixed(2));
        var filtered = sortedStrikes.filter(function(r) {
          return r.strike >= lowerBound && r.strike <= upperBound;
        });
        if (filtered.length > 0) {
          sortedStrikes = filtered;
        } else {
          console.log('[Chain] no strikes in range, showing all');
        }
      }
    ...
    function buildChainScreenshot(cb) {
      ...
      var pctRange = _rangePercent / 100;
      console.log('[Screenshot] spot='+spot+', range='+_rangePercent+'%, total strikes: '+_strikes.length);
      var lowerBound = spot * (1 - pctRange);
      var upperBound = spot * (1 + pctRange);
      console.log('[Screenshot] filtering to: '+lowerBound.toFixed(2)+' - '+upperBound.toFixed(2));
      var filtered = _strikes.filter(function(r) {
        return r.strike >= lowerBound && r.strike <= upperBound;
      }).sort(function(a,b){ return b.strike - a.strike; });
      console.log('[Screenshot] filtered strikes: '+filtered.length);
