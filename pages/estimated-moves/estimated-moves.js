    ...
    if (!EM.bulkSubscribed) {
      try{
        ...
        await fetch(EM.proxyUrl('/proxy/dxlink/subscribe'), {
          ...
          body:JSON.stringify({symbols:['SPX','VIX','ESM','NQM','SPY','QQQ','SMH','AAPL','AMD','AMZN','GOOGL','META','MSFT','NVDA','TSLA','COIN','HOOD','IWM','NFLX','PLTR','NDX']})
        });
        EM.bulkSubscribed = true;
        // Wait for dxLink Greeks to populate — 1500ms is not enough on first load
        EM.setStatus('Waiting for Greeks…','#00e5ff');
