...
const DB = {
  ...
  // ========================================================================
  async saveMVCSnapshot(mvcOIVol, mvcVolOnly, currentPrice, expiration = '—', triggerType = 'manual', totalNetGEX = 0) {
    const now = new Date();
    ...
    // Calculate % of net GEX
    const pctOIVol = totalNetGEX !== 0 ? (Math.abs(mvcOIVol.value) / Math.abs(totalNetGEX) * 100) : 0;
    const pctVolOnly = totalNetGEX !== 0 ? (Math.abs(mvcVolOnly.value) / Math.abs(totalNetGEX) * 100) : 0;
    ...
    const record = {
      ...
      pctNetGEXOIVol: parseFloat(pctOIVol.toFixed(2)),
      volumeOIVol: mvcOIVol.volume,
      strikeVolOnly: mvcVolOnly.strike,
      mvcValueVolOnly: mvcVolOnly.value,
      pctNetGEXVolOnly: parseFloat(pctVolOnly.toFixed(2)),
      volumeVolOnly: mvcVolOnly.volume,
      currentPrice: currentPrice,
      totalNetGEX: totalNetGEX
    };
    console.log('📊 MVC snapshot saving:', {expiration, triggerType, pctNetGEXOIVol: record.pctNetGEXOIVol, totalNetGEX});
