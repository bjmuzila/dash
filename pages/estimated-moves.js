...
EM.normalizeOptions = function(chain){
  return (Array.isArray(chain?.options) ? chain.options : []).map(o=>({
    ...
    iv:Number(o.iv || o.impliedVolatility || o.volatility || 0),
    dte:Number(o.dte || o.daysToExpiration || 0)
  })).filter(o=>o.expiration && Number.isFinite(o.strike));
};

EM.mid = function(o){ 
  return (o.bid && o.ask && o.bid>0 && o.ask>0) ? (o.bid+o.ask)/2 : 0; 
};
...
EM.estimateMove = async function(ticker){
  ...
  if(!Number.isFinite(strike)) throw new Error('No options');
  const call=expOptions.find(o=>o.strike===strike && o.type==='CALL');
  const put=expOptions.find(o=>o.strike===strike && o.type==='PUT');
  if(!call || !put) throw new Error('No straddle');
  const avgIV = ((call.iv || 0) + (put.iv || 0)) / 2;
  let dte = call.dte || put.dte || 0;
  if(dte <= 0) dte = EM.daysTo(targetExp);
  const midPrice = EM.mid(call) + EM.mid(put);
  let em = (avgIV > 0 && dte > 0) ? 0.84 * avgIV * close * Math.sqrt(dte / 365) : midPrice;
  if(!Number.isFinite(em) || em <= 0) em = midPrice || 0;
...
window.onMonthlyExpChange = function(val){ EM.monthlyExpOverride=val; };
