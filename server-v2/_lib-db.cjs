...
__export(db_exports, {
  ...
  listPropRecurring: () => listPropRecurring,
...
// was wrong in two ways at once: ensureAllTables is ONE enormous statement, so
// anything that throws earlier in it skips everything after; and getDb() sets
...
async function getDb() {
  ...
  if (!_tablesEnsured) {
    ...
    try {
      ...
      console.warn('[db] ensureAllTables failed:', String(err && err.message).slice(0, 200));
    }
  }
