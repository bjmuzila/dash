...
function HistoryChart({ history, metric }: { history: Snapshot[]; metric: MetricKey }) {
  ...
  if (pts.length < 2) {
    return <div style={{ padding: 24, textAlign: "center", color: HOME_THEME.muted, fontSize: 14 }}>
...
export default function WatchPage() {
  ...
  const label: React.CSSProperties = {
    fontSize: 14, fontWeight: 700, color: HOME_THEME.muted,
    textTransform: "uppercase", letterSpacing: ".1em",
  };
  const input: React.CSSProperties = { ...homeInputStyle, width: "100%", fontSize: 14, colorScheme: "dark" };
  ...
  const statLabel: React.CSSProperties = {
    fontSize: 14, fontWeight: 700, letterSpacing: ".1em", textTransform: "uppercase", color: "rgba(255,255,255,0.45)",
  };
  const statValue: React.CSSProperties = {
    fontSize: 14, fontWeight: 700, color: HOME_THEME.text, fontFamily: "var(--font-mono)",
  };
    ...
    <div style={homeShellStyle}>
      ...
      <div style={homeHeaderStyle}>
        <div style={{ display: "flex", alignItems: "center", gap: 12, flexWrap: "wrap" }}>
          <span style={{ fontSize: 17, fontWeight: 800, textTransform: "uppercase", letterSpacing: ".12em", color: HOME_THEME.cyan }}>
            Owner · Watch
          </span>
          <span style={{ fontSize: 14, color: HOME_THEME.text, opacity: 0.8, fontFamily: "var(--font-mono)" }}>
      ...
      <div style={{ ...homeContentStyle, overflow: "auto" }}>
        ...
        {err && (
          <div style={{ ...homePanelStyle, padding: "10px 14px", color: HOME_THEME.red, fontSize: 14, borderLeft: `2px solid ${HOME_THEME.red}` }}>
            {err}
          </div>
        )}
                ...
                >
                  ...
                  <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 8 }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
                      <span style={{ fontSize: 17, fontWeight: 800, color: HOME_THEME.text }}>{r.ticker}</span>
                      <span style={{ fontSize: 14, fontWeight: 700, padding: "2px 7px", borderRadius: 4, color: sideCol, background: rgba(sideCol, 0.12), border: `1px solid ${rgba(sideCol, 0.3)}` }}>
                  ...
                  <div style={{ fontSize: 14, color: HOME_THEME.muted, marginTop: 4 }}>
                    {r.expiration}{r.note && <span style={{ fontStyle: "italic" }}> · {r.note}</span>}
                  </div>
                  <div style={{ display: "flex", alignItems: "baseline", gap: 10, marginTop: 14 }}>
                    <span style={{ fontSize: 14, fontWeight: 800, color: HOME_THEME.cyan, fontFamily: "var(--font-mono)" }}>
                      {fmt(s?.mark)}
                    </span>
                  ...
                  <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginTop: 6 }}>
                    <span style={{ fontSize: 14, color: HOME_THEME.muted }}>Updated {timeAgo(s?.ts)}</span>
                    <span style={{ color: HOME_THEME.muted, fontSize: 14, transition: "transform .15s", transform: isOpen ? "rotate(90deg)" : "none", display: "inline-block" }}>▶</span>
                  </div>
                  ...
                  {isOpen && (
                    <div onClick={(e) => e.stopPropagation()} style={{ marginTop: 14, paddingTop: 14, borderTop: `1px solid ${HOME_THEME.border}`, cursor: "default" }}>
                          ...
                          {RANGES.map((r) => {
                            ...
                            return (
                              <button key={r.key} onClick={() => changeRange(r.key)} style={{
                                fontSize: 14, fontWeight: 700, padding: "4px 10px", borderRadius: 6, cursor: "pointer",
                      ...
                      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 10, marginBottom: 10 }}>
                        ...
                        <span style={{ fontSize: 14, color: HOME_THEME.muted, fontFamily: "var(--font-mono)" }}>
                      ...
                      {historyLoading && !hist.length
                        ? <div style={{ padding: 24, textAlign: "center", color: HOME_THEME.muted, fontSize: 14 }}>Loading history…</div>
        ...
        <div style={{ fontSize: 14, color: HOME_THEME.muted, padding: "0 2px" }}>
