...
export default function EsCandlesPage({ leading, embedded = false }: { leading?: ReactNode; embedded?: boolean } = {}) {
  ...
  // so rebuilding it every render would defeat the memo on every parent render.
  //
  ...
  return (
    <div className="es-candles-page flex h-full flex-col" style={{ background: HOME_THEME.bg, backgroundImage: HOME_THEME.shellGlow }}>
      ...
      {/* No padding of its own — the portaled dock brings px-4 pt-2, and setting
          it here too stacked the two into a band of empty bar above the toolbar. */}
      {multi && (
        <div ref={setDockTarget} style={{ position: "relative", zIndex: 35, minWidth: 0 }} />
      )}
