...
export interface MountOpts {
  ...
  /**
   * Is the frame this chart lives in worth painting right now? `ChartFrame`
   * hands the card exactly this predicate on `handle.visible` — pass it
   * straight through.
   *
   * NON-NEGOTIABLE 5. Without it the `draw()` loop below runs its full bubble +
   * rail redraw for a card that is scrolled out of the board's viewport, and on
   * a twelve-card board the cards below the fold are most of the frame budget.
   * A steady rAF loop is the case the rule says to answer with `handle.visible()`
   * rather than `onVisibility` — the loop keeps ticking while hidden, it just
   * does nothing, and it repaints itself on the first frame back in view
   * (see `missedWhileHidden`).
   *
   * Optional so a caller that has no frame (a test, a standalone mount) still
   * type-checks; absent means "always paint", which is the old behaviour.
   */
  visible?: () => boolean
}
...
export async function mountEsChart(container: HTMLElement, mountOpts: MountOpts): Promise<EsChartHandle> {
  ...
  // The visibility predicate, resolved once. `mountOpts.visible` is
  // ChartFrame's `handle.visible` — see MountOpts.visible.
  const isVisible = mountOpts.visible ?? (() => true)
  /** Did the loop skip a frame because the card was off screen? */
  let missedWhileHidden = false

  function draw() {
    raf = requestAnimationFrame(draw)

    // ── A card nobody can see does not paint (non-negotiable 5) ───────────────
    // The loop keeps being scheduled — cancelling it would mean re-arming it
    // from the visibility edge, and rAF is already stopped outright by a hidden
    // TAB. What it must not do is WORK: below this line is a full overlay
    // rebuild (up to ~320 bubble segments, each with its own priceToCoordinate)
    // for pixels inside a clipped scroll container nobody is looking at.
    //
    // The gate is checked before readPlotW(), so a hidden card costs one boolean
    // per frame and nothing else.
    if (!isVisible()) {
      missedWhileHidden = true
      return
    }
    // First frame back in view. The view almost certainly moved while we were
    // away (live bars kept arriving), and the signature we are about to compute
    // could still match the last one we DREW, so clear it and force one repaint.
  ...
  return {
    setBars(bars, reframe = false) {
      ...
      if (reframe && bars.length) {
        ...
        // ── A CORRECTION, NOT A WRITE EVERY FRAME ──────────────────────────
        // `shiftVisibleRangeOnNewBar` is off while locked (see setAxisLock), so
        // this should already be a no-op — appending a bar no longer moves the
        // view and there is nothing to put back. It stays as the backstop for
        // the cases that CAN still move a range under us (a resize landing in
        // the same tick, a library-side clamp when the bar count drops), and it
        // is guarded so it only writes when the range actually moved.
        //
        // Writing unconditionally is what made the bubbles jitter: the library
        // shifted the range on the appended bar, this shifted it back, and the
        // overlay — which is painted from our own rAF, not from the library's
        // commit — sampled the time scale somewhere between the two. One BAR of
        // disagreement, which at 5m (≈19px a bar) is a visible jump and at 1m
        // (≈4px) is a shimmer. The candles looked fine because the library draws
        // them inside its own commit, after the argument is settled.
        //
        // Deliberately NOT falling through to reanchorIfStranded either: "the
        // series shrank and the pane went empty" is the NORMAL state of a
        // rewound chart scrubbed back toward the open, and re-anchoring on it is
        // the jump the lock exists to stop.
        if (heldRange) {
          let now: { from: number; to: number } | null = null
          try {
            now = ts.getVisibleLogicalRange()
          } catch {
            now = null
          }
          const moved =
            !now ||
            Math.abs(now.from - heldRange.from) > 0.01 ||
            Math.abs(now.to - heldRange.to) > 0.01
          if (moved) {
            try {
    ...
    setAxisLock(on, range) {
      ...
      // ── THE TIME HALF: STOP THE SHIFT, DO NOT UNDO IT ────────────────────
      // lightweight-charts scrolls the visible range by one bar whenever a bar
      // is APPENDED (`shiftVisibleRangeOnNewBar`, on by default) — which is
      // right for a live tape and wrong for a rewound one, where every step of
      // the cursor appends a bar. The first version of this lock left that
      // behaviour alone and put the range back afterwards in `setBars`, and the
      // two writes per frame are what the bubbles were jittering between: the
      // overlay is painted from our own rAF and can sample the time scale before
      // the correction lands, so the marks pick one of two positions a BAR
      // apart. At 5m that bar is ~19px wide.
      //
      // Off at the source instead, so there is nothing to correct and nothing to
      // race. Back on when the lock is released, because a live chart following
      // the tape is exactly what the option is for.
      try {
