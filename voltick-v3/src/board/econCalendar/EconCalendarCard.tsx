...
export function EconCalendarCard() {
  ...
  useEffect(() => {
    // A hidden tab does not need the clock: `now` only moves rows from ahead to
    // past, and nobody is reading them. Left ungated it re-rendered this card —
    // the week's calendar plus the earnings table — once a minute forever in a
    // background tab. The visibilitychange handler snaps it current the moment
    // the tab comes back, so nothing is ever stale on screen.
    const fire = () => {
