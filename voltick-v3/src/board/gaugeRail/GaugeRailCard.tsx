...
function useDelta15m(values: Record<string, number | null>): Record<string, number | null> {
  ...
  useEffect(() => {
    ...
    // does not sample. This one fires every FIVE seconds and writes React state
    // on a card whose rings only ever move once a minute, so a backgrounded
    // board was re-rendering the rail twelve times for every bucket it could
    // possibly fill. One tick on the way back in catches the gap up.
    const fire = () => {
