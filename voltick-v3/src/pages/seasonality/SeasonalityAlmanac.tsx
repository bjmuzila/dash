...
import { T, alpha, mix } from "@/design/theme";
import { HOME_THEME, ES_CANDLE_UP, ES_CANDLE_DOWN } from "./homeTheme";
...
const A3 = "var(--color-sea-overlay-1)";
const INK = HOME_THEME.text;   // the only text color on this page

/**
 * The white washes and the heat base, named once.
 *
 * `alpha()` over a token rather than a typed rgba(): the value keeps tracking
 * --color-fg, and the literal is banned outright (check-theme, non-negotiable
 * #1). GRID_ZERO is the axis, GRID an ordinary gridline — the two-step
 * contrast is what says which line is zero.
 *
 * All of these are CSS strings, so on an SVG element they go through `style`
 * and NEVER through a `fill=` / `stroke=` presentation attribute: those are
 * parsed as a <paint> and do not resolve var() or color-mix(), so the shape
 * would render unpainted with nothing to warn you.
 */
const GRID = alpha(T.text, 0.08);       // gridlines
const GRID_ZERO = alpha(T.text, 0.32);  // the zero line
const WASH_04 = alpha(T.text, 0.04);    // an unselected pill
const WASH_05 = alpha(T.text, 0.05);    // a hovered row, a bar track
const WASH_06 = alpha(T.text, 0.06);    // a table row rule

/** The neutral a heat cell starts from before it is mixed toward up/down. */
const HEAT_BASE = "var(--color-sea-heat-base)";
...
const NOTE: CSSProperties = { marginTop: 14, fontSize: "var(--text-sm)", lineHeight: 1.65, color: INK, maxWidth: 900 };
...
const pillBtn = (on: boolean): CSSProperties => ({
  ...
  background: on
    ? `linear-gradient(180deg, ${alpha(HOME_THEME.cyan, 0.2)}, ${alpha(HOME_THEME.cyan, 0.05)})`
    : WASH_04,
...
function Legend({ items }: { items: { color: string; label: string }[] }) {
  return (
    <div style={{ display: "flex", gap: 18, flexWrap: "wrap", fontSize: "var(--text-xs)", fontWeight: 700, color: INK, marginBottom: 12 }}>
      {items.map((i) => (
  ...
  return (
    <details ref={ref} className="sea-disc" open={open} style={{ marginTop: 14, border: `1px solid ${SEA.line}`, borderRadius: 12, background: SEA.card2 }}>
      ...
      >
        ...
        <span aria-hidden className="sea-disccaret" style={{ color: HOME_THEME.cyan, fontSize: "var(--text-2xs)", display: "inline-block" }}>▶</span>
        <span>{label}</span>
        {hint ? (
          <span style={{ fontSize: "var(--text-xs)", fontWeight: 600, letterSpacing: "0.02em", textTransform: "none", color: INK }}>
            {hint}
          </span>
        ) : null}
      </summary>
      ...
      {innerW > 0 ? (
        <>
          <svg width={width} height={height} role="img" style={{ display: "block", touchAction: "none" }} onPointerLeave={() => setHover(null)}>
            {ticks.map((t) => (
              <g key={`t${t}`}>
                <line x1={PAD.left} x2={PAD.left + innerW} y1={y(t)} y2={y(t)} style={{ stroke: t === 0 ? GRID_ZERO : GRID }} />
                <text x={PAD.left - 8} y={y(t) + 4} textAnchor="end" fontSize={10} style={{ fill: INK, fontVariantNumeric: "tabular-nums" }}>
                  {fmt(t)}
                </text>
              </g>
            ))}
            {values.map((v, i) => {
              ...
              return (
                <g key={labels[i]} onPointerEnter={() => setHover(i)}>
                  ...
                  <path d={barPath(x, w, up ? y(v) : y(0), Math.abs(y(v) - y(0)), up)} style={{ fill: up ? UP : DOWN }} opacity={hover == null || hover === i ? 1 : 0.55} />
                  {showValues ? (
                    <text x={x + w / 2} y={up ? y(v) - 6 : y(v) + 13} textAnchor="middle" fontSize={9.5} style={{ fill: INK, fontVariantNumeric: "tabular-nums" }}>
                      {fmt(v)}
                    </text>
                  ) : null}
                  <text x={x + w / 2} y={height - 9} textAnchor="middle" fontSize={10} fontWeight={700} style={{ fill: INK }}>
                    {labels[i]}
                  </text>
                </g>
              );
          ...
          <div style={{ minHeight: 20, fontSize: "var(--text-xs)", color: INK, fontVariantNumeric: "tabular-nums" }}>
          ...
          <svg width={width} height={height} role="img" style={{ display: "block", touchAction: "none" }} onPointerLeave={() => setHover(null)}>
            ...
            {labels.map((lab, i) => (
              <g key={lab} onPointerEnter={() => setHover(i)}>
                ...
                {i % labelEvery === 0 ? (
                  <text x={PAD.left + i * gw + gw / 2} y={height - 9} textAnchor="middle" fontSize={10} fontWeight={700} style={{ fill: INK }}>
                    {lab}
                  </text>
                ) : null}
              </g>
            ...
            {groups.map((g, i) => (
              <g key={g} onPointerEnter={() => setHover(i)}>
                ...
                {series.map((s, k) => {
                  ...
                  return (
                    <g key={s.label}>
                      <path d={barPath(x, bw, up ? y(v) : y(0), Math.abs(y(v) - y(0)), up)} style={{ fill: s.color }} opacity={hover == null || hover === i ? 1 : 0.45} />
                      <text x={x + bw / 2} y={up ? y(v) - 6 : y(v) + 13} textAnchor="middle" fontSize={9.5} style={{ fill: INK, fontVariantNumeric: "tabular-nums" }}>
                        {fmt(v)}
                      </text>
                    </g>
                  );
                })}
                <text x={PAD.left + i * gw + gw / 2} y={height - 9} textAnchor="middle" fontSize={11} fontWeight={800} style={{ fill: INK }}>
                  {g}
                </text>
              </g>
            ))}
...
function HBars({
  ...
  const panelHead = (title: string, d: { lo: number; hi: number }, fmt: (v: number) => string) => (
    <div style={{ width: panelW, minWidth: 0 }}>
      <div style={{ ...capLabel, fontSize: "var(--text-3xs)", marginBottom: 2, textAlign: "center" }}>{title}</div>
      <div style={{ fontSize: "var(--text-2xs)", color: INK, opacity: 0.8, display: "flex", justifyContent: "space-between", fontVariantNumeric: "tabular-nums" }}>
        <span>{fmt(d.lo)}</span>
        <span>{fmt(d.hi)}</span>
      </div>
    </div>
        ...
        <>
          ...
          <div style={{ maxHeight, overflowY: "auto", overflowX: "hidden" }}>
            ...
            >
              ...
              <line x1={scale(0, dA, LABEL_W)} x2={scale(0, dA, LABEL_W)} y1={0} y2={H} style={{ stroke: GRID_ZERO }} />
              <line
                x1={scale(0, dB, LABEL_W + panelW + GAP)}
                x2={scale(0, dB, LABEL_W + panelW + GAP)}
                y1={0}
                y2={H}
                style={{ stroke: GRID_ZERO }}
              />
              ...
              {rows.map((r, i) => {
                ...
                const bar = (v: number | null, d: { lo: number; hi: number }, x0: number, fixed?: string, fmt?: (n: number) => string) => {
                  ...
                  return (
                    <>
                      <rect x={x} y={yTop + 3} width={w} height={rowH - 7} rx={2} style={{ fill: col }} opacity={on ? 1 : 0.45} />
                      {room && fmt ? (
                        <text
                          x={tx}
                          y={yTop + rowH / 2 + 3}
                          ...
                          fontSize={9.5}
                          opacity={on ? 0.95 : 0.4}
                          style={{ fill: INK, fontVariantNumeric: "tabular-nums" }}
                        >
                          {fmt(v)}
                        </text>
                      ) : null}
                ...
                return (
                  <g key={r.key} onPointerEnter={() => setHover(i)}>
                    <rect x={0} y={yTop} width={width} height={rowH} style={{ fill: hover === i ? WASH_05 : "transparent" }} />
                    <text x={0} y={yTop + rowH / 2 + 4} fontSize={10.5} fontWeight={700} opacity={on ? 1 : 0.5} style={{ fill: INK, fontVariantNumeric: "tabular-nums" }}>
                      {r.label}
                    </text>
          ...
          <div style={{ minHeight: 20, marginTop: 6, fontSize: "var(--text-xs)", color: INK, fontVariantNumeric: "tabular-nums" }}>
...
function DataTable({ head, rows }: { head: string[]; rows: Cell[][] }) {
  return (
    <div style={{ overflowX: "auto" }}>
      <table style={{ borderCollapse: "collapse", width: "100%", fontSize: "var(--text-sm)", color: INK, fontVariantNumeric: "tabular-nums" }}>
        <thead>
          <tr>
                ...
                return (
                  <td
                    ...
                    style={{
                      ...
                      borderBottom: ri === rows.length - 1 ? "none" : `1px solid ${WASH_06}`,
                    }}
                  >
                    {cell?.bar != null ? (
...
/**
 * The heatmap ramp: a neutral cell colour run toward the up/down hue, so a zero
 * cell reads as "no signal" rather than as a washed-out version of one of the
 * two directions.
 *
 * `color-mix()` rather than the hand-rolled channel blend this used to be. Both
 * ends are now `var(--color-…)` — the browser is the only thing that can see
 * their values, and it does the same linear sRGB mix. Nothing here paints a
 * canvas, so a computed colour string is all the cell needs.
 */
function heatFill(to: string, t: number) {
  return mix(to, HEAT_BASE, t);
}
              ...
              {(data[i] ?? []).map((v, j) => (
                <td
                  ...
                  style={{
                    ...
                    textAlign: "center",
                    borderRadius: 3,
                    background: v == null ? "transparent" : heatFill(v >= 0 ? UP : DOWN, Math.pow(Math.min(1, Math.abs(v) / scale), 0.72)),
                  }}
                >
...
function Tile({ label, value, sub, color }: { label: string; value: ReactNode; sub?: string; color?: string }) {
  return (
    ...
    >
      ...
      <div style={{ fontSize: "var(--text-xl)", fontWeight: 800, marginTop: 4, fontVariantNumeric: "tabular-nums", color: color ?? INK }}>{value}</div>
      {sub ? <div style={{ fontSize: "var(--text-xs)", color: INK, marginTop: 3 }}>{sub}</div> : null}
    </div>
  );
}
    ...
    fomc: (
      ...
      >
        ...
        >
          ...
          />
          <div style={{ ...NOTE, fontSize: "var(--text-xs)" }}>
        ...
        {isMonthEnd ? (
          <div
            style={{
              ...
              padding: "10px 14px",
              borderRadius: 10,
              border: `1px solid ${alpha(HOME_THEME.cyan, 0.4)}`,
              background: `linear-gradient(180deg, ${alpha(HOME_THEME.cyan, 0.12)}, ${SEA.card2})`,
              fontSize: "var(--text-sm)",
              lineHeight: 1.55,
              color: INK,
            }}
    ...
    cycles: (
      <SeaCard title="Presidential &amp; Decennial Cycles" subtitle="Mean calendar-year return, price only" padding={20}>
        <div style={{ display: "grid", gap: 18, gridTemplateColumns: "repeat(auto-fit, minmax(min(320px,100%), 1fr))" }}>
          <div>
            <div style={{ fontSize: "var(--text-xs)", fontWeight: 800, color: INK, marginBottom: 8 }}>Four-year political cycle</div>
            <DivBars
          ...
          <div>
            <div style={{ fontSize: "var(--text-xs)", fontWeight: 800, color: INK, marginBottom: 8 }}>Decade digit</div>
            <DivBars
