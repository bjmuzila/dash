...
function firstVolumeNumber(...vals) {
  ...
  // Accept float-encoded whole numbers (e.g. 1540.0) as well as true integers
  const whole = nums.find(n => Math.floor(n) === n);
  return whole ?? nums[0] ?? 0;
}
function maxWholeNumber(...vals) {
  // Accept both true integers and float-encoded whole numbers (e.g. 1540.0 from TastyTrade REST)
  const nums = vals.map(finiteNumber).filter(n => n !== null && n >= 0 && Math.floor(n) === n);
...
async function fetchYahooOptionFallback(symbol, expDates) {
  const result = new Map();
  const yahooSymbol = symbol.toUpperCase() === 'SPX' ? '^SPX' : symbol.toUpperCase();
  await Promise.all((expDates || []).map(async expDate => {
    const unix = Math.floor(Date.parse(expDate + 'T00:00:00Z') / 1000);
    if (!Number.isFinite(unix)) return;
    try {
      const url = `https://query2.finance.yahoo.com/v7/finance/options/${encodeURIComponent(yahooSymbol)}?date=${unix}`;
      const json = await httpsGetJson(url);
      const opt = json?.optionChain?.result?.[0]?.options?.[0];
      const add = (side, row) => {
        const strike = Number(row?.strike);
        if (!Number.isFinite(strike)) return;
        result.set(`${expDate}|${strike}|${side}`, {
          openInterest: Number(row.openInterest) || 0,
          volume: Number(row.volume) || 0
        });
      };
      (opt?.calls || []).forEach(row => add('call', row));
      (opt?.puts || []).forEach(row => add('put', row));
    } catch (e) {
      log('Yahoo options fallback failed for', symbol, expDate, e.message);
    }
  }));
  return result;
}
...
function normalizeRequestedCandleSymbols(sym) {
  ...
  if (!match) return [raw];
  // For options (with {=timeframe}), return only the original symbol
  // Don't expand to multiple contracts
  return [raw];
}
    ...
    if (msg.type === 'AUTH_STATE') {
      ...
      dxSocket.send(JSON.stringify({
        ...
        acceptEventFields:{
          ...
          Summary: ['eventType','eventSymbol','openInterest','prevDayVolume','dayOpenPrice','dayHighPrice','dayLowPrice','prevDayClosePrice']
        }
      }));
      sendSubscriptions();
    }
      ...
      if (Array.isArray(msg.data) && msg.data.length >= 2) {
        ...
        if (typeof eventType === 'string' && Array.isArray(rows)) {
          ...
          const summaryFields = ['openInterest','prevDayVolume','dayOpenPrice','dayHighPrice','dayLowPrice','prevDayClosePrice'];
...
const server = http.createServer(async (req, res) => {
  ...
  // ── GET /proxy/api/tt/expirations/:symbol ── returns expiration dates only (no strikes)
  if (req.method === 'GET' && p.startsWith('/proxy/api/tt/expirations/')) {
    let sym = decodeURIComponent(p.slice('/proxy/api/tt/expirations/'.length).split('?')[0]);
    sym = sym.replace(/^\$/, '');
    const { status: sExp, data: dExp } = await ttGet(`/option-chains/${encodeURIComponent(sym)}/nested`);
    if (sExp !== 200 || !dExp?.data?.items?.length) return sendJSON(res, sExp, dExp);
    const chainObjExp = dExp.data.items.find(c => c['root-symbol'] === 'SPXW') || dExp.data.items[0];
    const expList = (chainObjExp.expirations || []).map(e => e['expiration-date']).filter(Boolean).sort();
    return sendJSON(res, 200, { expirations: expList });
  }
  ...
  if (req.method === 'GET' && p.startsWith('/proxy/api/tt/chains/')) {
    ...
    // If expiration is specified, skip the nested call and go straight to the chain fetch.
    // For SPX we always prefer the SPXW root (weeklies); for others we try the symbol directly.
    let rootSymbol = (sym === 'SPX' || sym === '$SPX') ? 'SPXW' : sym;
    let targetExps = [];

    if (exp) {
      // Expiration known — no nested call needed
      targetExps = [exp];
      log('chains skip-nested: using provided expiration', exp, 'for', rootSymbol);
    } else {
      // No expiration provided — fetch nested to find the nearest one
      const { status: s1, data: d1 } = await ttGet(`/option-chains/${encodeURIComponent(sym)}/nested`);
      if (s1 !== 200 || !d1?.data?.items?.length) return sendJSON(res, s1, d1);
      const chainObj = d1.data.items.find(c => c['root-symbol'] === 'SPXW') || d1.data.items[0];
      rootSymbol = chainObj['root-symbol'] || sym;
      const allExps = (chainObj.expirations || []).map(e => e['expiration-date']).filter(Boolean).sort();
      targetExps = allExps.slice(0, 1);
      log('chains root-symbol:', rootSymbol, 'nearest expiration:', targetExps[0]);
    }
    ...
    const atmOnly = u.searchParams.get('atmOnly') === 'true';
    if (atmOnly) {
      // Always fetch live price so ATM selection works for equities (inferUnderlyingPrice may return 0)
      const qp = await fetchUnderlyingLast(sym).catch(() => 0);
      const up = inferUnderlyingPrice(allOptions, qp) || qp;
      if (!(up > 0)) {
        return sendJSON(res, 200, { symbol: sym, underlyingPrice: 0, options: [] });
      }
      const byExp = {};
      filteredOptions.forEach(o => { const e=o['expiration-date']; if(!byExp[e]) byExp[e]=[]; byExp[e].push(o); });
      const atmOpts = [];
      Object.values(byExp).forEach(opts => {
        const strikes=[...new Set(opts.map(o=>parseFloat(o['strike-price']||0)))].filter(s=>s>0).sort((a,b)=>Math.abs(a-up)-Math.abs(b-up));
        if (!strikes.length) return;
        const atm=strikes[0];
        opts.filter(o=>parseFloat(o['strike-price']||0)===atm).forEach(o=>atmOpts.push(o));
      });
      log('atmOnly:', sym, 'price', up, '→', atmOpts.length, 'ATM options');
      // Subscribe ATM streamer-symbols to dxLink and wait briefly for live quotes
      const atmSyms = atmOpts.map(o => o['streamer-symbol']).filter(Boolean);
      if (atmSyms.length && dxSocket && dxSocket.readyState === WebSocket.OPEN && dxChannelOpen) {
        const add = atmSyms.flatMap(s => [
          { type: 'Quote', symbol: s },
          { type: 'Trade', symbol: s }
        ]);
        dxSocket.send(JSON.stringify({ type: 'FEED_SUBSCRIPTION', channel: DX_CHANNEL, reset: false, add }));
        atmSyms.forEach(s => subscriptions.add(s));
        // Wait up to 3s for quotes to arrive in cache
        const waitStart = Date.now();
        while (Date.now() - waitStart < 3000) {
          const allReady = atmSyms.every(s => {
            const q = dxQuoteCache[s]; const t = dxTradeCache[s];
            return (q?.bidPrice > 0 && q?.askPrice > 0) || (t?.price > 0);
          });
          if (allReady) break;
          await new Promise(r => setTimeout(r, 100));
        }
        log('atmOnly: waited', Date.now() - (Date.now()), 'ms for quotes');
      }
      return sendJSON(res, 200, { symbol: sym, underlyingPrice: up, options: atmOpts.map(o => ({
        expiration: o['expiration-date'],
        strike: parseFloat(o['strike-price']||0),
        optionType: (o['option-type']||'').toUpperCase(),
        bid: firstFiniteNumber(dxQuoteCache[o['streamer-symbol']]?.bidPrice, o.bid, o['bid-price'], 0),
        ask: firstFiniteNumber(dxQuoteCache[o['streamer-symbol']]?.askPrice, o.ask, o['ask-price'], 0),
        last: firstFiniteNumber(dxTradeCache[o['streamer-symbol']]?.price, o.last, o['last-price'], o['close-price'], 0),
        iv: firstFiniteNumber(dxGreeksCache[o['streamer-symbol']]?.volatility, o['implied-volatility'], o['iv'], 0),
        dte: Math.max(0, Math.ceil((new Date(o['expiration-date']+'T16:00:00') - new Date()) / 86400000)),
      })) });
    }

        const yahooFallback = await fetchYahooOptionFallback(sym, targetExps).catch(e => {
      log('Yahoo options fallback unavailable:', e.message);
      return new Map();
    });
    if (yahooFallback.size) log('Yahoo options fallback rows:', yahooFallback.size);
    ...
    for (const opt of filteredOptions) {
      ...
      const quote   = dxQuoteCache[streamerSym]   || {};
      const trade   = dxTradeCache[streamerSym]   || {};
      const yahoo   = yahooFallback.get(`${expDate}|${strikePrice}|${side}`) || {};
      ...
      expMap[expDate].strikes[strikePrice][side] = {
        ...
        'open-interest':      maxWholeNumber(opt['open-interest'], opt.openInterest, summary.openInterest, yahoo.openInterest),
        volume:               maxWholeNumber(trade.dayVolume, opt['day-volume'], opt['day-volume-count'], opt['volume-count'], opt.volume, opt['volume'], yahoo.volume),
        _rawOpenInterest:     opt['open-interest'] ?? opt.openInterest ?? null,
        _summaryOpenInterest: summary.openInterest ?? null,
        _yahooOpenInterest:   yahoo.openInterest ?? null,
        _rawVolume:           opt.volume ?? opt['volume'] ?? opt['day-volume'] ?? null,
        _tradeDayVolume:      trade.dayVolume ?? null,
        _summaryPrevDayVolume: summary.prevDayVolume ?? null,
        _yahooVolume:         yahoo.volume ?? null,
  ...
  if (req.method === 'GET' && p === '/proxy/api/dxlink/candles') {
    ...
    const waitTimeout = 15000;  // 15s timeout for initial data
...
async function prewarmCache() {
  ...
  try {
    ...
    // Filter to ATM ±5 strikes only
    const atmOptions = allOptions.filter(opt => Math.abs((opt['strike-price'] || 0) - underlyingPrice) <= 50);
    const allSyms = pickNearestOptionStreamerSymbols(atmOptions.length > 0 ? atmOptions : allOptions, underlyingPrice, 50);
